...
lstmn=6L & lstdy=1L & lstyr=2008L
ledmn=6L & leddy=30L & ledyr=2008L
lstday=0L & ledday=0L
;
; get date range
;
...
save,file='geos5_pv_freq_maxout_nh_'+syr+smn+'.sav',pvfreq,sdates,sdate0,sdate1,th
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='geos5_nh_pv_'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_maxout.ps'
   device,/color
...
plot,pvfreq,th,xrange=[0.,100.],yrange=[min(th),max(th)],color=0,title=sdate0+'-'+sdate1,$
     xtitle='Percent of the time max PV in high SF bins',ytitle='Theta (K)',thick=5

if setplot ne 'ps' then stop
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim geos5_nh_pv_'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_maxout.ps -rotate -90 '+$
         'geos5_nh_pv_'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_maxout.jpg'
   spawn,'/usr/bin/rm geos5_nh_pv_'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_maxout.ps'
endif
end
