...
; --- Test for end condition and close windows.
      ...
      if ndays gt ledday then begin
         ...
         if setplot eq 'ps' then begin
            device, /close
            spawn,'convert -trim polar_temp+mark_geos5-mls_'+stheta+'km_4days.ps '+$
                  ' -rotate -90  polar_temp+mark_geos5-mls_'+stheta+'km_4days.jpg'
            stop
         endif
      endif
...
;        !p.font=0
         ...
         device,/landscape,bits=8,$
                 filename='polar_temp+mark_geos5-mls_'+stheta+'km_4days.ps'
         device,/color
...
; day 1
;
if imn eq 7 and idy eq 13 then begin
erase
xmn=xorig(0)
xmx=xorig(0)+xlen
ymn=yorig(0)
...
MAP_SET,90,0,-90,/stereo,/noeras,/grid,/contin,title='GEOS-5',charsize=1.5
nlvls=19
col1=1+indgen(nlvls)*icolmax/nlvls
level=130.+3.*findgen(nlvls)
...
contour,geosmark,x,latbin,/overplot,levels=[0.1],/follow,color=0,c_labels=0*level,thick=5
contour,geosmark,x,latbin,/overplot,levels=[-0.1],/follow,color=mcolor,c_labels=0*level,thick=5
MAP_SET,90,0,-90,/stereo,/noeras,/contin
...
!type=2^2+2^3
!psym=0
MAP_SET,90,0,-90,/stereo,/noeras,/grid,/contin,title='MLS',charsize=1.5
...
ymx=yorig(2)+ylen
set_viewport,xmn,xmx,ymn,ymx
MAP_SET,90,0,-90,/stereo,/noeras,/grid,/contin,title='GEOS-MLS',charsize=1.5
...
; day 2
;
if imn eq 7 and idy eq 14 then begin
xmn=xorig(3)
xmx=xorig(3)+xlen
ymn=yorig(3)
ymx=yorig(3)+ylen
...
; day 3
;
if imn eq 7 and idy eq 15 then begin
xmn=xorig(6)
xmx=xorig(6)+xlen
ymn=yorig(6)
ymx=yorig(6)+ylen
...
; day 4
;
if imn eq 7 and idy eq 16 then begin
xmn=xorig(9)
xmx=xorig(9)+xlen
ymn=yorig(9)
ymx=yorig(9)+ylen
