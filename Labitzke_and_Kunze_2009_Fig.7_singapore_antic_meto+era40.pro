...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          ...
          filename='Labitzke_and_Kunze_2009_Fig.7_singapore_antic_meto+era40_'+slev+'_Feb.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
;
; restore ERA40 marker zonal mean counts
;
restore,'ERA40_antic_area_alllat_1957-1991.sav
deltax=3.75
nc=long(360./deltax)
...
tpavg30=markavg
index=where(tpavg30 gt 0. and uavgeq50 lt 0.,nn)
a=findgen(9)*(2*!pi/8.)
usersym,cos(a),sin(a)
...
a=findgen(9)*(2*!pi/8.)
usersym,cos(a),sin(a),/fill
index=where(tpavg30 gt 0. and uavgeq50 lt 0. and uavg60N10 lt 0.,nn)
oplot,f10_feb(index),tpavg30(index),psym=8,color=0
for i=0L,nn-1L do xyouts,f10_feb(index(i)),tpavg30(index(i)),strmid(strcompress(years(index(i)),/remove_all),2,2),$
    charsize=1,/data,color=0
xyouts,xmx,ymx+0.04,'Feb',/normal,color=0,charsize=2
;
; QBO changed sign
;
;loadct,39
...
usersym,cos(a),sin(a),/fill
loadct,0
xyouts,210.,.3,'ERA',color=0,/data
oplot,[200.,200.],[.3,.3],psym=8,color=0,symsize=1.25
xyouts,210.,.1,'MetO',color=150,/data
oplot,[200.,200.],[.1,.1],psym=8,color=150,symsize=1.25
;
; oplot MetO
;
;mtpavg30=mgpavg30/1000.
mtpavg30=markavg
index=where(mtpavg30 gt 0. and muavgeq50 lt 0. and muchgeq50 ne 1. and years gt 1991,nn)
a=findgen(9)*(2*!pi/8.)
usersym,cos(a),sin(a)
...
a=findgen(9)*(2*!pi/8.)
usersym,cos(a),sin(a),/fill
index=where(mtpavg30 gt 0. and muavgeq50 lt 0. and muavg60N10 lt 0. and years gt 1991,nn)
...
;a=findgen(9)*(2*!pi/8.)
;usersym,cos(a),sin(a)
;index=where(mtpavg30 gt 0. and muavgeq50 lt 0. and muavgeq50 gt -5 and years gt 1991,nn)
...
;
index=where(tpavg30 le 0.)
tpavg30(index)=0./0.
index=where(mtpavg30 le 0.)
mtpavg30(index)=0./0.
index=where(muavgeq50 eq -99.)
muavgeq50(index)=0./0.
index=where(finite(tpavg30) eq 0 and finite(mtpavg30) ne 0)
filltpavg30=tpavg30
if index(0) ne -1L then filltpavg30(index)=mtpavg30(index)
;
...
ymx=yorig(1)+ylen
set_viewport,xmn,xmx,ymn,ymx
index=where(tpavg30 gt 0. and uavgeq50 gt 0.,nn)
a=findgen(9)*(2*!pi/8.)
usersym,cos(a),sin(a)
...
a=findgen(9)*(2*!pi/8.)
usersym,cos(a),sin(a),/fill
index=where(tpavg30 gt 0. and uavgeq50 gt 0. and uavg60N10 lt 0.,nn)
oplot,f10_feb(index),tpavg30(index),psym=8,color=0
for i=0L,nn-1L do xyouts,f10_feb(index(i)),tpavg30(index(i)),strmid(strcompress(years(index(i)),/remove_all),2,2),$
    charsize=1,/data,color=0
;
...
; oplot MetO
;
index=where(mtpavg30 gt 0. and muavgeq50 gt 0. and years gt 1991,nn)
a=findgen(9)*(2*!pi/8.)
usersym,cos(a),sin(a)
oplot,f10_feb(index),mtpavg30(index),color=0,psym=8
for i=0L,nn-1L do xyouts,f10_feb(index(i)),mtpavg30(index(i)),strmid(strcompress(years(index(i)),/remove_all),2,2),$
...
a=findgen(9)*(2*!pi/8.)
usersym,cos(a),sin(a),/fill
index=where(mtpavg30 gt 0. and muavgeq50 gt 0. and muavg60N10 lt 0. and years gt 1991,nn)
oplot,f10_feb(index),mtpavg30(index),psym=8,color=150
for i=0L,nn-1L do xyouts,f10_feb(index(i)),mtpavg30(index(i)),strmid(strcompress(years(index(i)),/remove_all),2,2),$
    charsize=1,/data,color=0
xyouts,210.,.3,'ERA',color=0,/data
...
;a=findgen(9)*(2*!pi/8.)
;usersym,cos(a),sin(a)
;index=where(mtpavg30 gt 0. and muavgeq50 gt 0. and muavgeq50 lt 5 and years gt 1991,nn)
;if index(0) ne -1L then oplot,f10_feb(index),mtpavg30(index),psym=8,color=mcolor*.4
;
; save jpg
;
...
if setplot eq 'ps' then begin
device,/close
spawn,'convert -trim Labitzke_and_Kunze_2009_Fig.7_singapore_antic_meto+era40_'+slev+'_Feb.ps -rotate -90 Labitzke_and_Kunze_2009_Fig.7_singapore_antic_meto+era40_'+slev+'_Feb.jpg'
endif
