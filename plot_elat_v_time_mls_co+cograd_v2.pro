...
; 2 ways to minimize intra-vortex gradient maxima: divide by CO value or multiply by cos(elat). Neither work
    ...
    index=where(dcoday eq max(dcoday))
...
contour,ytco_big,findgen(nfile),yeq,levels=colevel,xrange=[0,nfile],yrange=[40,90],/noeras,color=0,c_color=col1,title='Mean CO',ytitle='Equivalent Latitude',xticks=nxticks-1,xtickname=xlabs,xtickv=xindex,charsize=1.5,charthick=2
oplot,findgen(nfile),smooth(elatedge,7,/edge_truncate,/Nan),color=0,thick=3
...
colevel=imin+((imax-imin)/float(nlvls-1))*findgen(nlvls)
contour,ytdco_big,findgen(nfile),yeq,levels=colevel,xrange=[0,nfile],yrange=[40,90],/noeras,color=0,c_color=col1,title='dCO/dElat',ytitle='Equivalent Latitude',xticks=nxticks-1,xtickname=xlabs,xtickv=xindex,charsize=1.5,charthick=2
oplot,findgen(nfile),smooth(elatedge,7,/edge_truncate,/Nan),thick=3,color=0
;
; CO tendency
;
dcodt=0.*ytco_big
for j=0L,n_elements(yeq)-1L do begin
    covals=reform(ytco_big(*,j))
    result=deriv(covals)
    dcodt(*,j)=abs(result)
;print,yeq(j)
endfor
dcodt=smooth(dcodt,7,/edge_truncate,/Nan)
...
nlvls=26L
col1=1+(indgen(nlvls)/float(nlvls))*mcolor
index=where(abs(dcodt) lt 10. and finite(dcodt) eq 1)
imin=min(dcodt(index))
imax=max(dcodt(index))
colevel=imin+((imax-imin)/float(nlvls-1))*findgen(nlvls)
contour,dcodt,findgen(nfile),yeq,levels=colevel,xrange=[0,nfile],yrange=[40,90],/noeras,color=0,c_color=col1,title='CO Tendency',ytitle='Equivalent Latitude',xticks=nxticks-1,xtickname=xlabs,xtickv=xindex,charsize=1.5,charthick=2
...
ytprod_big=0.*ytco_big
ytprod_big=dcodt+ytdco_big
;ytprod_big=smooth(ytprod_big,7,/edge_truncate,/Nan)
xmn=xorig(3)
xmx=xorig(3)+xlen
ymn=yorig(3)
ymx=yorig(3)+ylen
...
imin=min(ytprod_big(index))
imax=max(ytprod_big(index))
colevel=imin+((imax-imin)/float(nlvls-1))*findgen(nlvls)
contour,ytprod_big,findgen(nfile),yeq,levels=colevel,xrange=[0,nfile],yrange=[40,90],/noeras,c_color=col1,color=0,title='dCO/dElat + CO Tend',ytitle='Equivalent Latitude',xticks=nxticks-1,xtickname=xlabs,xtickv=xindex,charsize=1.5,charthick=2
;
; compute edge based on both horizontal and time gradients
;
elatedge2=fltarr(nfile)
for i=0L,nfile-1L do begin
;
; 2 ways to minimize intra-vortex gradient maxima: divide by CO value or multiply by cos(elat). Neither work
    dcoday=reform(ytprod_big(i,*))       ;/reform(ytco_big(i,*)) ;*cos(yeq*!pi/180.)
    index=where(dcoday eq max(dcoday))
    elatedge2(i)=yeq(index(0))
endfor
index=where(elatedge2 le 30.)
if index(0) ne -1L then elatedge2(index)=0./0.
oplot,findgen(nfile),smooth(elatedge2,7,/edge_truncate,/Nan),color=0,thick=2
