...
;
dir='/aura7/harvey/WACCM_data/Datfiles/Datfiles_Brakebusch/wcm_geos_50-60km_daily_2x_0.1rlx.'
re=40000./2./!pi
...
sdate=result2(0)
print,sdate
dum=findfile(pthout+'dmps_mls_'+sver+'.waccm.'+sdate+'.sav')
...
; based on fractional year and day and analyses valid at 00Z
;
...
;
    newday=0L
    ifile=dir+sdate0+'.nc3'
    dum=findfile(ifile)
    if dum(0) eq '' then goto,jumpprof
    if dum(0) ne '' and icount eq 0L then begin
       rd_waccm_nc3_meto_brakebush,dum(0),ncw,nrw,nthw,alon,alat,th,$
          pvold,pold,uold,vold,qdfold,markold,sfold,vpold,iflag
       icount=1L
       newday=1L
       print,'reading '+ifile
       file1=ifile
...
; read data on day 1
;
    ifile=dir+sdate1+'.nc3'
    dum=findfile(ifile)
    if dum(0) ne '' and icount2 eq 0L then begin
       rd_waccm_nc3_meto_brakebush,dum(0),ncw,nrw,nthw,alon,alat,th,$
          pvnew,pnew,unew,vnew,qdfnew,marknew,sfnew,vpnew,iflag
...
;              if v1 ne -99. and v2 ne -99. then velat_prof(n,kk,iedge)=v1+zscale*(v2-v1)
               if v1 ne -99. then velat_prof(n,kk,iedge)=v1
               endfor
...
plot,velat_prof(*,1,0),psym=4,yrange=[0.,90.],symsize=0.5,title='Vortex Elat 600 K',/noeras
oplot,elat_prof(*,1),psym=3,color=mcolor*.9
set_viewport,.55,.9,.3,.6
plot,pv_prof(*,1),psym=4,yrange=[min(pv_prof(*,1)),max(pv_prof(*,1))],$
     symsize=0.5,title='PV 600 K',/noeras
;
; save daily DMP file
;
time=hh2
save,file=pthout+'dmps_mls_'+sver+'.waccm.'+sdate+'.sav',id,date,time,longitude,latitude,thlev,$
