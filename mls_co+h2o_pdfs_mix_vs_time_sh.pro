...
syr1=syr
yrlab=syr0+'-'+syr1
save,file='mls_co+h2o_3dpdfs_'+yrlab+'_sh.sav',copdf3d,h2opdf3d,th,kday,date_all,xbins
;
...
;
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='mls_co+h2o_pdfs_mix_vs_time_'+slev+'_'+yrlab+'_'+sver+'_sh.ps'
         device,/color
...
;
      ...
      contour,copdf2d_time,xbins,findgen(kday),levels=level,title='CO PDF',/noerase,/fill,c_color=col1,$
              xtitle='CO (ppmv)',color=0,ytickv=xindex,yticks=nxtick,ytickname=' '+strarr(nxtick+1),xrange=[min(xbins),6.]
...
;     contour,h2opdf2d_time,xbins,findgen(kday),levels=level,/overplot,/follow,color=0,c_labels=0*level
      ...
      if setplot eq 'ps' then begin
         device,/close
         spawn,'convert -trim mls_co+h2o_pdfs_mix_vs_time_'+slev+'_'+yrlab+'_'+sver+'_sh.ps -rotate -90 '+$
               'mls_co+h2o_pdfs_mix_vs_time_'+slev+'_'+yrlab+'_'+sver+'_sh.jpg'
      endif
endfor
end
