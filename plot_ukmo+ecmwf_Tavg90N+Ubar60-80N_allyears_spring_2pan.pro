...
restore,'ecmwf_Tavg90N_allyears.sav'	;,TAVG_90N_ALL,th,yyyymmdd,doy
etavg_90n_all=TAVG_90N_ALL
eth=th
eyyyymmdd=yyyymmdd
...
etavg_90N_all_lev=reform(etavg_90N_all(*,*,2))	; take ECMWF level 1600
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='ukmo+ecmwf_tavg_90N_Ubar60-80N_allyears_spring_'+sth+'K.ps'
   device,/color
...
for i=0L,kday-1 do begin
    tavg_90Nday=reform(tavg_90N_ALL_LEV(0L:nyear-4L,i))		; min/max/avg up through 2002
...
; ECMWF
;
etavg_90N_min_lev=fltarr(ekday)
etavg_90N_max_lev=fltarr(ekday)
etavg_90N_1sig_lev=fltarr(ekday)
etavg_90N_avg_lev=fltarr(ekday)
for i=0L,ekday-1 do begin
    etavg_90Nday=reform(etavg_90N_ALL_LEV(0L:enyear-1L,i))
    index=where(etavg_90Nday ne 0.,nn)
    if nn gt 1L then begin
       etavg_90N_min_lev(i)=min(etavg_90Nday(index))
       etavg_90N_max_lev(i)=max(etavg_90Nday(index))
       result=moment(etavg_90Nday(index))
       etavg_90N_avg_lev(i)=result(0)
       etavg_90N_1sig_lev(i)=sqrt(result(1))
    endif
endfor
etavg_90N_min_lev(0)=etavg_90N_min_lev(1)
etavg_90N_max_lev(0)=etavg_90N_max_lev(1)
etavg_90N_avg_lev(0)=etavg_90N_avg_lev(1)
etavg_90N_1sig_lev(0)=etavg_90N_1sig_lev(1)
etavg_90N_min_lev=smooth(etavg_90N_min_lev,3)
etavg_90N_max_lev=smooth(etavg_90N_max_lev,3)
etavg_90N_avg_lev=smooth(etavg_90N_avg_lev,3)
etavg_90N_1sig_lev=smooth(etavg_90N_1sig_lev,3)
...
plot,[1,kday,kday,1,1],[amin,amin,amax,amax,amin],min_value=0.,color=0,$
      xrange=[1,kday],yrange=[amin,amax],/nodata,charsize=1.75,title=sth+' K',$
...
for i=1L,kday do begin
    ...
    if etavg_90N_min_lev(i-1) ne 0. then begin
       plots,i,etavg_90N_min_lev(i-1)
       plots,i,etavg_90N_max_lev(i-1),/continue,thick=20,color=200
    endif
    ...
    if etavg_90N_avg_lev(i-1) ne 0. then begin
       plots,i,etavg_90N_avg_lev(i-1)-etavg_90N_1sig_lev(i-1)
       plots,i,etavg_90N_avg_lev(i-1)+etavg_90N_1sig_lev(i-1),/continue,thick=20,color=150
       plots,i,etavg_90N_avg_lev(i-1)-1.0
       plots,i,etavg_90N_avg_lev(i-1)+1.0,/continue,thick=20,color=20
    endif
endfor
loadct,38
...
for iyear=nyear-3L,nyear-1L do begin
    ...
    xyouts,kday-10.,amin+15.-(iyear-(nyear-3L))*yinc,slab(iyear-(nyear-3L)),charsize=2,color=col1(iyear-(nyear-3L)),$
           /data,charthick=2
endfor
;
; superimpose ECMWF 
...
for iyear=1L,1L	do begin	;enyear-1L do begin
    etavg_90N_ave=reform(etavg_90N_all_lev(iyear,*))
;   index=where(abs(etavg_90N_ave) lt 0.01)
;   if index(0) ne -1 then etavg_90N_ave(index)=-9999.
;   index1=where(etavg_90N_ave ne -9999.,ngood)
;   index2=where(etavg_90N_ave eq -9999.)
;   if ngood gt 1 and index1(0) ne -1 and index2(0) ne -1 then begin
;      filled=interpol(etavg_90N_ave(index1),index1,index2)
;      etavg_90N_ave(index2)=filled
;   endif
;   etavg_90N_ave=smooth(etavg_90N_ave,3)
    kindex=where(etavg_90N_ave ne 0.)
    oplot,kindex,etavg_90N_ave(kindex),color=mcolor*(iyear+1.)/(enyear+2.),thick=3,min_value=-9999.
    xyouts,kday-10.,amin+22.,slab(iyear),charsize=2,$
           color=mcolor*(iyear+1.)/(enyear+2.),/data,charthick=2
endfor
;
; zonal mean wind diagnostic
;
restore,'ecmwf_Ubar60to80N_allyears.sav' ;,UBAR_60TO80N_ALL,th,yyyymmdd,doy
eyyyymmdd=yyyymmdd
result=size(eyyyymmdd)
enyear=result(1)
ekday=result(2)
eubar_60TO80N_ALL=UBAR_60TO80N_ALL
...
eUBAR_60TO80N_all_lev=reform(eUBAR_60TO80N_all(*,*,2))		; ECMWF 1600 K
;
...
; ECMWF
;
esmax_min_lev=fltarr(ekday)
esmax_max_lev=fltarr(ekday)
esmax_1sig_lev=fltarr(ekday)
esmax_avg_lev=fltarr(ekday)
for i=0L,ekday-1 do begin
    esmaxday=reform(eUBAR_60TO80N_ALL_LEV(0L:enyear-1L,i))
    index=where(esmaxday ne 0.,nn)
    if nn gt 1L then begin
       esmax_min_lev(i)=min(esmaxday(index))
       esmax_max_lev(i)=max(esmaxday(index))
       result=moment(esmaxday(index))
       esmax_avg_lev(i)=result(0)
       esmax_1sig_lev(i)=sqrt(result(1))
    endif
endfor
esmax_min_lev(0)=esmax_min_lev(1)
esmax_max_lev(0)=esmax_max_lev(1)
esmax_avg_lev(0)=esmax_avg_lev(1)
esmax_1sig_lev(0)=esmax_1sig_lev(1)
esmax_min_lev=smooth(esmax_min_lev,3)
esmax_max_lev=smooth(esmax_max_lev,3)
esmax_avg_lev=smooth(esmax_avg_lev,3)
esmax_1sig_lev=smooth(esmax_1sig_lev,3)
...
for i=1L,kday do begin
    ...
    if esmax_min_lev(i-1) ne 0. then begin
       plots,i,esmax_min_lev(i-1)
       plots,i,esmax_max_lev(i-1),/continue,thick=20,color=200
    endif
    ...
    if esmax_avg_lev(i-1) ne 0. then begin
       plots,i,esmax_avg_lev(i-1)-esmax_1sig_lev(i-1)
       plots,i,esmax_avg_lev(i-1)+esmax_1sig_lev(i-1),/continue,thick=20,color=150
       plots,i,esmax_avg_lev(i-1)-2.0
       plots,i,esmax_avg_lev(i-1)+2.0,/continue,thick=20,color=20
    endif
endfor
;
; superimpose ECMWF
...
slab=['1978','1979','1980','1981','1982','1983','1984','1985','1986','1987','1988','1989',$
      '1990','1991','1992','1993','1994','1995','1996','1997','1998','1999','2000','2001','2002']
for iyear=1L,1 do begin	;enyear-1L do begin
    esmax_ave=reform(eUBAR_60TO80N_all_lev(iyear,*))
;   index=where(abs(eUBAR_60TO80N_ave) lt 0.01)
;   if index(0) ne -1 then esmax_ave(index)=-9999.
;   index1=where(esmax_ave ne -9999.,ngood)
;   index2=where(esmax_ave eq -9999.)
;   if ngood gt 1 and index1(0) ne -1 and index2(0) ne -1 then begin
;      filled=interpol(esmax_ave(index1),index1,index2)
;      esmax_ave(index2)=filled
;   endif
;   esmax_ave=smooth(esmax_ave,3)
    kindex=where(esmax_ave ne 0.)
    oplot,kindex,esmax_ave(kindex),color=mcolor*(iyear+1.)/(enyear+2.),thick=3,min_value=-9999.
;   xyouts,kday-10.,amin+41.,slab(iyear),charsize=2,$
;          color=mcolor*(iyear+1.)/(enyear+2.),/data,charthick=2
endfor
...
for iyear=nyear-3L,nyear-1L do begin
    ...
    xyouts,kday-10.,amin+30.-(iyear-(nyear-3L))*yinc,slab(iyear-(nyear-3L)),charsize=2,color=col1(iyear-(nyear-3L)),$
           /data,charthick=2
endfor

if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ukmo+ecmwf_tavg_90N_Ubar60-80N_allyears_spring_'+sth+'K.ps -rotate -90 ukmo+ecmwf_tavg_90N_Ubar60-80N_allyears_spring_'+sth+'K.jpg'
endif
end
