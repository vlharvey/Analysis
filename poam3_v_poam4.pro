...
dirs='/aura3/data/SAGE_II_data/Sound_data/poam4_'
...
dirp4='/aura3/data/POAM_data/Sound_data/poam3v4_'
dirp3='/aura3/data/POAM_data/Sound_data/poam3_'
dirp2='/aura3/data/POAM_data/Sound_data/poam2_'
diru='/aura3/data/UKMO_data/Datfiles/ukmo_'
ifile='                             '
lstmn=12 & lstdy=1 & lstyr=2 & lstday=0
ledmn=12 & leddy=1 & ledyr=2 & ledday=0
;
...
;
      norbith=0L & norbits3=0L & norbits2=0L & norbitp3=0L & norbitp4=0L
      ...
      rd_poam3_o3_soundings,dirp3+sfile+'_o3.sound',norbitp3,tpoam3,$
         xpoam3,ypoam3,troppp3,tropzp3,tropthp3,modep3,o3poam3,ppoam3,$
         thpoam3,zpoam3,clpoam3,qo3poam3,nlevp3
      rd_poam3_o3_soundings,dirp4+sfile+'_o3.sound',norbitp4,tpoam4,$
         xpoam4,ypoam4,troppp4,tropzp4,tropthp4,modep4,o3poam4,ppoam4,$
         thpoam4,zpoam4,clpoam4,qo3poam4,nlevp4
...
;
      ...
      ypoam40=-999. & ypoam41=-999.
      if norbitp4 gt 0L then begin
         index=where(qo3poam4/o3poam4 gt 0.5)
         if index(0) ne -1 then o3poam4(index)=-999.
         index=where(modep4 eq 0L)
         if index(0) ne -1 then begin
            ypoam4sr=ypoam4(index)
            xpoam4sr=xpoam4(index)
            o3poam4sr=o3poam4(index,*)
            thpoam4sr=thpoam4(index,*)
            index=where(ypoam4sr ge -90. and ypoam4sr le 90.)
            if index(0) eq -1 then goto,jumppoam4sr
            ypoam40=index(0)
            if n_elements(index) gt 1 then begin
               result=moment(ypoam4sr(index))
               ypoam40=result(0)
            endif
         endif
         jumppoam4sr:
         index=where(modep4 eq 1L)
         if index(0) ne -1 then begin
            ypoam4ss=ypoam4(index)
            xpoam4ss=xpoam4(index)
            o3poam4ss=o3poam4(index,*)
            thpoam4ss=thpoam4(index,*)
            index=where(ypoam4ss ge -90. and ypoam4ss le 90.)
            if index(0) eq -1 then goto,jumppoam4ss
            ypoam41=index(0)
            if n_elements(index) gt 1 then begin
               result=moment(ypoam4ss(index))
               ypoam41=result(0)
            endif
         endif
         jumppoam4ss:
      ...
      yorder=[yhal0,yhal1,ypoam30,ypoam31,ysage30,ysage31,ypoam40,ypoam41]
      instorder=['HALOESR','HALOESS','POAM3SR','POAM3SS',$
                 'SAGE3SR','SAGE3SS','POAM4SR','POAM4SS']
;
; remove missing and SH modes
;
      index=where(yorder ge -90. and yorder le 0.,npan)
...
;
      if setplot eq 'ps' then begin
         ...
         device,/landscape,bits=8,$
                 filename='xz_o3_soundings+v_sh_'+ifile+'.ps'
         device,/color
...
;
      erase
      xmn=0.1 & ymn=0.1
      ywide=(0.9-0.05*float(npan0))/float(npan0)
      xwide=ywide*2.
      for ipan=0,npan0-2 do begin
          xmx=xmn+xwide
          ymx=ymn+ywide
          xmn=(xmx+xmn)/2.
          ymn=ymx+0.05
...
;
          case 1 of
            ...
            (instorder(ipan) eq 'POAM4SR'): begin
             ydata=ypoam4sr
             xdata=xpoam4sr
             o3data=o3poam4sr
             thdata=thpoam4sr
            end
            (instorder(ipan) eq 'POAM4SS'): begin
             ydata=ypoam4ss
             xdata=xpoam4ss
             o3data=o3poam4ss
             thdata=thpoam4ss
