...
;
spawn,'ls zt_vortex_shape_sdwaccm_????-????.sav',ifiles
...
sdum=strcompress(dum,/remove_all)
yearlab=sdum(0)+'-'+sdum(-1)	;syr(0)+'-'+syr(-1)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_vortex_shape_sdwaccm_35yr.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
nfile=182L
erase
;xyouts,.325,.875,'SD-WACCM 2004-2012',/normal,charsize=2,charthick=2,color=0
!type=2^2+2^3
xmn=xorig(0)
xmx=xorig(0)+xlen
ymn=yorig(0)
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle=yearlab+' WACCM Area (%)',/noeras,charthick=2,charsize=1.5
ybox=[0,10,10,0,0]
x1=imin
...
contour,smooth(clat_10yr,3,/edge_truncate),findgen(nfile),altitude,levels=[80],/noeras,color=0,/foll,/overplot,thick=3,c_labels=[0]
imin=min(level)
imax=max(level)
ymnb=yorig(0) -cbaryoff
ymxb=ymnb  +cbarydel
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_vortex_shape_sdwaccm_35yr.ps -rotate -90 zt_vortex_shape_sdwaccm_35yr.jpg'
    endif
