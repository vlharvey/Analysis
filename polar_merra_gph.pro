...
;
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       set_plot,'ps'
       device,/color,/landscape,bits=8,filename='polar_merra_gph_'+sdate+'_'+slev+'.ps'
...
;   contour,sp,x,alat,levels=[60],color=mcolor*.75,/follow,/overplot,/noeras,thick=10,c_labels=[0]
    ...
    plot,[imin,imax],[0,0],yrange=[0,10],$
          xrange=[imin,imax],xtitle=slev+' hPa MERRA GPH',/noeras,$
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim polar_merra_gph_'+sdate+'_'+slev+'.ps -rotate -90 '+$
                            'polar_merra_gph_'+sdate+'_'+slev+'.jpg'
;       spawn,'rm -f polar_merra_gph_'+sdate+'_'+slev+'.ps'
     endif
     jumpday:
goto,jump
end
