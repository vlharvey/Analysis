...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_vortex_shape_mlsco_'+yearlab+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
contour,AREA1_tot,findgen(nfile),pmls,levels=level,/noeras,c_color=col1,/cell_fill,color=0,xticks=nxticks-1,xtickv=xindex,xtickname=xticklab,ytitle='Pressure (hPa)',yrange=[10,0.01],/ylog,charsize=1.5,charthick=2
;contour,AREA1_tot,findgen(nfile),pmls,levels=level,/noeras,color=0,/foll
imin=min(level)
imax=max(level)
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle=yearlab+' MLS CO Area (%)',/noeras,charsize=1.5,charthick=2
ybox=[0,10,10,0,0]
x1=imin
...
nlvls=n_elements(level)
col1=1+indgen(nlvls)*mcolor/nlvls
contour,clat_tot,findgen(nfile),pmls,levels=level,/noeras,c_color=col1,/cell_fill,color=0,xticks=nxticks-1,xtickv=xindex,xtickname=xticklab,ytitle='Pressure (hPa)',yrange=[10,0.01],/ylog,charsize=1.5,charthick=2
;contour,clat_tot,findgen(nfile),pmls,levels=level,/noeras,color=0,/foll
imin=min(level)
imax=max(level)
ymnb=yorig(0) -cbaryoff
ymxb=ymnb  +cbarydel
...
nlvls=n_elements(level)
col1=1+indgen(nlvls)*mcolor/nlvls
contour,ellip_tot,findgen(nfile),pmls,levels=level,/noeras,c_color=col1,/cell_fill,color=0,xticks=nxticks-1,xtickv=xindex,xtickname=xticklab,yrange=[10,0.01],ytitle='Pressure (hPa)',/ylog,charsize=1.5,charthick=2
;contour,ellip_tot,findgen(nfile),pmls,levels=level,/noeras,color=0,/foll
imin=min(level)
imax=max(level)
...
nlvls=n_elements(level)
col1=1+indgen(nlvls)*mcolor/nlvls
contour,nvort_tot,findgen(nfile),pmls,levels=level,/noeras,c_color=col1,/cell_fill,color=0,xticks=nxticks-1,xtickv=xindex,xtickname=xticklab,ytitle='Pressure (hPa)',yrange=[10,0.01],/nodata,/ylog,charsize=1.5,charthick=2
nth=n_elements(pmls)
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_vortex_shape_mlsco_'+yearlab+'.ps -rotate -90 zt_vortex_shape_mlsco_'+yearlab+'.jpg'
    endif
