...
; save postscript version
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='IPY/'+sdate+'_3D_pf.ps'
         device,/color
...
dx=x2d(1,0)-x2d(0,0)
           for ihigh=0,nhigh-1 do begin
               ...
               if min(y2d(index)) le 16.2500 then goto,jump1
...
;   xyouts,.74,.09,'Sondrestrom (67N,51W)',charsize=1.5,/normal,color=mcolor*.1,charthick=2
    xyouts,.75,.1,'Poker Flat',charsize=1.5,/normal,color=mcolor*.95,charthick=2
    xyouts,.75,.075,'(65N,147W)',charsize=1.5,/normal,color=mcolor*.95,charthick=2
;   xyouts,.83,.05,'IAP (54N,12E)',charsize=1.5,/normal,color=mcolor*.35,charthick=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim IPY/'+sdate+'_3D_pf.ps -rotate -90 IPY/'+sdate+'_3D_pf.png'
       spawn,'/usr/bin/rm -f IPY/'+sdate+'_3D_pf.ps'
    endif
goto, jump
end
