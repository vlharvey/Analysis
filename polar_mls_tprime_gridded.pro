;
; plot daily polar maps of MLS gridded temperature anomaly (from zonal mean)
;
@stddat
@kgmt
@ckday
...
;
        ...
        if setplot eq 'ps' then begin
           ...
           device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
                  /bold,/color,bits_per_pixel=8,/helvetica,filename='polar_mls_tprime_gridded_'+sdate+'_'+salt+'.ps'
           !p.charsize=1.25
           !p.thick=2
           !p.charthick=5
           !y.thick=2
...
;
       imin=-10
       level=imin+findgen(21)
       imax=max(level)
       nlvls=n_elements(level)
       col1=reverse(1+(indgen(nlvls)/float(nlvls))*mcolor)
;
; wrap longitude
;
       nc=n_elements(lon)
       ...
       tp2(nc,*)=tp2(0,*)
tzm=mean(tp2,dim=1)
for j=0L,nr-1L do tp2(*,j)=tp2(*,j)-tzm(j)
       uu2=fltarr(nc+1,nr)
       uu2(0:nc-1,*)=uu
       uu2(nc,*)=uu2(0,*)
       lon2=fltarr(nc+1)
...
index=where(level lt 0.)
       contour,tp2,lon2,lat,levels=level(index),/overplot,/noeras,c_color=mcolor,/follow,c_labels=0*level,c_linestyle=5
index=where(level gt 0.)
       contour,tp2,lon2,lat,levels=level(index),/overplot,/noeras,c_color=0,/follow,c_labels=0*level
...
;      contour,uu2,lon2,lat,levels=[-100.,-90,-80,-70,-60,-50,-40,-30,-20,-10],/overplot,/noeras,c_color=mcolor,/follow,c_linestyle=5
       ...
       !type=2^2+2^3+2^6
       plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],/noeras,color=0,charsize=1,xtitle='MLS '+salt+' Temperature Anomaly (K)'
       ybox=[0,10,10,0,0]
       x2=imin
...
; Close PostScript file and return control to X-windows
       ...
       if setplot eq 'ps' then begin
          device, /close
          spawn,'convert -trim polar_mls_tprime_gridded_'+sdate+'_'+salt+'.ps -rotate -90 '+$
                              'polar_mls_tprime_gridded_'+sdate+'_'+salt+'.jpg'
          spawn,'rm -f polar_mls_tprime_gridded_'+sdate+'_'+salt+'.ps'
       endif
