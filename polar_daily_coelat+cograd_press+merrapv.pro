...
;   if imon ne 0 and imon lt 11 then goto,skipmon		; Dec and Jan only
    if imon gt 1 and imon lt 10 then goto,skipmon		; Nov-Feb
...
print,'CO Elat Edge ',lowlat_elatedge_time(n)
print,'Nash Elat Edge ',nashedge
; 
; save postscript version
;
       erase
       xyouts,.4,.925,sdate,color=0,charsize=2,charthick=2,/normal
       xmn=xorig(0)
       xmx=xorig(0)+xlen
       ymn=yorig(0)
       ymx=yorig(0)+ylen
       ...
       map_set,90,0,-90,/ortho,/contin,/grid,/noerase,color=0,charsize=1.25,/noborder	;,limit=[40.,0.,90.,360.]
dum=fltarr(nc+1,nr)
dum(0:nc-1,*)=co1
dum(nc,*)=dum(0,*)
x=fltarr(nc+1)
...
x(nc)=x(0)
       ...
       map_set,90,0,-90,/ortho,/contin,/noerase,color=mcolor,/noborder
dum(0:nc-1,*)=elat
dum(nc,*)=dum(0,*)
;      contour,dum,x,alat,levels=10+10*findgen(8),/noeras,charsize=2,color=mcolor,/foll,/overplot,c_labels=[1,1,1,1,1,1,1,1]			; all contours
;      contour,dum,x,alat,levels=50,/noeras,charsize=2,color=mcolor,/foll,/overplot,c_labels=[1],thick=5
       contour,dum,x,alat,levels=lowlat_elatedge_time(n),/noeras,charsize=2,color=mcolor,/foll,/overplot,c_labels=[0],thick=10
       ymnb=ymn -cbaryoff
       ymxb=ymnb+cbarydel
       set_viewport,xmn+0.01,xmx-0.01,ymnb,ymxb
xyouts,xmx-0.1,ymxb+0.01,'Elat='+strcompress(long(lowlat_elatedge_time(n)),/remove_all),charsize=1.25,color=0,/normal,charthick=2
       ...
       xmn=xorig(1)+0.05
       xmx=xorig(1)+xlen-0.1
       ymn=yorig(1)
       ymx=yorig(1)+ylen
       set_viewport,xmn,xmx,ymn,ymx
       !type=2^2+2^3
       plot,yeq,cobin,color=0,/noeras,charsize=1,ytitle='Mean CO',xtitle='Equivalent Latitude',xrange=[20,90],thick=5,charthick=2
       axis,/yax,yrange=[min(delatlevs),max(delatlevs)],/save,ytitle='dCO/dElat',charsize=1,color=mcolor*.9,charthick=2
       index=where(finite(delatlevs) eq 1)
       oplot,yeq(index),delatlevs(index),color=mcolor*.9,thick=4
       oplot,[lowlat_elatedge_time(n),lowlat_elatedge_time(n)],[min(delatlevs),max(delatlevs)],psym=0,color=0
;
; PV
;
       xmn=xorig(2)
       ...
       imax=max(pvbin)
       level=imin+((imax-imin)/float(nlvls-1))*findgen(nlvls)
       map_set,90,0,-90,/ortho,/contin,/grid,/noerase,color=0,charsize=1.5,/noborder  ;,limit=[40.,0.,90.,360.]
dum(0:nc-1,*)=pv1
dum(nc,*)=dum(0,*)
       contour,dum,x,alat,levels=level,/noeras,charsize=2,c_color=col1,/cell_fill,/overplot
       map_set,90,0,-90,/ortho,/contin,/noerase,color=mcolor,/noborder
...
;      contour,dum,x,alat,levels=10+10*findgen(8),/noeras,charsize=2,color=mcolor,/foll,/overplot,c_labels=[1,1,1,1,1,1,1,1]                       ; all contours
loadct,0
       contour,dum,x,alat,levels=nashedge,/noeras,charsize=2,color=mcolor,/foll,/overplot,c_labels=[0],thick=12
loadct,39
;      contour,dum,x,alat,levels=pvelatedge,/noeras,charsize=2,color=mcolor,/foll,/overplot,c_labels=[0],thick=8
;      contour,elat,alon,alat,levels=elatedge,/noeras,charsize=2,color=mcolor,/foll,/overplot,c_labels=[1],thick=5,c_linestyle=5
dum(0:nc-1,*)=sp1
dum(nc,*)=dum(0,*)
       contour,dum,x,alat,levels=40+20*findgen(4),/noeras,charsize=2,c_color=[150,250,0],/foll,/overplot,thick=5
       ymnb=ymn -cbaryoff
       ymxb=ymnb+cbarydel
       set_viewport,xmn+0.01,xmx-0.01,ymnb,ymxb
xyouts,xmx-0.1,ymxb+0.01,'Elat='+strcompress(long(nashedge),/remove_all),charsize=1.25,color=0,/normal,charthick=2

       !type=2^2+2^3+2^6
       plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],/noeras,color=0,charsize=1,xtitle='MERRA '+sth+' PV',charthick=2
       ybox=[0,10,10,0,0]
       x2=imin
       dx=(imax-imin)/(float(nlvls)-1)
       for j=1,nlvls-1 do begin
...
;
       xmn=xorig(3)+0.05
       xmx=xorig(3)+xlen-0.1
       ymn=yorig(3)
       ymx=yorig(3)+ylen
       set_viewport,xmn,xmx,ymn,ymx
       !type=2^2+2^3
       index=where(finite(pvbin) eq 1)
       plot,yeq(index),pvbin(index),color=0,/noeras,charsize=1,ytitle='Mean PV',xtitle='Equivalent Latitude',xrange=[20,90],thick=5,charthick=2
;      axis,/yax,yrange=[min(prod),max(prod)],/save,ytitle='dPV/dElat * Speed/Elat',charsize=1.25,color=mcolor*.9,charthick=2
       axis,/yax,yrange=[min(dpvelatlevs),max(dpvelatlevs)],/save,charsize=1.25,color=mcolor*.9,charthick=2,ytitle='dPV/dElat'
       index=where(finite(dpvelatlevs) eq 1)
       oplot,yeq(index),dpvelatlevs(index),color=mcolor*.9,thick=4
       axis,/yax,yrange=[min(spbin),max(spbin)],/save,charsize=1.25,color=mcolor*.3,charthick=2,yticks=1,ytickname=[' ',' ']
       index=where(finite(spbin) eq 1)
       oplot,yeq(index),spbin(index),color=mcolor*.3,thick=4
;      oplot,yeq,prod,color=mcolor*.9,thick=4
       oplot,[nashedge,nashedge],[min(spbin),max(spbin)],psym=0,color=0
;
; speed axis
;
;      !type=2^2+2^3+2^6
;      set_viewport,xmx+0.17,xmx+0.171,ymn,ymx
;      plot,findgen(10),findgen(10),/nodata,yrange=[min(spbin),max(spbin)],charsize=1.25,color=mcolor*.3,charthick=2,ytitle='Speed/dElat',/noeras,xticks=1,xtickname=[' ',' ']

; Close PostScript file and return control to X-windows
       if setplot ne 'ps' then stop	;wait,1
       if setplot eq 'ps' then begin
          device, /close
          spawn,'convert polar_daily_coelat+cograd_press_merrapv_'+sdate+'_'+spress+'.ps -rotate -90 '+$
...
; save monthly file
;
save,filename='daily_mls_coelatedge+merra_pvelatedge_'+syear(iyear)+smon(imon)+'_'+spress+'.sav',elatedge_time,pvelatedge_time,nashedge_time,sdate_time,spress,sth,lowlat_elatedge_time,lowlat_coedge_time
