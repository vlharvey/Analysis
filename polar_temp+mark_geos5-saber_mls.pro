;
; GEOS-5 SABER and MLS polar projections
...
nxdim=750
nydim=750
xorig=[0.05,0.3625,0.675,0.05,0.3625,0.675]
yorig=[0.6,0.6,0.6,0.25,0.25,0.25]
yorig=[0.6,0.6,0.6,0.25,0.25,0.25]-0.05
xlen=0.3
ylen=0.3
cbaryoff=0.02
cbarydel=0.01
...
;
    ...
    print,mdir+'cat_mls_v2.2_'+sdate+'.sav'
    nlvh=n_elements(altitude)
    if icount eq 0 then begin
       theta0=40.
;      print,altitude
...
;        !p.font=0
         ...
         device,/landscape,bits=8,$
                 filename='polar_temp+mark_geos5-saber_mls_'+sdate+'_'+stheta+'km.ps'
         device,/color
...
!type=2^2+2^3
!psym=0
xyouts,0.45,0.9,sdate,charsize=1.5,charthick=2,/normal
...
loadct,39
   ...
   if setplot ne 'ps' then stop
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim polar_temp+mark_geos5-saber_mls_'+sdate+'_'+stheta+'km.ps -rotate -90 polar_temp+mark_geos5-saber_mls_'+sdate+'_'+stheta+'km.jpg'
    endif
goto,jump
end
