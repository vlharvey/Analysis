...
dirhi='/aura3/data/HIRDLS_data/Merged_data/'
...
; read HIRDLS data
;
    ifile='hirdls_'+strmid(filename,0,7)+syear+'.merged'
    hdum=findfile(dirhi+ifile)
    hicount=0L
    if hdum(0) eq '' then goto,jumphirdls
    close,4
    openr,4,dirhi+ifile
    rd_hirdls_merged_data,yymmdd,ncount,thir,xhir,yhir,thhir,xsathir,ysathir,$
       phir,zhir,ptrhir,ztrhir,thtrhir,clhir,mhir,h2ohir,$
       o3hir,no2hir,hno3hir,eh2ohir,eo3hir,eno2hir,ehno3hir
    close,4
    if ncount eq 0L then goto,jumphirdls
    index=where(thhir eq 400. and o3hir gt 0.,hicount)
    xhir=xhir(index)
    yhir=yhir(index)
    o3hir=o3hir(index)*1.e6
    index=where(xhir lt 0. and xhir ge -180.)
    if index(0) ne -1 then xhir(index)=xhir(index)+360.
    print,'merged file is '+ifile,hicount
    jumphirdls:
;
; read HALOE data
;
    ...
    index=where(thhal eq 400. and o3hal gt 0.,hcount)
    xhal=xhal(index)
    yhal=yhal(index)
...
;
    ...
    index=where(thsage eq 400. and o3sage gt 0.,scount)
    xsage=xsage(index)
    ysage=ysage(index)
...
;
    ...
    index=where(thsage3 eq 400. and o3sage3 gt 0.,scount3)
    xsage3=xsage3(index)
    ysage3=ysage3(index)
...
;
    ...
    index=where(thpoam eq 400. and o3poam gt 0.,pcount)
...
;
    ...
    index=where(thilas eq 400. and o3ilas gt 0.,icount)
    xilas=xilas(index)
    yilas=yilas(index)
...
;
    if setplot eq 'ps' then begin
       ...
       device,/color,/landscape,bits=8,$
              filename='merc_ukmo+hirdls_'+syymmdd+'_'+stheta+'.ps'
    ...
    MAP_SET,0,0,0,/contin,/grid,/noeras,color=lc,charsize=2,$
    ...
    o3min=2.0
    o3max=11.
    if hicount gt 0L then begin
       a=findgen(9)*(2*!pi/8.)
       usersym,2.*cos(a),2.*sin(a),/fill
       for i=0,hicount-1 do $
           oplot,[xhir(i),xhir(i)],[yhir(i),yhir(i)],$
                 psym=8,color=mcolor*(o3hir(i)-o3min)/(o3max-o3min)
       a=findgen(9)*(2*!pi/8.)
    ...
    contour,sf,x,alat,nlevels=30,c_color=lc,/overplot,/follow,$
            c_labels=0,/noeras
    contour,mark,x,alat,levels=[0.,1.],c_color=0,/overplot,/follow,$
            c_labels=0,/noeras,thick=3
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert merc_ukmo+hirdls_'+syymmdd+'_'+stheta+'.ps -rotate -90 '+$
                     'hirdls_ukmo+occul_'+syymmdd+'_'+stheta+'.jpg'
    endif
endfor
endfor
end
