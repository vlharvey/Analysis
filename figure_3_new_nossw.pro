...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='figure_3_new_nossw.ps'
endif
erase
!type=2^2+2^3
xmn=xorig(0)
...
;
    ...
    if itime eq 0L then begin
       plot,sfdiffmean,th,xrange=[0.,40.],yrange=[300.,max(th)],color=0,$
            xtitle='!4D !XLatitude',ytitle='Theta (K)',/nodata
    endif
...
xyouts,30.,500.,'SH',color=0,/data,charsize=2
;
; NH
;
...
ymx=yorig(1)+ylen
set_viewport,xmn,xmx,ymn,ymx
restore,'geos5_nh_sf_pv_difflat_save_daily_nossw.sav
...
xyouts,30.,500.,'NH',color=0,/data,charsize=2
xyouts,xmx+0.075,ymn+0.13,'Approximate Altitude (km)',color=0,/normal,orientation=90.
xyouts,xmx+0.01,ymn+0.02,'20',color=0,/normal
xyouts,xmx+0.01,ymn+0.06,'30',color=0,/normal
xyouts,xmx+0.01,ymn+0.13,'40',color=0,/normal
xyouts,xmx+0.01,ymn+0.23,'50',color=0,/normal
xyouts,xmx+0.01,ymn+0.37,'60',color=0,/normal
xyouts,xmx+0.01,ymx-0.09,'70',color=0,/normal

if setplot ne 'ps' then stop
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim figure_3_new_nossw.ps -rotate -90 figure_3_new_nossw.jpg'
;  spawn,'/usr/bin/rm figure_3_new_nossw.ps'
endif
end
