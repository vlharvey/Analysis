...
lstmn=12L & lstdy=1L & lstyr=2006L
...
;
      if setplot eq 'ps' then begin
         ...
         device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='figure_1_'+sdate+'.ps'
      endif
;
...
;     contour,mark,x,alat,/overplot,levels=[0.1],color=0,thick=10,/noeras,/follow,c_labels=0
      ...
      xyouts,xmn+0.005,ymx-0.02,slev,/normal,color=0
      imin=min(level)
      imax=max(level)
...
;          xticks=nxticks-1,xtickv=xindex,xtickname=strcompress(long(th(xindex)),/remove_all)
      ybox=[0,10,10,0,0]
      x1=imin
      dx=(imax-imin)/float(nlvls)
      for j=0,nlvls-1 do begin
      ...
      if setplot eq 'ps' then begin
         device,/close
;        spawn,'convert -trim figure_1.ps -rotate -90 figure_1.jpg'
         spawn,'convert -trim figure_1_'+sdate+'.ps -rotate -90 figure_1_'+sdate+'.jpg'
;        spawn,'/usr/bin/rm figure_1.ps'
      endif
