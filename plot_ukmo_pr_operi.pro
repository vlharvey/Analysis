...
;***Read UKMO data
      file=dir+'ppassm_y'+syr+'_m'+smn+'_d'+sdy+'_h12.pp.dat'
      print,file
      rd_ukmo_operi,file,iflg,nlg,nlat,nlv,alon,alat,wlon,wlat,p,$
              zp,tp,up,vp
      ...
      if icount eq 0L then begin 
         ...
         index=where(abs(rsfc-p) eq min(abs(rsfc-p)))
         lsfc=index(0)
...
;     dat(0:nlg-1,0:nlat-1)=zp(0:nlg-1,0:nlat-1,lsfc)/1000.
      dat(0:nlg-1,0:nlat-1)=tp(0:nlg-1,0:nlat-1,lsfc)
      dat(nlg,*)=dat(0,*)
...
; Autoscale if scale values for parameter/level are not defined
      ...
      date=strcompress(string(FORMAT='(A3,A1,I2,A2,I4)',$
                              month(imn-1),' ',idy,', ',iyr))
      mtitle=runtitle+title2(1)+'at'+mb+'on '+date
