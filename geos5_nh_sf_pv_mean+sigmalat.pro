...
;
      ...
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='geos5_nh_sf_pv_mean+sigmalat_'+sdate+'.ps'
         device,/color
...
xmin=-1.e-5
xmax=5.e-6
plot,sfbin(index),meanlat_sf(index,ilev),color=0,thick=5,/noeras,xtitle='SF Bins',$
...
for ilev=nth-1L,0L,-1L do begin
    ...
    sfbin=sfmin+((sfmax-sfmin)/float(nbins))*findgen(nbins)
    index=where(meanlat_sf(*,ilev) ne 0.)
    oplot,sfbin(index),meanlat_sf(index,ilev),color=col2(ilev),psym=0,thick=5
    sfdiff(ilev)=abs(meanlat_sf(0,ilev)-max(meanlat_sf(index,ilev)))
endfor    ; loop over altitude
for ilev=nth-1L,0L,-1L do begin
    sf=transpose(sf2(*,*,ilev))
    index=where(lat gt 40.)
...
xmin=1.e-6
xmax=5.
plot,pvbin(index),meanlat_pv(index,ilev),color=0,thick=5,/noeras,xtitle='PV Bins',$
...
plot,sfdiff,th,color=0,thick=7,/noeras,xtitle='Mean Lat at Min SF - Max Lat',$
     ytitle='Theta (K)',xrange=[0.,30.],yrange=[450.,4600.]
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim geos5_nh_sf_pv_mean+sigmalat_'+sdate+'.ps -rotate -90 '+$
         'geos5_nh_sf_pv_mean+sigmalat_'+sdate+'.jpg'
   spawn,'/usr/bin/rm geos5_nh_sf_pv_mean+sigmalat_'+sdate+'.ps'
endif
