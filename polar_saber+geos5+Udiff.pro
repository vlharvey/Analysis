...
sdir='/aura6/data/SABER_data/Datfiles_winds/GRID_PHI_WINDS.'
nz=121
altitude=findgen(nz)
...
;if index(0) ne -1L then ubarz_yt_saber(index)=-999.
    ...
    xyouts,.3,.7,sdate+' Zonal Wind at '+salt+' km',color=0,charsize=2,/normal
    !type=2^2+2^3
    xmn=xorig(0)
    xmx=xorig(0)+xlen
    ymn=yorig(0)
    ...
    contour,ugeos5s,alon2,alat,/noeras,/overplot,levels=level(index),color=0,min_value=-999
    index=where(level lt 0.)
    contour,ugeos5s,alon2,alat,/noeras,/overplot,levels=level(index),color=mcolor,c_linestyle=5,min_value=-999
    contour,ugeos5s,alon2,alat,/noeras,/overplot,levels=[0],color=0,thick=3,min_value=-999
...
usaber=smooth(usaber,3)
    contour,usaber,alon2,alat,/noeras,levels=level,c_color=col1,/cell_fill,min_value=-999,/overplot
    index=where(level gt 0.)
    contour,usaber,alon2,alat,charsize=1.5,/noeras,/overplot,levels=level(index),color=0,min_value=-999
    index=where(level lt 0.)
    contour,usaber,alon2,alat,charsize=1.5,/noeras,/overplot,levels=level(index),color=mcolor,c_linestyle=5,min_value=-999
    contour,usaber,alon2,alat,charsize=1.5,/noeras,/overplot,levels=[0],color=0,thick=3,min_value=-999
    map_set,90,0,-90,/ortho,/noerase,/contin,/grid,color=0
    imin=min(level)
    imax=max(level)
    ymnb=yorig(1)-cbaryoff
