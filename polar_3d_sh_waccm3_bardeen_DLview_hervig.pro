...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='Figures/'+sday+'_sh_'+xlab+'_hervig.ps'
       device,/color
...
;
        loadct,0
        map_continents,mlinethick=2,/t3d,zvalue=nz(kk),color=mcolor*.4,/fill_continents
;
...
;
; superimpose profile at Chuck's location
;
        yy=-65.3 & xx=259.9
        ANG = (90. + yy) * RADG * 0.5
        FACTOR = TAN(ANG) * FAC20
        THETA = (xx - 90.) * RADG
        xn = FACTOR * COS(THETA)
        yn = -1.0*FACTOR * SIN(THETA)
        a=findgen(8)*(2*!pi/8.)
        usersym,2*cos(a),2*sin(a),/fill
        oplot,[xn,xn],[yn,yn],zvalue=nz(kk),/T3D,psym=8,color=0
        dist=nz(kp1)-nz(kk)
        if dist gt 0. then begin
        for m=0,10 do $
            oplot,[xn,xn],[yn,yn],zvalue=nz(kk)+float(m)*dist/11.,/T3D,psym=8,color=0
        endif
...
jumplev:
    ...
    !psym=0
    xyouts,0.45,0.88,xlab,/normal,charsize=3.0,color=0,charthick=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim Figures/'+sday+'_sh_'+xlab+'_hervig.ps -rotate -90 Figures/'+sday+'_sh_'+xlab+'_hervig.jpg'
;      spawn,'/usr/bin/rm Figures/'+sday+'_sh_'+xlab+'_hervig.ps'
    endif
