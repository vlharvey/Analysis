...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
           /bold,/color,bits_per_pixel=8,/helvetica,filename='figure_2_ubar_mls.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
contour,jja_uyz,alat,altitude,/noera,/cell_fill,color=0,c_color=col1,levels=tlevel,xrange=[-90,90],yrange=[30,90],ytitle='Altitude (km)',charsize=1.5,charthick=2,title='JJA',xticks=6
index=where(tlevel gt 0.)
contour,jja_uyz,alat,altitude,/noera,/foll,color=0,levels=tlevel(index),/overplot,c_labels=0*tlevel(index)
index=where(tlevel lt 0.)
contour,jja_uyz,alat,altitude,/noera,/foll,color=mcolor,c_linestyle=5,levels=tlevel(index),/overplot,c_labels=0*tlevel(index)
...
ymx=yorig(1)+ylen
set_viewport,xmn,xmx,ymn,ymx
contour,djf_uyz,alat,altitude,/noera,/cell_fill,color=0,c_color=col1,levels=tlevel,xrange=[-90,90],yrange=[30,90],charsize=1.5,charthick=2,title='DJF',xticks=6
index=where(tlevel gt 0.)
...
ZBAR_JJA=ZBAR_JJA/1000.
ZBAR_DJF=ZBAR_DJF/1000.
;
; interpolate to altitude for difference plots
;
mls_djf_u=fltarr(n_elements(waccm_latitude),nz)
mls_jja_u=fltarr(n_elements(waccm_latitude),nz)
for j=0L,nr-1L do begin
    uprof=reform(ubar_djf(j,*))
    zprof=reform(zbar_djf(j,*))
    mls_djf_u(j,*)=interpol(uprof,zprof,altitude,/Nan)
index=where(altitude lt min(zprof) or altitude gt max(zprof))
if index(0) ne -1L then mls_djf_u(j,index)=0./0.		; do not allow extrapolation

    uprof=reform(ubar_jja(j,*))
    zprof=reform(zbar_jja(j,*))
    mls_jja_u(j,*)=interpol(uprof,zprof,altitude,/Nan)
index=where(altitude lt min(zprof) or altitude gt max(zprof))
if index(0) ne -1L then mls_jja_u(j,index)=0./0.
endfor
index=where(abs(alat) le 10.)
mls_jja_u(index,*)=0./0.
mls_djf_u(index,*)=0./0.
...
ymx=yorig(2)+ylen
set_viewport,xmn,xmx,ymn,ymx
contour,mls_jja_u,alat,altitude,levels=tlevel,/cell_fill,c_color=col1,/noeras,xticks=6,ytitle='Altitude (km)',color=0,yrange=[30,90],charsize=1.5,charthick=2
index=where(tlevel gt 0.)
contour,mls_jja_u,alat,altitude,/noera,/foll,color=0,levels=tlevel(index),/overplot,c_labels=0*tlevel
index=where(tlevel lt 0.)
contour,mls_jja_u,alat,altitude,/noera,/foll,color=mcolor,c_linestyle=5,levels=tlevel(index),/overplot,c_labels=0*tlevel
...
ymx=yorig(3)+ylen
set_viewport,xmn,xmx,ymn,ymx
contour,mls_djf_u,alat,altitude,levels=tlevel,/cell_fill,c_color=col1,/noeras,xticks=6,color=0,yrange=[30,90],charsize=1.5,charthick=2
index=where(tlevel gt 0.)
contour,mls_djf_u,alat,altitude,/noera,/foll,color=0,levels=tlevel(index),/overplot,c_labels=0*tlevel
index=where(tlevel lt 0.)
contour,mls_djf_u,alat,altitude,/noera,/foll,color=mcolor,c_linestyle=5,levels=tlevel(index),/overplot,c_labels=0*tlevel
...
xyouts,.97,.5,'Zonal Wind (m/s)',/normal,orientation=90,color=0,charsize=1.5,charthick=2
xbox=[0,10,10,0,0]
y2=0
...
jja_diff=jja_uyz-mls_jja_u
index=where(mls_jja_u eq 0.)
if index(0) ne -1L then jja_diff(index)=0./0.
tlevel=-50+10*findgen(nlvls)
contour,jja_diff,waccm_latitude,altitude,levels=tlevel,/cell_fill,c_color=col2,/noeras,xticks=6,color=0,yrange=[30,90],charsize=1.5,charthick=2,ytitle='Altitude (km)',xtitle='Latitude'
index=where(tlevel gt 0.)
...
djf_diff=djf_uyz-mls_djf_u
index=where(mls_djf_u eq 0.)
if index(0) ne -1L then djf_diff(index)=0./0.
tlevel=-50+10*findgen(nlvls)
contour,djf_diff,waccm_latitude,altitude,levels=tlevel,/cell_fill,c_color=col2,/noeras,xticks=6,color=0,yrange=[30,90],charsize=1.5,charthick=2,xtitle='Latitude'
index=where(tlevel gt 0.)
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim figure_2_ubar_mls.ps -rotate -90 figure_2_ubar_mls.jpg'
;  spawn,'rm -f figure_2_ubar_mls.ps'
endif
