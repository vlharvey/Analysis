...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='ukmo_nhvortex_area_allyears_1000-2000K_spring.ps'
   device,/color
...
plot,[1,kday0,kday0,1,1],[0.,0.,amax,amax,0.],min_value=0.,color=0,$
      ...
      ytitle='Millions of km!u2!n',xtickname=[smon,' '],xticks=n_elements(smon),$
      title='Average Arctic Vortex Area 1000-2000 K'
;
; range of areas prior to 2004
;
area_min_lev=fltarr(kday)
area_max_lev=fltarr(kday)
for i=0L,kday0-1 do begin
...
area_min_lev=smooth(area_min_lev,5)
area_max_lev=smooth(area_max_lev,5)
loadct,0
for i=1L,kday0-1L do begin
...
;print,reform(yyyymmdd(iyear,*))
    ...
    area_ave=smooth(area_ave,5,/edge_truncate)
    oplot,findgen(kday),area_ave,color=col1(iyear-(nyear-3L)),thick=8,min_value=-9999.
    xyouts,kday0-10.,amax-7.-(iyear-(nyear-3L))*yinc,slab(iyear-(nyear-3L)),charsize=2,color=col1(iyear-(nyear-3L)),/data,charthick=2
endfor

if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ukmo_nhvortex_area_allyears_1000-2000K_spring.ps -rotate -90 '+$
         'ukmo_nhvortex_area_allyears_1000-2000K_spring.jpg'
endif
end
