...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='ilas_loc.ps'
   !p.thick=2
   !p.charthick=5
   !y.thick=2
   !x.thick=2
...
xinc=0.8/float(nyear)
for iyear=0L,nyear-1L do begin
    if iyear le 5L then xyouts,xmn+iyear*xinc,ymn-0.1,syear(iyear),color=col1(iyear),/normal,charsize=1.5
    if iyear gt 5L then xyouts,xmn+(iyear-6L)*xinc,ymn-0.125,syear(iyear),color=col1(iyear),/normal,charsize=1.5
;
; SAGE II
;
...
; ILAS II
;
    ex=findfile(diri+'cat_ilas2_v2.0.'+syear(iyear))
    if ex(0) ne '' then begin
       restore,diri+'cat_ilas2_v2.0.'+syear(iyear)
       oplot,fdoy,latitude,psym=8,color=col1(iyear)
    endif
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ilas_loc.ps -rotate -90 ilas_loc.jpg'
endif
end
