;
; combine daisy petals into one array and plot from low to high albedo
; to reduce effect of overlapping clouds
...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='polar_daily_cipsl2a_alb_'+sdate+'_combine.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
...
;     uttime_all=-99.+0.*fltarr(norbit,npts)
;
; loop over orbits
;
...
;
             ...
             lat_all(iorbit,0:ngood-1L)=latitude
             lon_all(iorbit,0:ngood-1L)=longitude
;            uttime_all(iorbit,0:ngood-1L)=ut_time
          ENDIF
...
alb=alb_all(good)
latitude=lat_all(good)
longitude=lon_all(good)
alb_all=0
lat_all=0
lon_all=0
...
index=where(alb gt 3. and alb le 5.)
if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.2
index=where(alb gt 5. and alb le 7.)
if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.3
index=where(alb gt 7. and alb le 10.)
if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.4
index=where(alb gt 10. and alb le 15.)
if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.57
index=where(alb gt 15. and alb le 25.)
if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.65
index=where(alb gt 25. and alb le 40.)
if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.75
index=where(alb gt 40. and alb le 50.)
if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.8
index=where(alb gt 50.)
if index(0) ne -1L then oplot,longitude(index),latitude(index),psym=3,color=mcolor*.9

slevel=['1','3','7','10','15','25','40','50','>50']
nlvls=n_elements(slevel)
col1=indgen(nlvls)
col1=[.1,.2,.3,.4,.57,.65,.75,.8,.9]*mcolor
imin=1.
imax=60.
    ymnb=ymn -cbaryoff
    ymxb=ymnb+cbarydel
...
print,'converting to jpg'
        spawn,'convert -trim polar_daily_cipsl2a_alb_'+sdate+'_combine.ps -rotate -90 '+$
                            'polar_daily_cipsl2a_alb_'+sdate+'_combine.jpg'
print,'converted'
