...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='ukmo_nhtmin_allyears_'+sth+'K.ps'
   device,/color
...
for i=0L,kday-1 do begin
    tminday=reform(TMIN_ALL_LEV(0L:nyear-3L,i))
    index=where(tminday ne 0.)
    if index(0) ne -1L then tmin_min_lev(i)=min(tminday(index))
    if index(0) ne -1L then tmin_max_lev(i)=max(tminday(index))
endfor
loadct,0
for i=1L,kday do begin
    if tmin_min_lev(i-1) ne 0. then begin
       plots,i,tmin_min_lev(i-1)
       plots,i,tmin_max_lev(i-1),/continue,thick=3,color=200
    endif
endfor
loadct,38
...
;   tmin_ave=smooth(tmin_ave,7)
    if iyear lt nyear-1L then oplot,findgen(kday),tmin_ave,color=col1(iyear-(nyear-3L)),thick=4,min_value=-9999.
    if iyear eq nyear-1L then oplot,findgen(118),tmin_ave(0:117),color=col1(iyear-(nyear-3L)),thick=4,min_value=-9999.
    xyouts,kday-40.,amax-7.-(iyear-(nyear-3L))*yinc,slab(iyear-(nyear-3L)),charsize=2,color=col1(iyear-(nyear-3L)),/data,charthick=2
endfor

if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ukmo_nhtmin_allyears_'+sth+'K.ps -rotate -90 ukmo_nhtmin_allyears_'+sth+'K.jpg'
endif
end
