;-----------------------------------------------------------------------------------------------------------------------------
;MLS daily plots for each ES event; saves lat/lon of max Z, T for each day.
;	 -------------------------------
;       |         Jeff France           |
;       |         LASP, ATOC            |
...
restore, '/Volumes/MacD68-1/france/France_et_al_2012/Post_process/Figures_2-5_post_process_2004_2012.sav'
restore, '/Volumes/MacD68-1/france/WACCM_paper/Post_process/Figure_4_pre_process_composit_ES_MLS.sav'
...
;
    ...
    marker = fltarr(n_elements(MLSdailymark[*,0L]) + 2L, n_elements(MLSdailymark[0L,*]))
    marker[0L:n_elements(MLSdailymark[*,0L]) - 1L, *] = MLSdailymark[*,*]
   marker[n_elements(MLSdailymark[*,0L])-1L:n_elements(MLSdailymark[*,0L])+1L,*] = MLSdailymark[0L:2L,*]
   lons = fltarr(n_elements(lon) + 2L)
   lons[0:n_elements(lon)-1L] = lon
   lons[n_elements(lon)-1L:n_elements(lon)+1L] = lon[0L:2L]


   MLSt = fltarr(n_elements(MLSdailytemp[*,0L]) + 2L, n_elements(MLSdailytemp[0L,*]))
    MLSt[0L:n_elements(MLSdailytemp[*,0L]) - 1L, *] = MLSdailytemp
   MLSt[n_elements(MLSdailytemp[*,0L])-1L:n_elements(MLSdailytemp[*,0L])+1L,*] = MLSdailytemp[0L:2L,*]
     MLSZ = fltarr(n_elements(MLSdailyheight[*,0L]) + 2L, n_elements(MLSdailyheight[0L,*]))
    MLSZ[0L:n_elements(MLSdailyheight[*,0L]) - 1L, *] = MLSdailyheight
   MLSZ[n_elements(MLSdailyheight[*,0L])-1L:n_elements(MLSdailyheight[*,0L])+1L,*] = MLSdailyheight[0L:2L,*]
...
;   MLSz = smooth(MLSz,3,/nan)
;   MLSt = smooth(MLSt,3,/nan)
...
PlotArray = smooth(MLSt,5,/edge,/nan)
x = where(MLSt gt max(level1),nx)
if nx gt 0 then PlotArray[x] = max(level1)-1.
x = where(MLSt lt min(level1),nx)
if nx gt 0 then PlotArray[x] = min(level1) +1.
    ...
    contour,MLSt[*,xx], lons, lat[xx], levels=level1a,color=0,charsize=3,$
...
PlotArray = smooth(MLSz,5,/edge,/nan)

x = where(MLSz gt max(level1),nx)
if nx gt 0 then PlotArray[x] = max(level1)-.5
x = where(MLSz lt min(level1),nx)
if nx gt 0 then PlotArray[x] = min(level1) +.5
    ...
    contour,MLSz[*,xx], lons, lat[xx], levels=level1a,color=0,charsize=3,$
        min_value = -2., max_value =40000.,/overplot, xticks=5, thick=1
...
;-----------------------------------------------------------------------
   ...
   if setplot eq 'ps' then begin
	...
	'pnmtopng idl001.ppm > /Volumes/MacD68-1/france/ES_paper/Figures/ES_daily/MLS_ES_event_' + strtrim(strcompress(string(format='(I2.2)',nnday + 1L)),2) +$
...
save, filename = '/Volumes/MacD68-1/france/ES_paper/Post_process/MLS_ES_daily_max_T_Z.sav', $
		dailymaxheightlon, dailymaxheightlat,dailymaxtemplon, dailymaxtemplat, esdate
end
