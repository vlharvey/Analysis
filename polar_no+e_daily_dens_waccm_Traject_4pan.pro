; 
; WACCM files from with AOA1 set to electron field
...
dir='/Volumes/earth/harvey/WACCM_data/Datfiles/Datfiles_Traject/Traject.cam2.h1.'
...
;
          for ivar=0,result0.nvars-1 do begin
              ...
              if result.name eq 'AOA2' then aoagrd=data
          ...
          FOR ilon = 0, nc-1 DO $
              FOR ilat = 0, nr-1 DO $
                  ...
                  FOR itime = 0, nt-1 DO $
                      ...
                      pgrd(ilon,ilat,ialt,itime) = hyam(ialt)*Pzero + hybm(ialt)*PSFC(ilon,ilat,itime)
;
...
for itime=10L,nt-1L do begin
    q1=reform(no_conc(*,*,0,itime))      ; NO at top level
    q=0.*fltarr(nc+1,nr)
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='polar_no+e_'+sdate(itime)+'_'+slev0+'_dens_waccm_Traject_4pan.ps'
       device,/color
       device,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize
    endif

    zavg=mean(zgrd(*,nr/2:nr-1,ilev,itime))
    szavg=string(FORMAT='(i3)',zavg)+'km'
    no1=reform(no_conc(*,*,ilev,itime))
    no=0.*fltarr(nc+1,nr)
    no(0:nc-1,0:nr-1)=no1(0:nc-1,0:nr-1)
    no(nc,*)=no(0,*)
    e1=reform(egrd(*,*,ilev,itime))
    e=0.*fltarr(nc+1,nr)
    e(0:nc-1,0:nr-1)=e1(0:nc-1,0:nr-1)
    e(nc,*)=e(0,*)
    u1=reform(ugrd(*,*,ilev,itime))
    u=0.*fltarr(nc+1,nr)
    u(0:nc-1,0:nr-1)=u1(0:nc-1,0:nr-1)
    u(nc,*)=u(0,*)
    v1=reform(vgrd(*,*,ilev,itime))
    v=0.*fltarr(nc+1,nr)
    v(0:nc-1,0:nr-1)=v1(0:nc-1,0:nr-1)
    v(nc,*)=v(0,*)
    aoa1=reform(aoagrd(*,*,ilev,itime))
if max(aoa1) eq 0. then goto,jumplev
    aoa=0.*fltarr(nc+1,nr)
    aoa(0:nc-1,0:nr-1)=aoa1(0:nc-1,0:nr-1)
    aoa(nc,*)=aoa(0,*)
    z1=reform(zgrd(*,*,ilev,itime))
    z=0.*fltarr(nc+1,nr)
...
print,'Max Speed at ',lev(ilev),zavg,max(speed)
    ...
    !type=2^2+2^3
    xyouts,.15,.95,'WACCM-4 '+sdate(itime)+'     '+slev+' ('+szavg+')',/normal,color=0,charsize=1.5
...
;endif
;print,sdate(itime),'  NO ',imin,imax
;imin=3.e-16
;imax=2.e-15
;imin=nomin(ilev)
;imax=nomax(ilev)
...
;   RGB_TABLE=rgbTable, $
;   /STREAMLINES, view_title=sdate(itime)+'     ('+szavg+')'
    ...
    set_viewport,xmn,xmx,ymn,ymx
    !type=2^2+2^3
    MAP_SET,90,0,-90,/ortho,/noeras,/grid,/contin,title='AOA electrons',charsize=1.5,color=0,/noborder
;   MAP_SET,0,0,0,/noeras,/grid,/contin,title='AOA electrons',charsize=1.5,color=0,/noborder
index=where(alat ge -90.)
imin=min(aoa(*,index))
imax=max(aoa(*,index))
    nlvls=11
    level=imin+((imax-imin)/float(nlvls))*findgen(nlvls+1)
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*icolmax/nlvls
    contour,aoa,x,alat,/overplot,levels=level,c_color=col1,/cell_fill,/noeras
index=where(level gt 0.)
if index(0) ne -1L then contour,aoa,x,alat,/overplot,levels=level(index),/follow,c_labels=0*level(index),/noeras,color=0
    MAP_SET,90,0,-90,/ortho,/noeras,/grid,/contin,color=mcolor
...
;velovect,u,v,x,alat,color=0,/overplot,length=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim polar_no+e_'+sdate(itime)+'_'+slev0+'_dens_waccm_Traject_4pan.ps -rotate -90 polar_no+e_'+sdate(itime)+'_'+slev0+'_dens_waccm_Traject_4pan.jpg'
       spawn,'rm -f polar_no+e_'+sdate(itime)+'_'+slev0+'_dens_waccm_Traject_4pan.ps'
    endif
jumplev:
