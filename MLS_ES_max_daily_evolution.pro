...
restore, '/Volumes/MacD68-1/france/ES_paper/Post_process/MLS_ES_daily_max_T_Z.sav'
...
;
 ...
 for iday = 30, 59L do begin
    ...
    dailymark = reform(mlsmarks[sdateindex,*,*])

        ;-------------create wrap around for plot------------
    marker = fltarr(n_elements(dailymark[*,0L]) + 2L, n_elements(dailymark[0L,*]))
    marker[0L:n_elements(dailymark[*,0L]) - 1L, *] = dailymark[*,*]
   marker[n_elements(dailymark[*,0L])-1L:n_elements(dailymark[*,0L])+1L,*] = dailymark[0L:2L,*]
...
if setplot eq 'ps' then begin
	device, /close
	spawn,'pstopnm -dpi=300 -landscape idl.ps'	spawn,'pnmtopng idl001.ppm > /Volumes/MacD68-1/france/ES_paper/Figures/MLS_ES_event'+strtrim(strcompress(iES),2)+'_max_daily_evolution.png'
