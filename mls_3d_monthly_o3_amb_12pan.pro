...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/portrait,bits=8,filename='mls_3d_o3_amb_12pan.ps'
   device,/color
...
xyouts,.3,.95,'MLS Ambient Ozone',charsize=3,/normal
contour,out_o3_avg_all,ybin,th,xrange=[-90.,90.],yrange=[400.,2000.],/noeras,/cell_fill,$
        c_color=col1,levels=level,yticks=n_elements(ynames)-1,ytickv=th(yindex),$
        ytickname=ynames,min_value=-9999.,title=strupcase(strmid(mon(imonth),0,3)),charsize=1.75
contour,out_o3_avg_all,ybin,th,/overplot,levels=level,/follow,color=0,min_value=-9999.,c_labels=0*level
endfor		; loop over months
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim mls_3d_o3_amb_12pan.ps mls_3d_o3_amb_12pan.jpg'
endif
end
