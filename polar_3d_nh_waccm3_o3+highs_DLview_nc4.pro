...
; Arctic vortex colored by Ozone
...
;
      ...
      x(nc)=alon(0)+360.
mark2l=mark2	;smooth(mark2,3,/edge_truncate)
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='Figures/'+ifile+'_nh_5000K.ps'
       device,/color
...
;
        ...
        col1(kk)=nz2*icolmax
        dum=fltarr(nc+1,nr2)
        dum(0:nc-1,0:nr2-1)=mark1l(0:nc-1,nr/2:nr-1)    ; NH
        dum(nc,*)=dum(0,*)
;
...
;              if min(y2d(index)) le 13.7500 then goto,jump1
               if min(y2d(index)) le 3.75 then goto,jump1
               ...
               if index(0) ne -1 then dum(index)=9999.
;              index=where(tmp ne 9999. and y2d gt 13.7500 and dum eq -1.0*(ihigh+1))
               ...
               oplot,xcn(index),ycn(index),psym=8,color=0,/T3D,zvalue=nz,symsize=1.8
contour,dum,xcn,ycn,levels=[-.1],color=mcolor*.7,c_labels=0,/T3D,zvalue=nz,thick=3
...
jumplev:
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim Figures/'+ifile+'_nh_5000K.ps -rotate -90 Figures/'+ifile+'_nh_5000K.jpg'
       spawn,'/usr/bin/rm Figures/'+ifile+'_nh_5000K.ps'
    endif
