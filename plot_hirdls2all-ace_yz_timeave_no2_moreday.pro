...
;
;restore,file='/aura3/data/HIRDLS_data/Analysis/HIRDLS2ALL_no2_20060501-20060531.sav'
;restore,file='/aura3/data/HIRDLS_data/Analysis/HIRDLS2ALL_no2_20060501-20060531_locatimes_12-18.sav'
restore,file='/aura3/data/HIRDLS_data/Analysis/HIRDLS2ALL_no2_v2.02.06.20060507-20060531_locatimes_12-18.sav'
...
for k=0,n_elements(altitude)-1L do begin
    ...
    for kk=0L,n_elements(hirdls_pressure)-2L do begin
        ...
        if hirdls_NO2Yp(j,kk) ne -99. and hirdls_NO2Yp(j,kk+1L) ne -99. then begin
        if hz0 lt zp and hz1 ge zp then begin
           zscale=(hz1-zp)/(hz1-hz0)
           hirdls_no2yz(j,k)=hirdls_NO2Yp(j,kk+1)-zscale*(hirdls_NO2Yp(j,kk+1L)-hirdls_NO2Yp(j,kk))
;print,hz0,zp,hz1,zscale
;print,hirdls_NO2Yp(j,kk),hirdls_no2yz(j,k),hirdls_NO2Yp(j,kk+1L)
;stop
        endif
        endif
    endfor
...
if setplot eq 'ps' then begin
   ...
   ysize=nydim/100.
   device,font_size=9
   device,/landscape,bits=8,filename='hirdls2allv2.02.06-ace_yz_'+adate0+'-'+adate1+'_no2_moreday.ps'
   device,/color
   device,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize
endif

erase
hdata=HIRDLS_no2YZ*1.e9
adata=ace_no2YZ*1.e9
ddata=diff_no2YZ
...
;
    ...
    contour,hdata,latbin,altitude,xrange=[-90.,90.],yrange=[15.,50.],xticks=6,ytitle='Altitude (km)',$
            ...
            title='HIRDLS v2.02.06 NO!l2!n LocTime=12-18 '+hdate0+'-'+hdate1,c_color=col1,color=0,min_value=-99999.
...
;   contour,ddata,latbin,altitude,/overplot,levels=[0],color=mcolor,/follow,min_value=-99999.,thick=3
    ...
    contour,adata,latbin,altitude,xrange=[-90.,90.],yrange=[15.,50.],xticks=6,ytitle='Altitude (km)',$
            xtickname=xlabs,charsize=2.5,levels=level,/cell_fill,$
            title='ACE NO!l2!n '+adate0+'-'+adate1,c_color=col1,color=0,min_value=-99999.
...
if x(0) ne -1L then ddata(x)=-99999.
    ...
    contour,ddata,latbin,altitude,xrange=[-90.,90.],yrange=[15.,50.],xticks=6,$
            ...
            title='HIRDLS-ACE NO!l2!n Difference (%)',c_color=col1,color=0,min_value=-99999.,xtitle='Latitude'
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim hirdls2allv2.02.06-ace_yz_'+adate0+'-'+adate1+'_no2_moreday.ps -rotate -90 hirdls2allv2.02.06-ace_yz_'+adate0+'-'+adate1+'_no2_moreday.jpg'
endif
end
