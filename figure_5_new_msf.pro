...
; minimum PV is not in the lower half of MSF bins
;
@stddat
@kgmt
@ckday
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='figure_5_new_msf.ps'
endif
erase
!type=2^2+2^3
xmn=xorig(0)
...
;
    spawn,'ls geos5_pv_freq_minout_sh_*'+stimes(itime)+'_msf.sav',gfiles
...
;
    spawn,'ls geos5_pv_freq_maxout_nh_*'+stimes(itime)+'_msf.sav',gfiles
    nyear=n_elements(gfiles)
    for iyear=0L,nyear-1L do begin
        restore,gfiles(iyear)
        pvfreq=smooth(pvfreq,3)
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim figure_5_new_msf.ps -rotate -90 figure_5_new_msf.jpg'
;  spawn,'/usr/bin/rm figure_5_new_msf.ps'
endif
end
