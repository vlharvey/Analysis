;
; input x,y,date,time and U and V profiles
...
pthout='/Volumes/earth/aura3/data/UKMO_data/Marker_profiles/'
mon=['jan_','feb_','mar_','apr_','may_','jun_',$
     'jul_','aug_','sep_','oct_','nov_','dec_']
lstmn=1L & lstdy=1L & lstyr=2010L
ledmn=5L & leddy=22L & ledyr=2012L

lstmn=10L & lstdy=17L & lstyr=1991L
ledmn=12L & leddy=31L & ledyr=1999L
...
zonal_wind_profile=reform(pj1+yscale*(pjp1-pj1))

pj1=vgrd(j,i,*)+xscale*(vgrd(j,ip1,*)-vgrd(j,i,*))
pjp1=vgrd(jp1,i,*)+xscale*(vgrd(jp1,ip1,*)-vgrd(jp1,i,*))
meridional_wind_profile=reform(pj1+yscale*(pjp1-pj1))
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename=pthout+'mark2poker_'+sdate+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
plot,vortex_marker_profile,th,thick=10,xtitle='Vortex Marker',ytitle='Potential Temperature',$
     title='Poker Flat '+sloc,xrange=[-1.,1.],color=0,yrange=[400.,max(th)]
...
save,file=pthout+'meto_at_poker_'+sdate+'.sav',date,time,longitude,latitude,$
     potential_temperature_profile,vortex_marker_profile,pressure_profile,$
     temperature_profile,altitude_profile,zonal_wind_profile,meridional_wind_profile,comment
print,'saved DMP '+sdate

if setplot eq 'x' then stop
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim '+pthout+'mark2poker_'+sdate+'.ps -rotate -90 '+pthout+'mark2poker_'+sdate+'.jpg'
   spawn,'rm -f '+pthout+'mark2poker_'+sdate+'.ps'
endif
goto,jump
end
