...
;if index(0) ne -1L then oplot,mlon(index),mlat(index),psym=8,color=mcolor*.3
        ...
        outvort=where(mmark_lev le 0.5 and mmark_lev ne -99. and mtp_lev ne -99. and mco_lev ne -99.,nout)
...
;
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       set_plot,'ps'
       device,/color,/landscape,bits=8,filename='prof_nh_mls_dthdz_in_out_'+sdate+'_z.ps'
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim prof_nh_mls_dthdz_in_out_'+sdate+'_z.ps -rotate -90 '+$
                            'prof_nh_mls_dthdz_in_out_'+sdate+'_z.jpg'
        spawn,'/usr/bin/rm prof_nh_mls_dthdz_in_out_'+sdate+'_z.ps'
     endif
     jumpday:
goto,jump
end
