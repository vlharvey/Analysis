...
;
WACCMtemp = 0L 
restore, '/Volumes/MacD68-1/france/WACCM_paper/Post_process/MLS_area_weighted_strat_daily_mean.sav'
;
spawn,'ls /Volumes/MacD68-1/france/ES_paper/Post_process/MLS_timeseries_NH_20*.sav',ifiles
...
iyear2 = iyear+4L
xdate = where(dates eq '20'+string(FORMAT='(I2.2)',iyear2)+'1101')
...
; [, SIGMA=variable] [, STATUS=variable] [, YBAND=variable] [, YERROR=variable] [, YFIT=variable] )
	...
	st = reform(waccmtemp[ii,15:99])
	z1 = reform(altitude[15:99])
...
level  = 6.*findgen(16)+180.		
levela  = 6.*findgen(21)+168.		
...
if nx gt 0L then fake[x] = min(level) +.1
  ...
  contour, fake,days, z ,  levels=level, /cell_fill, c_color = col1,$
           xrange = [1,181],yrange=[20,100],ytitle='Altitude (km)', ystyle = 1,title = 'NH 20' + string(FORMAT='(I2.2)',iyear2),$
           min_value = 0.,max_value = 500., xticks = 6, position = [.1,.71,.9,.93],$
		   xtickname = ['Nov1', 'Dec1', 'Jan1', 'Feb1', 'Mar1', 'Apr1','May1'], xtickv = [1, 31, 62, 93, 121, 152, 181]
...
oplot, days,waccmstratopause,  psym = 8, color = 0,symsize = .6
...
loadct, 39
  ...
  if setplot eq 'ps' then begin
	...
	'pnmtopng idl001.ppm > /Volumes/MacD68-1/france/ES_paper/Figures/timeseries/MLS_timeseries_NH_20' + string(FORMAT='(I2.2)',iyear2) + '.png'
