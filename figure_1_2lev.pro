...
nxdim=700
nydim=700
xorig=[0.10,0.55,0.10,0.55,0.10,0.55]
yorig=[0.7,0.7,0.42,0.42,0.13,0.13]
xlen=0.25
ylen=0.25
cbaryoff=0.1
cbarydel=0.02
...
;
      ...
      if setplot eq 'ps' then begin
         ...
         device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='figure_1_'+sdate+'_2lev.ps'
      endif
      thlev1=3600.
...
;
      erase
      !type=2^2+2^3
      xyouts,.4,.975,sdate,/normal,color=0,charsize=1.5
      xmn=xorig(0)
      xmx=xorig(0)+xlen
      ymn=yorig(0)
      ymx=yorig(0)+ylen
...
;     level=-1.e-5+1.e-6*findgen(21)
      ...
      plot,[0,0],[imin,imax],xrange=[0,10],yrange=[imin,imax],title='(m!u2!ns!u-1!n)',color=0,xticks=4
      xbox=[0,10,10,0,0]
      y1=imin
...
;
      ...
      index=where(y2d gt 0.)
      pvmax=max(pv(index))-0.2*(max(pv(index)))
      pvmin=min(pv(index))
;pvmax=.1
pvmin=0.
      ...
      !type=2^2+2^3+2^5+2^7
      plot,[0,0],[imin,imax],xrange=[0,10],yrange=[imin,imax],title='(Km!u-2!nkg!u-1!ns!u-1!n)',$
...
;
; mean latitude per SF and PV bin
;
      !type=2^2+2^3
      xmn=xorig(4)
      xmx=xorig(4)+xlen
      ...
      sfmax=6e-6
      sfmin=-6e-6
      plot,findgen(10),findgen(10),/nodata,xrange=[sfmax,sfmin],yrange=[40.,90.],$
           color=0,xtitle='!4W!1',ytitle='Mean Latitude',xticks=2,charsize=1
      nbins=20L
...
;
      for ilev=nth-1L,0L,-1L do begin
          ...
          if th(ilev) eq thlev or th(ilev) eq thlev1 then oplot,sfbin(index),meanlat_sfsm,color=0,psym=0,linestyle=2,thick=5
...
;
      ...
      set_viewport,xmx+0.075,xmx+0.125,ymn,ymx
      ...
      pvmin=1.e-4
      pvmax=5.
      plot,findgen(10),findgen(10),/nodata,xrange=[pvmin,pvmax],yrange=[40.,90.],$
           color=0,xtitle='PV',ytitle='Mean Latitude',charsize=1,/xlog,$
...
;
      ...
      if setplot eq 'ps' then begin
         device,/close
         spawn,'convert -trim figure_1_'+sdate+'_2lev.ps -rotate -90 figure_1_'+sdate+'_2lev.jpg'
         spawn,'/usr/bin/rm figure_1_'+sdate+'_2lev.ps'
      endif
