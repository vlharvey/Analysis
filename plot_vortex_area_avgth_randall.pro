...
smon=['Sep','Oct','Nov','Dec','Jan','Feb','Mar','Apr','May']
...
;for k=0,nth-1 do begin
;rth=th(k)
kindex=where(th ge rth and th le 2000.)
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='arctic_vortex_area_avgth_randall.ps'
   device,/color
...
dall=smooth(dall,5)
;
; truncate timeseries to 1 Sep - 1 May
;
dum=dum(62:305)
ds=ds(62:305)
dmin=dmin(62:305)
...
plot,dum/hem_area,xrange=[0.,n_elements(dum)],yrange=[0.,.5],psym=8,$
     ytitle='Fraction of the Hemisphere',$
     title='1991-2004 Arctic Vortex Area 1200-2000 K',charsize=1.5,$
;    xtickv=[0,32,60,91,121,152,182,213,244,274,305,335],$
     xticks=n_elements(smon)-1,xtickname=smon,min_value=0.,/nodata
oplot,index,dum(index)/hem_area
oplot,index,(dum(index)/hem_area)+(ds(index)/hem_area)
oplot,index,(dum(index)/hem_area)-(ds(index)/hem_area)
for i=0L,n_elements(dum)-2L do begin
