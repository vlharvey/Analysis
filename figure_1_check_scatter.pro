...
thlev=3600.
      ...
      if setplot eq 'ps' then begin
         ...
         device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='figure_1_'+sdate+'_'+slev+'K_scatter.ps'
      endif
...
ytickname=['10!u-5!n','10!u-4!n','10!u-3!n','10!u-2!n','10!u-1!n','1'],ytickv=[1.e-5,1.e-4,1.e-3,1.e-2,1.e-1,1]
...
;
; scatter plot of NH SF vs PV.  How much does PV vary within SF bins?
;
      !type=2^2+2^3
      xmn=xorig(3)
      xmx=xorig(3)+xlen
      ...
      pvmax=5.
      plot,findgen(10),findgen(10),/nodata,xrange=[sfmin,sfmax],yrange=[pvmin,pvmax],$
           /ylog,color=0,xtitle='!4W!1',ytitle='PV',xticks=4,charsize=1,$
ytickname=['10!u-5!n','10!u-4!n','10!u-3!n','10!u-2!n','10!u-1!n','1'],ytickv=[1.e-5,1.e-4,1.e-3,1.e-2,1.e-1,1]
      sfbins=sfmin+((sfmax-sfmin)/20.)*findgen(21)
      dsf=sfbins(1)-sfbins(0)
      for ilev=0L,nth-1L do begin
          ...
          sigmapv=fltarr(n_elements(sfbins))
          for ii=0L,n_elements(sfbins)-1L do begin
              index=where(y2d gt 0. and sf ge sfbins(ii)-dsf and sf lt sfbins(ii)+dsf,npts)
              if npts gt 1L then oplot,sfbins(ii)+0.*findgen(npts),pv(index),psym=8,symsize=0.1*(ilev+1),color=col2(ilev)
          endfor
      endfor
...
;          xtickname=strcompress(long(th(xindex)),/remove_all),xtickv=th(xindex)
      ...
      if setplot eq 'ps' then begin
         device,/close
         spawn,'convert -trim figure_1_'+sdate+'_'+slev+'K_scatter.ps -rotate -90 figure_1_'+sdate+'_'+slev+'K_scatter.jpg'
         spawn,'/usr/bin/rm figure_1_'+sdate+'_'+slev+'K_scatter.ps'
      endif
