...
;
    erase
    xyouts,.4,.7,sdate+' '+slev+' hPa',/normal,color=0,charsize=3
    !type=2^2+2^3
    xmn=xorig(0)
    xmx=xorig(0)+xlen
    ymn=yorig(0)
...
pdiff=mlsspeed1-sp1
    map_set,90,0,-90,/ortho,/contin,/grid,/noerase,color=0,title='MLS-MERRA',label=1,lonlab=10.,latlab=0,latdel=10.,londel=45.,charsize=1.5
    contour,pdiff,alon,alat,levels=level,c_color=col2,/cell_fill,/overplot,/noeras
    contour,pdiff,alon,alat,levels=level,color=0,/follow,/overplot,/noeras
    map_set,90,0,-90,/ortho,/contin,/grid,/noerase,color=0,label=1,lonlab=10.,latlab=0,latdel=10.,londel=45.,charsize=1.5
    imin=min(level)
    imax=max(level)
    set_viewport,xmn,xmx,ymn-cbaryoff,ymn-cbaryoff+cbarydel
