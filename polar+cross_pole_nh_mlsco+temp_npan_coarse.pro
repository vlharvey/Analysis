...
int=0.5
nlvls=15
    level=[0.1,imin+int*findgen(nlvls)]
    slevel=string(format='(f3.1)',level)
    nlvls=n_elements(level)
col1=1+indgen(nlvls)*icolmax/nlvls
    contour,coswath,findgen(mprofs),altitude,levels=level,/cell_fill,c_color=col1,color=0,title='t='+stime,$
...
;endif
    ...
    set_viewport,xmn,xmx,ymn,ymx
plot,latlevs(*,0),colevs(*,0),color=0,title='CO at '+slev+' km',yrange=[0,16],$
     xtitle='Latitude',xrange=[20,90],psym=1,charsize=1
for iswath=1L,nswath-1L do begin
    oplot,latlevs(*,iswath),colevs(*,iswath),color=(float(iswath)/float(nswath))*mcolor,psym=1
endfor
    ...
    plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],/noeras,color=0,charsize=1.5,charthick=2,$
         xtitle='MLS CO (ppmv)',xtickname=slevel,xticks=n_elements(level)-1
    ybox=[0,10,10,0,0]
    x2=imin
