...
lstmn=5L & lstdy=1L & lstyr=2015
ledmn=6L & leddy=30L & ledyr=2015
lstday=0L & ledday=0L
...
;        !p.font=0
         ...
         device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
                /bold,/color,bits_per_pixel=8,/times,filename='polar_pv_merra2_'+sdate+'_'+stheta+'K_sh.ps'
         !p.charsize=1.25
         !p.thick=2
         !p.charthick=5
         !p.charthick=5
      ...
      imin= -2.5e-05	;min(pv)+0.3*min(pv)
      imax=0
      endif
      nlvls=20
...
pv(*,0)=0./0.
pv=smooth(pv,3,/edge_truncate,/Nan)
...
loadct,39
     contour,sf,x,alat,/overplot,nlevels=20,thick=2,color=mcolor
loadct,23
;    contour,sp,x,alat,/overplot,levels=[30.],thick=5,color=.65*mcolor
...
loadct,39
      MAP_SET,-90,0,0,/stereo,/noeras,/grid,/contin,/noborder,charsize=2.0,latdel=10,color=mcolor	;,limit=[15,0,90,360]
loadct,23
      ymnb=ymn -cbaryoff
      ymxb=ymnb+cbarydel
      ...
      if setplot eq 'ps' then begin
         device, /close
         spawn,'convert -trim polar_pv_merra2_'+sdate+'_'+stheta+'K_sh.ps -rotate -90 polar_pv_merra2_'+sdate+'_'+stheta+'K_sh.png'
;        spawn,'rm -f polar_pv_merra2_'+sdate+'_'+stheta+'K_sh.ps'
      endif
      icount=icount+1L
goto,jump
end
