...
FOR YEAR=2007,2013 DO BEGIN
   ...
   if year mod 4 eq 0L then mday(1)=28
   if year mod 4 ne 0L then mday(1)=29
   FOR IMON=0,NMON-1L DO BEGIN
   smn=nmon(imon)
...
;
      ...
      IF IDAY EQ 0 THEN BEGIN
         ICE=FLTARR(nc,nr,NDAYS)-99	;WILL HOLD FINAL INTEGRATED ICE COLUMN MASS
