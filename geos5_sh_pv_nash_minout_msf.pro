...
; min PV is not in the highest half of MSF bins
;
@stddat
@kgmt
@ckday
...
lstmn=5L & lstdy=1L & lstyr=2008L
ledmn=5L & leddy=31L & ledyr=2008L
lstday=0L & ledday=0L
;
; get date range
;
...
;
; min PV out of vortex and in vortex
;
;         aindex=where(y2d lt -40. and mark1 eq 0.)
          aindex=where(y2d lt -40. and sf1 gt sfthresh)
          if aindex(0) eq -1L then goto,jumplev
          amin=min(pv1(aindex))
;         vindex=where(y2d lt -40. and mark1 eq 1.)
          vindex=where(y2d lt -40. and sf1 le sfthresh)
          if vindex(0) eq -1L then goto,jumplev
          vmin=min(pv1(vindex))
;
; increment this level if amin lt vmin
;
          if amin lt vmin then begin
...
;index=where(y2d lt 0. and pv1 eq amin)
;oplot,x2d(index),y2d(index),psym=8,symsize=2,color=mcolor*.9
;index=where(y2d lt 0. and pv1 eq vmin)
...
save,file='geos5_pv_freq_minout_sh_'+syr+smn+'_msf.sav',pvfreq,sdates,sdate0,sdate1,th
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='geos5_sh_pv_'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_minout_msf.ps'
   device,/color
...
plot,pvfreq,th,xrange=[0.,100.],yrange=[min(th),max(th)],color=0,title=sdate0+'-'+sdate1,$
     xtitle='Percent of the time PV min is out of SH vortex',ytitle='Theta (K)',thick=5

if setplot ne 'ps' then stop
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim geos5_sh_pv_'+sdate+'_'+slabs(0)+'_nash_minout_msf.ps -rotate -90 '+$
         'geos5_sh_pv_'+sdate+'_'+slabs(0)+'_nash_minout_msf.jpg'
   spawn,'/usr/bin/rm geos5_sh_pv_'+sdate+'_'+slabs(0)+'_nash_minout_msf.ps'
endif
end
