...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='3d_nh_'+mon(imonth)+'_geos5.ps'
       device,/color
...
;
dum=dum/(xcn^2+ycn^2)
        index=where(dum lt -0.6,nl)

        if index(0) ne -1 then begin
           oplot,xcn(index),ycn(index),/T3D,zvalue=nz,psym=8,symsize=2,color=0
loadct,0
           contour,dum,xcn,ycn,levels=[-0.6],color=mcolor*.7,$
...
jumplev:
    ...
    !psym=0
    xyouts,0.33,0.88,month(imonth)+' GEOS-5',/normal,charsize=3.0,color=0,charthick=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim 3d_nh_'+mon(imonth)+'_geos5.ps -rotate -90 3d_nh_'+mon(imonth)+'_geos5.jpg'
;      spawn,'/usr/bin/rm 3d_nh_'+mon(imonth)+'_geos5.ps'
    endif
