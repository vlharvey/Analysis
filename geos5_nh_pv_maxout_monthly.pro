...
; maximum PV is not located in the low half of SF bins
;
@stddat
@kgmt
@ckday
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='geos5_nh_pv_maxout_monthly.ps'
endif
erase
!type=2^2+2^3
xmn=xorig(0)
...
;   pvfreqmean=smooth(pvfreqmean,3)
    if itime eq 0L then begin
       plot,pvfreqmean,th,xrange=[0.,100.],yrange=[300.,max(th)],color=0,/nodata,$
            xtitle='Percent of the time max PV in high SF bins',ytitle='Theta (K)'
    endif
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim geos5_nh_pv_maxout_monthly.ps -rotate -90 '+$
         'geos5_nh_pv_maxout_monthly.jpg'
   spawn,'/usr/bin/rm geos5_nh_pv_maxout_monthly.ps'
endif
end
