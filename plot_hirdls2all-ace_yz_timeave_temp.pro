...
if setplot eq 'ps' then begin
   ...
   ysize=nydim/100.
   device,font_size=9
   device,/landscape,bits=8,filename='hirdls2allv2.02.06-ace_yz_'+adate0+'-'+adate1+'_temp.ps'
   device,/color
...
;
    ...
    xlabs=' '+strarr(7)
    contour,hdata,latbin,altitude,xrange=[-90.,90.],yrange=[10.,70.],xticks=6,ytitle='Altitude (km)',$
            xtickname=xlabs,charsize=2.5,levels=level,/cell_fill,$
            title='HIRDLS v2.02.06 Temperature '+hdate0+'-'+hdate1,c_color=col1,color=0,min_value=-99999.
...
;   contour,ddata,latbin,altitude,/overplot,levels=[0],color=mcolor,/follow,min_value=-99999.,thick=3
    ...
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*mcolor/nlvls
    contour,adata,latbin,altitude,xrange=[-90.,90.],yrange=[10.,70.],xticks=6,ytitle='Altitude (km)',$
            xtickname=xlabs,charsize=2.5,levels=level,/cell_fill,$
...
if x(0) ne -1L then ddata(x)=-99999.
    ...
    contour,ddata,latbin,altitude,xrange=[-90.,90.],yrange=[10.,70.],xticks=6,$
            xtickname=xlabs,charsize=2.5,levels=dlevel,/cell_fill,ytitle='Altitude (km)',$
            title='HIRDLS-ACE Temperature Difference (%)',c_color=col1,color=0,min_value=-99999.,xtitle='Latitude'
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim hirdls2allv2.02.06-ace_yz_'+adate0+'-'+adate1+'_temp.ps -rotate -90 hirdls2allv2.02.06-ace_yz_'+adate0+'-'+adate1+'_temp.jpg'
endif
end
