...
;
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='polar_mls_n2o+geos5_edge_nh_pv_'+sdate+'_'+slabs(itime)+'_'+slev+'_'+sver+'.ps'
         device,/color
...
;
      ...
      ymx=yorig(5)+ylen
      set_viewport,xmn,xmx,ymn,ymx
      plot,codata,h2odata,psym=8,color=0,xrange=[1.e-2,4.],yrange=[3,8],symsize=0.25,$
           xtitle='N!l2!nO',ytitle='H!l2!nO',/nodata,title='All Levels',/xlog
mtheta0=600.
mtheta1=2000.
index=where(mco ne -99. and mh2o ne -99. and mtheta le mtheta1 and mtheta ge mtheta0,npts)
for ii=0L,npts-1L do begin
     oplot,[1.e7*mco(index(ii)),1.e7*mco(index(ii))],[1.e6*mh2o(index(ii)),1.e6*mh2o(index(ii))],psym=8,symsize=0.25,$
           color=((mtheta(index(ii))-mtheta0)/(mtheta1-mtheta0))*mcolor
endfor
      set_viewport,xmx+cbaryoff,xmx+cbaryoff+cbarydel,ymn,ymx
      !type=2^2+2^3+2^5
      nlvls=11
...
omax=mtheta1
      ...
      nlvls=n_elements(level)
      col1=1+indgen(nlvls)*mcolor/nlvls
      plot,[0,0],[omin,omax],xrange=[0,10],yrange=[omin,omax],title='Theta (K)',color=0
      xbox=[0,10,10,0,0]
      y1=omin
      dy=(omax-omin)/float(nlvls)
      for j=0,nlvls-1 do begin
      ...
      if setplot eq 'ps' then begin
         device,/close
         spawn,'convert -trim polar_mls_n2o+geos5_edge_nh_pv_'+sdate+'_'+slabs(itime)+'_'+slev+'_'+sver+'.ps -rotate -90 '+$
               'polar_mls_n2o+geos5_edge_nh_pv_'+sdate+'_'+slabs(itime)+'_'+slev+'_'+sver+'.jpg'
      endif
