...
for ilev=1L,21L do begin	;nl-1L do begin
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='polar_lat+height_'+sdate+'_'+slev0+'_waccm_TEMAOA_4pan_nh.ps'
       device,/color
...
imax=max(aoa2yz)
    ...
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*icolmax/nlvls
    contour,aoa2yz,alat,zlev,yrange=[0.,140.],levels=level,c_color=col1,/cell_fill,/noeras,color=0,ytitle='Altitude (km)',xtitle='Latitude',title='AOA2=Altitude-Z0'
index=where(level gt 0)
...
;velovect,u,v,x,alat,color=0,/overplot,length=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim polar_lat+height_'+sdate+'_'+slev0+'_waccm_TEMAOA_4pan_nh.ps -rotate -90 polar_lat+height_'+sdate+'_'+slev0+'_waccm_TEMAOA_4pan_nh.jpg'
       spawn,'rm -f polar_lat+height_'+sdate+'_'+slev0+'_waccm_TEMAOA_4pan_nh.ps'
    endif
jumplev:
