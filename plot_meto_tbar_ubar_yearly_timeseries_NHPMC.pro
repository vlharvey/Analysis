...
; data is plotted wrt NH DFS -40 to DFS +80
;
@stddat
@kgmt
@ckday
...
;
index=where(sdate gt 20070101L)
sdate=sdate(index)
tbar=reform(tbar(*,*,index))
ubar=reform(ubar(*,*,index))
;
; compute DFS relative to NH for all dates
;
nn=n_elements(sdate)
fdoy=fltarr(nn)
dfs=fltarr(nn)
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/times,filename='meto_tbar_ubar_yearly_timeseries_NHPMC_'+slat+'_'+slev+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
for iseason=0L,nseason-1L do begin
    tdatayear=reform(tdata(dfsindex(iseason):dfsindex(iseason)+121L))
...
plot,-40.+findgen(121),findgen(121),/nodata,xrange=[-40.,80.],yrange=[tmin,tmax],/noeras,charsize=2,charthick=2,color=0,ytitle='MetO Tbar (K)',title=slat+' '+slev+' hPa',xtitle='DFS (21 June)'
dfsindex=where(dfs eq -40.,nseason)
yearmin=long(strmid(sdate(dfsindex(0)),0,4))
yearmax=long(strmid(sdate(dfsindex(nseason-1)),0,4))
for iseason=0L,nseason-1L do begin
    dfsyear=reform(dfs(dfsindex(iseason):dfsindex(iseason)+121L))
    tdatayear=reform(tdata(dfsindex(iseason):dfsindex(iseason)+121L))
    index=where(finite(tdatayear) eq 1L)
...
umin=100.
for iseason=0L,nseason-1L do begin
    udatayear=reform(udata(dfsindex(iseason):dfsindex(iseason)+121L))
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim meto_tbar_ubar_yearly_timeseries_NHPMC_'+slat+'_'+slev+'.ps -rotate -90 meto_tbar_ubar_yearly_timeseries_NHPMC_'+slat+'_'+slev+'.jpg'
endif
end
