...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='hov_Mprmark_'+sth+'K_'+slat+'_uars+mark_timeseries.ps'
   device,/color
...
plt=reform(msfprxt2d,nc,nfile*nyear)
...
contour,plt,alon,findgen(nfile*nyear),xrange=[0.,360.],/fill,$
        ...
        title='MSF-MSFavg',/noeras,levels=level,charsize=2,min_value=-9999.
...
plot,findgen(nday*nyear),plt1d,xrange=[0,nfile*nyear-1L],psym=0,yrange=[-5000.,5000.],$
...
plot,findgen(nday*nyear),plt1d,xrange=[0,nfile*nyear-1L],psym=0,yrange=[0.,40.],$
     xticks=nytick-1,xtickv=index0,xtickname=strmid(sfileall(index0),7,2),/noeras,title='ZM Anticyclones'

if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim hov_Mprmark_'+sth+'K_'+slat+'_uars+mark_timeseries.ps '+$
         '-rotate -90 hov_Mprmark_'+sth+'K_'+slat+'_uars+mark_timeseries.jpg'
endif
jump2:
end
