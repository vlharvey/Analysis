...
if setplot eq 'ps' then begin
   ...
   ysize=nydim/100.
   set_plot,'ps'
   device,/color,/landscape,bits=8,filename='polar_hirdls_mls_o3.ps'
...
xyouts,.2,.85,sdate+' 46.4159 hPa Ozone',charsize=2,/normal
xyouts,.15,.8,'Antarctic Vortex at 525 K (~43 hPa)',charsize=2,/normal
xmn=xorig(0)
xmx=xorig(0)+xlen
ymn=yorig(0)
ymx=yorig(0)+ylen
...
omin=0.
omax=5.
map_set,-90,0,0,/ortho,/contin,/grid,title='MLS',/noerase,charsize=2
index=where(fix(mpress2) eq 68,npt)
index=where(fix(mpress2) eq 46,npt)
mx=mlon2(index) & my=mlat2(index) & mozone=mo3(index)*1.e6
print,'mls ',min(mozone),max(mozone)
for i=0L,npt-1L do $
    oplot,[mx(i),mx(i)],[my(i),my(i)],psym=8,$
          color=((mozone(i)-omin)/(omax-omin))*icolmax
...
!type=2^2+2^3
set_viewport,xmn,xmx,ymn,ymx
map_set,-90,0,0,/ortho,/contin,/grid,title='HIRDLS',/noeras,charsize=2
index=where(fix(hpress2) eq 68,npt)
index=where(fix(hpress2) eq 46,npt)
hx=hlon2(index) & hy=hlat2(index) & hozone=ho3(index)*1.e6
print,'hirdls ',min(hozone),max(hozone)
for i=0L,npt-1L do $
    oplot,[hx(i),hx(i)],[hy(i),hy(i)],psym=8,$
          color=((hozone(i)-omin)/(omax-omin))*icolmax
contour,mark,x,alat,/overplot,levels=[0.1],/follow,thick=5,c_labels=[0],color=lc
;contour,mark,x,alat,/overplot,levels=[-0.1],/follow,thick=4,c_labels=[0],color=lc
;contour,pv,x,alat,/overplot,nlevels=20,/follow,c_labels=[0],color=lc
map_set,-90,0,0,/ortho,/contin,/grid,title='HIRDLS',/noeras,charsize=2
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert polar_hirdls_mls_o3.ps -rotate -90 polar_hirdls_mls_o3.jpg'
endif
