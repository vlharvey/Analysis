...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='arctic_vortex_area_'+stheta+'.ps'
   device,/color
...
dall=0.*reform(area_all(13,*,kindex(0)))
dmin=0.*reform(AREA_AVE(*,kindex(0)))
dmax=0.*reform(AREA_AVE(*,kindex(0)))
dum(0:184)=AREA_AVE(181:365,kindex(0))
dum(185:365)=AREA_AVE(0:180,kindex(0))
ds(0:184)=AREA_SIG(181:365,kindex(0))
ds(185:365)=AREA_SIG(0:180,kindex(0))
dmin(0:184)=AREA_min(181:365,kindex(0))
dmin(185:365)=AREA_min(0:180,kindex(0))
dmax(0:184)=AREA_max(181:365,kindex(0))
dmax(185:365)=AREA_max(0:180,kindex(0))
dall(0:184)=AREA_all(12,181:365,kindex(0))
dall(185:365)=AREA_all(13,0:180,kindex(0))

index=where(dum gt 0.)
plot,dum/hem_area,xrange=[0.,365.],yrange=[0.,.5],psym=8,$
     ytitle='Fraction of the Hemisphere',xtitle='Julian Day',$
     title='1991-2004 Arctic Vortex Area at '+stheta,charsize=1.5,$
...
for i=0,364 do begin
    if dum(i) ne 0. and dum(i+1) ne 0. then begin
     ...
     plots,i+1,dum(i+1)/hem_area,/continue,thick=8
    endif
endfor
...
for i=0,364 do begin
    if dall(i) ne 0. and dall(i+1) ne 0. then begin
     ...
     plots,i+1,dall(i+1)/hem_area,/continue,thick=5,color=icolmax*.3
    endif
endfor
