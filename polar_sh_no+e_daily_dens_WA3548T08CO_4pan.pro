...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='polar_sh_no+e_'+sdate+'_'+slev0+'_dens_WA3548T08CO_4pan.ps'
       device,/color
...
;velovect,u,v,x,alat,color=0,/overplot,length=2
    ...
    set_viewport,xmn,xmx,ymn,ymx
    !type=2^2+2^3
    MAP_SET,-90,0,90,/ortho,/noeras,/grid,/contin,title='Wind Speed',charsize=1.5,color=0,/noborder,limit=[-30.,0.,-90.,360.]
imin=0.
imax=250.
index=where(alat lt -50.)
...
;velovect,u,v,x,alat,color=mcolor,/overplot,length=1
    ...
    set_viewport,xmn,xmx,ymn,ymx
    !type=2^2+2^3
    MAP_SET,-90,0,90,/ortho,/noeras,/grid,/contin,title='d(Theta)/dt',charsize=1.5,color=0,/noborder,limit=[-30.,0.,-90.,360.]
imin=-500.
imax=500.
index=where(alat lt -50.)
...
;velovect,u,v,x,alat,color=0,/overplot,length=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim polar_sh_no+e_'+sdate+'_'+slev0+'_dens_WA3548T08CO_4pan.ps -rotate -90 polar_sh_no+e_'+sdate+'_'+slev0+'_dens_WA3548T08CO_4pan.jpg'
       spawn,'rm -f polar_sh_no+e_'+sdate+'_'+slev0+'_dens_WA3548T08CO_4pan.ps'
    endif
