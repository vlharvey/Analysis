...
; 
        if iday eq 0L then nvort=fltarr(ndays,nz)
        if iday eq 0L then sdate_all=strarr(ndays)
...
;
; approximate vortex area with a circle with radius=distance to edge
; draw range ring to vortex edge
;
xm0=xphase(nz-1-ii)
ym0=yphase(nz-1-ii)
;
; closest point to the center that is NOT in the vortex
;
index=where(marker ne 1.)
xdata=x2d(index)
ydata=y2d(index)
;oplot,xdata,ydata,psym=1,color=0
dxf=re*abs(xm0-xdata)*dtr*cos(ym0*dtr)
dyf=re*abs(ym0-ydata)*dtr
dist=sqrt(dxf*dxf+dyf*dyf)
dist0=min(dist) + ((alat(1)-alat(0))*111.)		; add dy distance to accommodate grid spacing
print,'closest out vortex ',dist0
print,'theta= ',th2(nz-1L-ii),' center of mass= ',xm0,ym0,' dist outward= ',dist0
range_ring,ym0,xm0,dist0,360,bear,rlats,rlons
;oplot,[xm0,xm0],[ym0,ym0],psym=8,color=thlevel(ii),symsize=3
;oplot,rlons,rlats,color=thlevel(ii),thick=3
;
...
; save file
;
save,filename='merra_zt_nvort_ES_event_'+sevent+'.sav',nvort,th2r,sdate_all
