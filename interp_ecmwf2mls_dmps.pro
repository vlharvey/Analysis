;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Linearly interpolate ECMWF analysis in space and time to MLS profiles
...
diru='/aura6/data/ECMWF_data/Datfiles/ecmwf_'
re=40000./2./!pi
...
sdate=result2(0)
dum=findfile(pthout+'dmps_mls_'+sver+'.ecmwf.'+sdate+'.sav')
...
restore,'/aura3/data/HIRDLS_data/Datfiles/theta.dat'
srzo=zo
thlev=zo
;;;
hpress=p
...
;
; read ECMWF data on day 0
;
    newday=0L
    ifile=string(FORMAT='(i4,i2.2,i2.2,a8)',iyr0,imn0,idy0,'_12Z.nc3')
    if n eq 0L then begin
       rd_ecmwf_nc3_manney,diru+ifile,ncw,nrw,nthw,alon,alat,th,pvold,pold,msfold,$
...
;      print,n,nfiles,ifile
    ...
    file1=ifile
;
; read ECMWF data on day 1
;
    ifile=string(FORMAT='(i4,i2.2,i2.2,a8)',iyr1,imn1,idy1,'_12Z.nc3')
    if n eq 0L then begin
       rd_ecmwf_nc3_manney,diru+ifile,ncw,nrw,nthw,alon,alat,th,pvnew,pnew,msfnew,$
...
;
; set bad data to special value
; ECMWF data processing prs2th code needs to flag for special values in the troposphere
;
    index=where(pvold gt 1000. or pvnew gt 1000.)
...
;
; interpolate ECMWF to MLS location
;
...
;
    for kk=0L,nth-1L do begin
        ...
        for k=1L,nthw-1L do begin
            kp1=k-1             ; ECMWF data is "top down"
            uz=th(k)
            uzp1=th(kp1)
            zscale=(th1-uz)/(uzp1-uz)
;
; jump level if any one of 8 gridpoints has no data
;
            if zgrd(j,i,k) eq -99. or zgrd(j,ip1,k) eq -99. or $
               zgrd(jp1,i,k) eq -99. or zgrd(jp1,ip1,k) eq -99. or $
               zgrd(j,i,kp1) eq -99. or zgrd(j,ip1,kp1) eq -99. or $
               zgrd(jp1,i,kp1) eq -99. or zgrd(jp1,ip1,kp1) eq -99. then begin
;              print,'bad geopotential height ',zgrd(j,i,k),zgrd(j,ip1,k),$
;                    zgrd(jp1,i,k),zgrd(jp1,ip1,k),zgrd(j,i,kp1),zgrd(j,ip1,kp1),$
;                    zgrd(jp1,i,kp1),zgrd(jp1,ip1,kp1)
               goto,jumpg
            endif
...
;
time=hh2
save,file=pthout+'dmps_mls_'+sver+'.ecmwf.'+sdate+'.sav',id,date,time,longitude,latitude,thlev,$
