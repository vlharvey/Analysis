...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='geos5_nh_sf_vs_pv_difflat_monthly.ps'
endif
erase
!type=2^2+2^3
xmn=xorig(0)
...
;
    ...
    if itime eq 0L then begin
       plot,sfdiffmean,th,xrange=[0.,30.],yrange=[300.,max(th)],color=0,$
            xtitle='Mean Lat at Min SF - Max Lat',ytitle='Theta (K)',thick=7
       oplot,pvdiffmean,th,thick=7,linestyle=5
    endif
    if itime eq 0L then begin
       plot,sfdiffmean,th,xrange=[0.,30.],yrange=[300.,max(th)],color=0,$
            xtitle='Mean Lat at Min SF - Max Lat',ytitle='Theta (K)',/nodata
    endif
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim geos5_nh_sf_vs_pv_difflat_monthly.ps -rotate -90 '+$
         'geos5_nh_sf_vs_pv_difflat_monthly.jpg'
;  spawn,'/usr/bin/rm geos5_nh_sf_vs_pv_difflat_monthly.ps'
endif
end
