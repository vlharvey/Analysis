;
; plot NH polar plot of MLS ozone and Met Office marker
;
@aura2date
@rd_ukmo_nc3
@stddat
...
ifile='                             '
lstmn=4 & lstdy=1 & lstyr=2014 & lstday=0
ledmn=6 & leddy=22 & ledyr=2014 & ledday=0
...
;
      ...
      restore,dirm+'o3_mls_'+sver+'_'+sdate+'.sav'              ; mix
...
;
; construct 2d MLS arrays to match O3
;
      mpress2=mpress
      mtime2=0.*mo3
      mlat2=0.*mo3
...
;
      if setplot eq 'ps' then begin
         ...
         device,/landscape,bits=8,$
                 filename='nh_polar_mls_o3+mark_'+lfile+'_'+stheta+'K.ps'
         device,/color
...
;
      ...
      a=findgen(8)*(2*!pi/8.)
      usersym,cos(a),sin(a),/fill
      index=where(mtheta gt rtheta-80. and mtheta le rtheta+80. and mlat2 gt 0.,npt)
...
;         kindex=where(abs(muttime-tplot) le 0.5 and mlat gt 0.,mprof)
          kindex=where(mlat gt 0.,mprof)
          o3swath=smooth(mo3(kindex,*),3,/edge_truncate)
          index=where(o3swath lt 0.)
          if index(0) ne -1L then o3swath(index)=0.1
;         if iswath eq 0L then begin
             o3amb=fltarr(mlev)	; mean ozone profile in NH ambient regions
             thamb=fltarr(mlev)	; mean th profile in NH ambient regions
;         endif

          thswath=mtheta(kindex,*)
          lonswath=mlon2(kindex,*)
          latswath=mlat2(kindex,*)
...
;
      ...
      a=findgen(8)*(2*!pi/8.)
      usersym,cos(a),sin(a),/fill
      index=where(thswath gt rtheta-80. and thswath le rtheta+80. and latswath gt 0.,npt)
      if index(0) eq -1L then goto,jump
      th0=string(FORMAT='(I4)',itheta)
      ...
      omin=-3.0
      omax=3.
      index=where(abs(o3mark) le 0.05 and ydata gt 25.,nn)
      o3mean=total(o3data(index))/float(nn)
print,'mean NH ambient ',o3mean
      o3pr=o3data-o3mean
      for i=0L,n_elements(o3data)-1L do begin
          if o3data(i) ne -99. then $
             oplot,[xdata(i),xdata(i)],[ydata(i),ydata(i)],psym=8,$
...
;
          ...
          o3pr=0.*o3swath
          o3prpcnt=0.*o3swath
          index=where(abs(o3swath) gt 100.)
...
;                print,'o3mean in NH ',total(THSWATH(xx,k))/nn,total(o3lev(xx))/float(nn)
...
print,'min o3 % ',min(o3prpcnt(index)),min(o3swath(index))
          ...
          index=where(thswath ge 600. and thswath le 1600. and $                ; altitude criteria
                      latswath gt 20. and $                                     ; latitude criteria
...
;
      ...
      omin=600.
      omax=1800.
      plot,xlop,ylop,/nodata,color=0,xrange=[0.,360.],yrange=[20.,90.],xtitle='Longitude',$
...
;
      ...
      omin=0.
      omax=360.
      plot,ylop,thlop,/nodata,color=0,xrange=[20.,90.],yrange=[500.,1800.],xtitle='Latitude',$
...
;
      ...
      if setplot eq 'ps' then begin
         device, /close
         spawn,'convert -trim nh_polar_mls_o3+mark_'+lfile+'_'+stheta+'K.ps -rotate -90 '+$
               ' nh_polar_mls_o3+mark_'+lfile+'_'+stheta+'K.jpg'
         spawn,'/usr/bin/rm nh_polar_mls_o3+mark_'+lfile+'_'+stheta+'K.ps'
      endif
      icount=icount+1L
goto,jump
end
