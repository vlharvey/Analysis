...
;
; min SF out of vortex and in vortex
;
          aindex=where(y2d gt 40. and mark eq 0.)
          if aindex(0) eq -1L then goto,jumplev
          amin=min(sf(aindex))
          vindex=where(y2d gt 40. and mark eq 1.)
...
index=where(y2d gt 0. and sf eq amin)
oplot,x2d(index),y2d(index),psym=8,symsize=2,color=mcolor*.9
index=where(y2d gt 0. and sf eq vmin)
...
sffreq(index)=100.*sffreq(index)/float(icount)	;float(nfreq(index))
;
...
save,file='geos5_sf_freq_minout_nh_'+syr+smn+'.sav',sffreq,sdates,sdate0,sdate1,th
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='geos5_nh_sf_'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_minout.ps'
   device,/color
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim geos5_nh_sf_'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_minout.ps -rotate -90 '+$
         'geos5_nh_sf__'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_minout.jpg'
   spawn,'/usr/bin/rm geos5_nh_sf_'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_minout.ps'
endif
end
