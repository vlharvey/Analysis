...
if setplot eq 'ps' then begin
   ...
   !p.thick=2
   device,font_size=9
   device,/landscape,bits=8,filename='mls_lops_mino3_xt+highs.ps'
   device,/color
...
;
saveit:
save,file='MLS_LOP_XT_mino3.sav',o3minnh,o3maxnh,o3minsh,o3maxsh,$
     fdoy_save,sdate_save,lonbin,nhighnh,nhighsh
;
; plot Hovmoller
;
plotit:
restore,'MLS_LOP_XT_mino3.sav'
...
contour,o3minnh,lonbin,fdoy_save-min(fdoy_save),levels=o3level,min_value=0.,/nodata,xrange=[0.,360.],$
        title='Arctic',/noeras,/cell_fill,c_color=col1,color=0,yticks=nxtick-1L,xticks=6,$
...
plots,xmin,fdoy_save(index0(0))-min(fdoy_save)
plots,xmax,fdoy_save(index0(0))-min(fdoy_save),/continue,/data,thick=10,color=mcolor*.1
plots,xmin,fdoy_save(index1(0))-min(fdoy_save)
plots,xmax,fdoy_save(index1(0))-min(fdoy_save),/continue,/data,thick=10,color=mcolor*.1
plots,xmin,fdoy_save(index0(0))-min(fdoy_save)
plots,xmin,fdoy_save(index1(0))-min(fdoy_save),/continue,/data,thick=10,color=mcolor*.1
plots,xmax,fdoy_save(index0(0))-min(fdoy_save)
plots,xmax,fdoy_save(index1(0))-min(fdoy_save),/continue,/data,thick=10,color=mcolor*.1
endif
...
contour,o3minsh,lonbin,fdoy_save-min(fdoy_save),levels=o3level,min_value=0.,xrange=[0.,360.],$
        title='Antarctic',/noeras,/cell_fill,c_color=col1,color=0,yticks=nxtick-1L,/nodata,xticks=6,$
        ytickv=xindex,ytickname=' '+strarr(n_elements(xindex)+1),xtitle='Longitude',charsize=1.5,$
        charthick=1.5
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim mls_lops_mino3_xt+highs.ps -rotate -90 mls_lops_mino3_xt+highs.jpg'
endif
