...
plot,sf(index),pv(index),xrange=[min(sfbin),max(sfbin)],yrange=[min(pvbin),max(pvbin)],$
     color=0,psym=1,/noeras,xtitle='SF',ytitle='PV'
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='geos5_nh_sf_pv_scatter_'+sdate+'.ps'
   device,/color
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim geos5_nh_sf_pv_scatter_'+sdate+'.ps -rotate -90 '+$
         'geos5_nh_sf_pv_scatter_'+sdate+'.jpg'
   spawn,'/usr/bin/rm geos5_nh_sf_pv_scatter_'+sdate+'.ps'
endif
