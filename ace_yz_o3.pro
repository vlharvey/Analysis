...
restore,dira+'o3orig_ace_v2.2.'+syear0
...
if setplot eq 'ps' then begin
   ...
   ysize=nydim/100.
   device,font_size=9
   device,/landscape,bits=8,filename='ace_yz_o3_'+sdate0+'-'+sdate1+'.ps'
   device,/color
...
if index(0) ne -1L then $
   o3array(index)=1.e6*o3array(index)/float(no3array(index))
...
contour,o3array,latbin,altitude,xrange=[-90.,90.],yrange=[1.,120.],xticks=6,$
        ...
        title='Zonal Mean ACE Ozone',c_color=col1,color=0,min_value=-99.
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ace_yz_o3_'+sdate0+'-'+sdate1+'.ps -rotate -90 ace_yz_o3_'+sdate0+'-'+sdate1+'.jpg'
endif
end
