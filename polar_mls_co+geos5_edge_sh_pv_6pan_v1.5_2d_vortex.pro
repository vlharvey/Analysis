...
;
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='polar_mls_co+geos5_edge_sh_pv_'+sdate+'_2d_v1.5_vortex.ps'
         device,/color
...
;
      ...
      ymx=yorig(2)+ylen
      set_viewport,xmn,xmx,ymn,ymx
      plot,codata,h2odata,psym=8,color=0,xrange=[comin,comax],yrange=[omin,omax],symsize=0.5,$
...
;
      ...
      if vavg_co_prof(ilev) ne -99. then begin
         ...
         plots,vavg_co_prof(ilev),ymax,color=.9*mcolor,/continue,thick=4
      endif
      if vavg_h2o_prof(ilev) ne -99. then begin
         plots,vavg_h2o_prof(ilev),0.
         plots,vavg_h2o_prof(ilev),ymax,color=.3*mcolor,/continue,thick=4
      endif
...
oplot,vavg_co_prof(index),th(index),color=0,thick=8
...
if index(0) ne -1L then begin
oplot,aavg_co_prof(index),th(index),color=100,thick=8
...
;
      ...
      ymx=yorig(5)+ylen
      set_viewport,xmn,xmx,ymn,ymx
      contour,h2opdf2d,xbins,th,levels=level,title='2D H!l2!nO PDF',/noerase,/fill,c_color=col1,$
              ytitle='Theta (K)',xtitle='H!l2!nO (ppmv)',color=0
;     contour,h2opdf2d,xbins,th,levels=level,/overplot,/follow,color=0,c_labels=0*level
index=where(vavg_h2o_prof gt 0.)
if index(0) ne -1L then begin
oplot,vavg_h2o_prof(index),th(index),color=0,thick=8
...
if index(0) ne -1L then begin
oplot,aavg_h2o_prof(index),th(index),color=100,thick=8
...
loadct,38
      ...
      if setplot eq 'ps' then begin
         device,/close
         spawn,'convert -trim polar_mls_co+geos5_edge_sh_pv_'+sdate+'_2d_v1.5_vortex.ps -rotate -90 '+$
               'polar_mls_co+geos5_edge_sh_pv_'+sdate+'_2d_v1.5_vortex.jpg'
         spawn,'/usr/bin/rm -f polar_mls_co+geos5_edge_sh_pv_'+sdate+'_2d_v1.5_vortex.ps'
      endif
      icount=icount+1L
goto,jump
end
