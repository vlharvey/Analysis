...
;
noy_conc=noygrd * (1./29.) * (1000./1.) * rho * 30.

noy_conc=noy_conc/1.e6                                 ; divide by 1.e6 for m-3 to cm-3 
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='Figures/polar_nh_noy_charm_'+sdate+'_'+slev+'_diff_dens.ps'
       device,/color
...
;   speed=sqrt(u^2+v^2)
    ...
    MAP_SET,90,0,-90,/ortho,/noeras,/grid,/contin,title='NOy',charsize=1.5,color=0,/noborder
...
;    contour,z,x,alat,/overplot,levels=10+findgen(150),/follow,c_labels=0*findgen(150),/noeras,color=mcolor
    ...
    contour,q,x,alat,/overplot,levels=[300.],/follow,c_labels=0,/noeras,color=mcolor,thick=3
...
if xlon gt 360. then xlon=xlon-360.
    ...
    !type=2^2+2^3+2^6
    plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],xtitle='(g/cm3)',charsize=1.,color=0
    ybox=[0,10,10,0,0]
    x1=imin
    ...
    set_viewport,xmn,xmx,ymn,ymx
    !type=2^2+2^3
    MAP_SET,90,0,-90,/ortho,/noeras,/grid,/contin,title='NOy Difference',charsize=1.5,color=0,/noborder
;imin=min(noyd(*,nr/2:nr-1))
;imax=max(noyd(*,nr/2:nr-1))
print,'NOy diff ',imin,imax
imin=-5.e-15
imax=5.e-15
    nlvls=21
    level=imin+((imax-imin)/float(nlvls))*findgen(nlvls+1)
...
;   drawvectors,nc+1,nr,x,alat,u,v,50,1
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim Figures/polar_nh_noy_charm_'+sdate+'_'+slev+'_diff_dens.ps -rotate -90 Figures/polar_nh_noy_charm_'+sdate+'_'+slev+'_diff_dens.jpg'
       spawn,'rm -f Figures/polar_nh_noy_charm_'+sdate+'_'+slev+'_diff_dens.ps'
    endif
