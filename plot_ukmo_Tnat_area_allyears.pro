...
restore,'ukmo_Tnat_area_allyears.sav'	;,area_ave,years,th,sfile
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='ukmo_tnat_area_allyears_'+sth+'K.ps'
   device,/color
...
plot,[1,kday,kday,1,1],[0.,0.,20.,20.,0.],min_value=0.,color=0,$
      xrange=[1,kday],yrange=[0.,20.],/nodata,charsize=2,$
      ytitle='Millions of km!u2!n',xtickname=[' ',' '],xticks=1,$
      title='N.H. Area where T<Tnat  '+sth+' K'
...
for i=0,nxtick-1 do begin
    ...
    xyouts,kindex(i)+1,-2.,xlab,/data,alignment=0.5,charsize=3,color=0
endfor
...
for iyear=0L,nyear-1L do begin
    ...
    xyouts,xmx+0.02,ymn+iyear*yinc,slab(iyear),charsize=2,color=col1(iyear),/normal,charthick=2
endfor
oplot,findgen(kday),area_ave,color=0,thick=8,min_value=-999.
xyouts,xmx+0.02,ymn+(nyear-1L)*yinc,slab(nyear-1),charsize=2,color=0,/normal,charthick=2

if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ukmo_tnat_area_allyears_'+sth+'K.ps -rotate -90 ukmo_tnat_area_allyears_'+sth+'K.jpg'
endif
