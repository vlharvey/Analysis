...
xnmon=15.+30.*findgen(n_elements(nmon))
;
; postscript file
;
if setplot eq 'ps' then begin
   ...
   !p.thick=2
   device,font_size=9
   device,/landscape,bits=8,filename='wa3_lops_depth_yt_mean.ps'
   device,/color
...
; set date bounds
;
idate=19900101L & edate=20040101L
...
;
; construct 15-year mean
;
thdepthnh_mean=fltarr(366,nlat)
nthdepthnh_mean=fltarr(366,nlat)
thdepthsh_mean=fltarr(366,nlat)
nthdepthsh_mean=fltarr(366,nlat)
fdoy_mean=fltarr(366)
sdate_mean=strarr(366)
for ii=0L,n_elements(LDATE_SAVE)-1L do begin
    mmdd=strmid(strcompress(ldate_save(ii),/remove_all),4,4)
    index=where(strmid(strcompress(ldate_save,/remove_all),4,4) eq mmdd,npt)
    if index(0) ne -1L then begin
       iyr = long(strmid(strcompress(ldate_save(ii),/remove_all),0,4))
       idy = long(strmid(strcompress(ldate_save(ii),/remove_all),6,2))
       imn = long(strmid(strcompress(ldate_save(ii),/remove_all),4,2))
;print,strcompress(ldate_save(ii),/remove_all),' ',iyr,imn,idy
       z = kgmt(imn,idy,iyr,iday)
       sdate_mean(iday-1)=strcompress(sdate_save(ii),/remove_all)
       fdoy_mean(iday-1)=iday
       for kk=0L,npt-1L do begin
           thdepthnh_mean(iday-1,*)=thdepthnh_mean(iday-1,*)+thdepthnh(index(kk),*)
           nthdepthnh_mean(iday-1,*)=nthdepthnh_mean(iday-1,*)+1.0
           thdepthsh_mean(iday-1,*)=thdepthsh_mean(iday-1,*)+thdepthsh(index(kk),*)
           nthdepthsh_mean(iday-1,*)=nthdepthsh_mean(iday-1,*)+1.0
       endfor
    endif
endfor
;
; remove blank days
;
index=where(fdoy_mean gt 0.,nday)
fdoy_mean=fdoy_mean(index)
sdate_mean=sdate_mean(index)
thdepthnh_mean=thdepthnh_mean(index,*)
nthdepthnh_mean=nthdepthnh_mean(index,*)
thdepthsh_mean=thdepthsh_mean(index,*)
nthdepthsh_mean=nthdepthsh_mean(index,*)
;
; average
;
index=where(nthdepthnh_mean gt 0.)
if index(0) ne -1 then thdepthnh_mean(index)=thdepthnh_mean(index)/nthdepthnh_mean(index)
index=where(nthdepthsh_mean gt 0.)
if index(0) ne -1 then thdepthsh_mean(index)=thdepthsh_mean(index)/nthdepthsh_mean(index)
;
; smooth
;
;thdepthnh_mean=smooth(thdepthnh_mean,5,/edge_truncate)
;thdepthsh_mean=smooth(thdepthsh_mean,5,/edge_truncate)

nxtick=n_elements(nmon)
erase
...
contour,thdepthnh_mean,fdoy_mean,latbin,levels=level,min_value=-9999.,yrange=[30.,85.],$
        title='WACCM 3 Arctic',/noeras,/cell_fill,c_color=col1,color=0,xticks=nxtick-1L,/nodata,$
        xtickv=xnmon,xtickname=nmon,ytitle='Latitude',charsize=1.5
loadct,0
;
...
nlvls=n_elements(level)
col1=1+indgen(nlvls)*mcolor/nlvls
contour,thdepthnh_mean,fdoy_mean,latbin,levels=level,min_value=-9999.,/overplot,$
        /noeras,/cell_fill,c_color=col1,color=0,charsize=1.5
contour,thdepthnh_mean,fdoy_mean,latbin,levels=level,min_value=-9999.,/overplot,$
        /follow,color=0L,c_labels=0*level,thick=1

set_viewport,.2,.8,.2,.5
contour,thdepthsh_mean,fdoy_mean,latbin,levels=level,min_value=-9999.,yrange=[-85.,-30.],$
...
nlvls=n_elements(level)
col1=1+indgen(nlvls)*mcolor/nlvls
contour,thdepthsh_mean,fdoy_mean,latbin,levels=level,min_value=-9999.,/overplot,$
        /noeras,/cell_fill,c_color=col1,color=0,charsize=1.5
contour,thdepthsh_mean,fdoy_mean,latbin,levels=level,min_value=-9999.,/overplot,$
        /follow,color=0L,c_labels=0*level,thick=1
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim wa3_lops_depth_yt_mean.ps -rotate -90 wa3_lops_depth_yt_mean.jpg'
endif
