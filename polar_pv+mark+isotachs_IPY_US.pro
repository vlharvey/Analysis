...
lstmn=12L & lstdy=1L & lstyr=11L
ledmn=2L & leddy=25L & ledyr=12L
lstday=0L & ledday=0L
...
;        !p.font=0
         ...
         device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
                /bold,/color,bits_per_pixel=8,/times,filename='Collins/'+sdate+'_PV_US.ps'
         !p.charsize=1.25
         !p.thick=2
         !p.charthick=5
         !p.charthick=5
...
usersym,cos(a),sin(a)
    ...
    xyouts,.65,.12,'K'+ue+'hlungsborn, Germany',charsize=1.5,/normal,color=mcolor*.475,charthick=7
      ...
      plot,[imin,imax],[0,0],yrange=[0,10],$
            xrange=[imin,imax],xtitle=stheta+' K (50 km, 1 hPa) Potential Vorticity',/noeras,$
      ...
      if setplot eq 'ps' then begin
         device, /close
         spawn,'convert -trim Collins/'+sdate+'_PV_US.ps -rotate -90 Collins/'+sdate+'_PV_US.png'
         spawn,'/usr/bin/rm Collins/'+sdate+'_PV_US.ps'
      endif
goto,jump
end
