;
; plot user specified time average zonal mean MLS temperature and differences
;
@stddat
@kgmt
@ckday
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='yz_mls_timeavg_diff_'+daterange+'.ps'
   device,/color
...
if index(0) ne -1L then tbar0(index)=-9999.
contour,tbar0,alat,altitude,/noeras,xrange=[-90.,90.],yrange=[0.,100.],charsize=1.5,color=0,$
      ...
      xtitle='Latitude',title='2009',charthick=2
index=where(level gt 0.)
contour,tbar0,alat,altitude,levels=level(index),color=0,/follow,/overplot,min_value=-9999.,c_labels=0*level(index)
contour,tbar0,alat,altitude,levels=[150.],color=mcolor,/follow,/overplot,min_value=-9999.,c_labels=0*level(index)
contour,tbar0,alat,altitude,levels=[280.],color=0,thick=3,/follow,/overplot,min_value=-9999.,c_labels=0*level(index)
imin=min(level)
imax=max(level)
...
contour,tbar,alat,altitude,/noeras,xrange=[-90.,90.],yrange=[0.,100.],charsize=1.5,color=0,$
      ...
      xtitle='Latitude',title='2010',charthick=2
index=where(level gt 0.)
...
col2=1+indgen(11)
pdiff=-99.+0.*tbar0
index=where(tbar0 gt 0. and tbar gt 0.)
if index(0) eq -1L then stop
pdiff(index)=tbar0(index)-tbar(index)
...
contour,pdiff,alat,altitude,xrange=[-90.,90.],yrange=[0.,100.],$
        ...
        title='dT',c_color=col2,color=0,charthick=2
index=where(level gt 0.)
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim yz_mls_timeavg_diff_'+daterange+'.ps -rotate -90 yz_mls_timeavg_diff_'+daterange+'.jpg'
;  spawn,'/usr/bin/rm -f yz_mls_timeavg_diff_'+daterange+'.ps'
endif
end
