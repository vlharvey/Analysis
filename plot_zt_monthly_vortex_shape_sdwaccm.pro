...
for imon=0L,nmon-1L do begin
    if smon(imon) gt 4 then begin
       restore,'vortex_shape_sdwaccm_'+syear(iyear)+smon(imon)+'.sav
       print,syear(iyear),smon(imon),nfile
    endif
    if smon(imon) lt 5 then begin
       restore,'vortex_shape_sdwaccm_'+syear(iyear+1)+smon(imon)+'.sav
...
save,file='zt_vortex_shape_sdwaccm_'+yearlab+'.sav',ALTITUDE_tot,AREA1_tot,clat_tot,ellip_tot,nvort_tot,sdate_tot,th
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='zt_vortex_shape_sdwaccm_'+yearlab+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle=yearlab+' SDW Area (%)',/noeras,charsize=1.5,charthick=2
ybox=[0,10,10,0,0]
x1=imin
...
set_viewport,xmn,xmx,ymnb,ymxb
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle=yearlab+' # Vortex Lobes',/noeras,charsize=1.5,charthick=2
ybox=[0,10,10,0,0]
x1=imin
dx=(imax-imin)/float(nlvls)
for jj=0,nlvls-1 do begin
...
x1=x1+dx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim zt_vortex_shape_sdwaccm_'+yearlab+'.ps -rotate -90 zt_vortex_shape_sdwaccm_'+yearlab+'.jpg'
    endif
