...
ifile='                             '
dates=['20030109','20030112','20030114','20030116','20030119',$
        '20030121','20030124','20030126','20030129','20030131',$
        '20030202','20030204','20030206']
ifiles=[$
'jan_09_03',$
'jan_12_03',$
'jan_14_03',$
...
for n=0,nfile-1 do begin
    if n eq 0 then ifile0=ifiles(n)
    ifile=ifiles(n)
    iflag=0
...
;
    ...
    dx2=(xlon(2)-xlon(0))*!pi/180.
    dy2=(xlat(2)-xlat(0))*!pi/180.
    dy1=(xlat(2)-xlat(1))*!pi/180.
    for k=0,nth-1 do begin
        pv1=transpose(pv2(*,*,k))
...
;
        for j = 0, nr-1 do begin
            ...
            for i = 0, nc-1 do begin
                ...
                if j eq 0 or j eq nr -1 then dqdy = (pv1(i,jp1)-pv1(i,jm1))/dy1
...
;       contour,pvgrad,x,alat,nlevels=40,thick=2,$
;               color=mcolor,/overplot,/noeras,/follow,c_labels=0+intarr(30)
;       MAP_SET,90,0,-90,/stereo,/noeras,title=mtitle,color=lc,/contin,/grid
        x2d=fltarr(nc+1,nr)
        y2d=fltarr(nc+1,nr)
        ...
        for i=0,nc-1 do begin
;
; look poleward of 40 N and 2 points away from the pole
;
...
;                 [lattmp(index(0)),lattmp(index(0))],psym=8,color=mcolor
...
;               color=0,/overplot,/noeras,/follow
;       contour,pv,x,alat,levels=0.00025,thick=4,c_labels=[0],$
;               color=mcolor,/overplot,/noeras,/follow
;       print,th(k),' MPV=',mpvmed
    endfor
...
;   write_ukmo_subvortex,'Datfiles/'+ifile+'_solve2.nc3.subvortex',$
...
;
    ...
    for k=0,nth-1 do begin
        ...
        qdfyz(0:nr/2-1,k)=qdf2(nr/2:nr-1,ilon1,k)
        qdfyz(nr/2:nr-1,k)=reverse(qdf2(nr/2:nr-1,ilon2,k))
...
;  oplot,x2d(index),y2d(index),psym=8,symsize=0.5
    result=moment(mpv0)
...
;   contour,mpvyz,alat,th,levels=result(0),color=mcolor,/overplot,thick=4
    ...
    contour,tyz,alat,th,levels=tlevel,/follow,c_color=0,/overplot,$
            c_labels=1+0*tlevel,c_linestyle=tlevel lt 0,thick=2
;   contour,qdfyz,alat,th,levels=[0],/overplot,/follow,c_color=mcolor,thick=2
;   contour,markyz,alat,th,levels=[0.1],/noeras,/overplot,$
;           color=mcolor,/follow,thick=3,c_labels=[0]
    oplot,alat,theta+0.*alat,color=icolmax,linestyle=2
    imin=min(mpvlevel)
...
; Close PostScript file and return control to X-windows
    ...
    if setplot eq 'x' then begin
       ...
       write_gif,'ukmo_'+ifile+'_'+sth+'.gif',img
    endif
    stop
    jump:
