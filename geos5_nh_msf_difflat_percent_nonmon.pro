...
lstday=0L & ledday=0L
close,1
openw,1,'geos5_nh_msf_difflat_percent_nonmon.dat'
;
; get date range
;
...
;
      ...
      if ndays gt ledday then close,1
;
...
index=where(th ge 3000.)
if mean(sfdiff(index)) ge 5. then printf,1,sdate,mean(sfdiff(index))
if mean(sfdiff(index)) ge 5. then print,sdate,mean(sfdiff(index))
