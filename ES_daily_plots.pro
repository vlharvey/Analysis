...
restore, '/Volumes/MacD68-1/france/WACCM_paper/Post_process/Figure_3_pre_process_composit_ES.sav'
...
ESdate = fltarr(n_elements(dayzeros)*61L)
inday = 0L
...
title = 'Day '+ strtrim(strcompress(string(format='(I3.2)',ndays - 30L)),2)+', ES event ' + strtrim(strcompress(string(format='(I2.2)',nnday + 1L)),2)
...
;-----------------------------------------------------------------------
   ...
   if setplot eq 'ps' then begin
	...
	'pnmtopng idl001.ppm > /Volumes/MacD68-1/france/WACCM_paper/Figures/ES_daily/ES_Day_'+$
	strtrim(strcompress(string(format='(I3.2)',ndays - 30L)),2)+'_ES_event_' + strtrim(strcompress(string(format='(I2.2)',nnday + 1L)),2)+'.png'
...
save, filename = '/Volumes/MacD68-1/france/WACCM_paper/Post_process/WACCM_ES_daily_max_T_Z.sav', $
