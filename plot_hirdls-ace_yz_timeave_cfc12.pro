...
hirdls_CFC11YZ=reform(CFC11YZ(*,x))
hirdls_CFC12YZ=reform(CFC12YZ(*,x))
...
diff_cfc11YZ=0.*cfc11YZ
x=where(HIRDLS_cfc11YZ ne -99. and ACE_cfc11YZ ne -99.)
if x(0) ne -1L then diff_cfc11YZ(x)=100.*(HIRDLS_cfc11YZ(x)-ace_cfc11YZ(x))/ace_cfc11YZ(x)
diff_cfc12YZ=0.*cfc12YZ
x=where(HIRDLS_cfc12YZ ne -99. and ACE_cfc12YZ ne -99.)
if x(0) ne -1L then diff_cfc12YZ(x)=100.*(HIRDLS_cfc12YZ(x)-ace_cfc12YZ(x))/ace_cfc12YZ(x)
diff_O3YZ=0.*O3YZ
...
if setplot eq 'ps' then begin
   ...
   ysize=nydim/100.
   device,font_size=9
   device,/landscape,bits=8,filename='hirdls-ace_yz_'+adate0+'-'+adate1+'_cfc12.ps'
   device,/color
   device,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize
endif

erase
hdata=HIRDLS_cfc12YZ*1.e9
adata=ace_cfc12YZ*1.e9
ddata=diff_cfc12YZ
index=where(hdata le 0.)
if index(0) ne -1L then hdata(index)=-99999.
index=where(adata le 0.)
if index(0) ne -1L then adata(index)=-99999.
index=where(ddata eq 0.)
if index(0) ne -1L then ddata(index)=-99999.
level=0.02*findgen(31)
...
;
    ...
    xlabs=' '+strarr(7)
    contour,hdata,latbin,altitude,xrange=[-90.,90.],yrange=[0.,40.],xticks=6,ytitle='Altitude (km)',$
            xtickname=xlabs,charsize=2.5,levels=level,/cell_fill,$
            title='HIRDLS CFC-12 '+hdate0+'-'+hdate1,c_color=col1,color=0,min_value=-99999.
...
;   contour,ddata,latbin,altitude,/overplot,levels=[0],color=mcolor,/follow,min_value=-99999.,thick=3
    ...
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*mcolor/nlvls
    contour,adata,latbin,altitude,xrange=[-90.,90.],yrange=[0.,40.],xticks=6,ytitle='Altitude (km)',$
            xtickname=xlabs,charsize=2.5,levels=level,/cell_fill,$
            title='ACE CFC-12 '+adate0+'-'+adate1,c_color=col1,color=0,min_value=-99999.
...
if x(0) ne -1L then ddata(x)=-99999.
    ...
    contour,ddata,latbin,altitude,xrange=[-90.,90.],yrange=[0.,40.],xticks=6,$
            xtickname=xlabs,charsize=2.5,levels=dlevel,/cell_fill,ytitle='Altitude (km)',$
            title='HIRDLS-ACE CFC-12 Difference (%)',c_color=col1,color=0,min_value=-99999.,xtitle='Latitude'
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim hirdls-ace_yz_'+adate0+'-'+adate1+'_cfc12.ps -rotate -90 hirdls-ace_yz_'+adate0+'-'+adate1+'_cfc12.jpg'
endif
end
