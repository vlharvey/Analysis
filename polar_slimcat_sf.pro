;
; plot SLIMCAT diabatic heating rates in mercator projection
;
@stddat
@kgmt
@ckday
...
dirh='/aura3/data/SLIMCAT_data/Datfiles/SLIMCAT_'
...
;
; get SLIMCAT filenames (a file is expected every 5 days)
;
spawn,'ls -1 '+dirh+'*_th.nc2',ifiles
nday=n_elements(ifiles)
for n=0L,nday-1L do begin    
;
; read SLIMCAT
;
    syear=strmid(ifiles(n),42,4)
    uyear=strmid(ifiles(n),44,2)
    smon=strmid(ifiles(n),46,2)
    sday=strmid(ifiles(n),48,2)
    rd_slimcat_nc2,ifiles(n),nc,nr,nth,alon,alat,thlev,ipv,prs,msf,u,v,q,qdf,vp,sf,iflg
...
level=pvmin+pvint*findgen(nlvls)
    ...
    map_set,90,0,0,/stereo,/contin,/grid,title=syear+smon+sday+' SLIMCAT Isotachs '+spress,$
            charsize=2,/noeras
