...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='Figures/polar_nh_co_charm_'+sdate+'_'+slev+'_diff.ps'
       device,/color
...
if xlon gt 360. then xlon=xlon-360.
    ...
    set_viewport,xmn,xmx,ymn,ymx
    !type=2^2+2^3
    MAP_SET,90,0,-90,/ortho,/noeras,/grid,/contin,title='CO Difference',charsize=1.5,color=0,/noborder
imin=-1.
imax=1.
    nlvls=21
...
;   drawvectors,nc+1,nr,x,alat,u,v,50,1
    ...
    if setplot ne 'ps' then wait,1	;stop
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim Figures/polar_nh_co_charm_'+sdate+'_'+slev+'_diff.ps -rotate -90 Figures/polar_nh_co_charm_'+sdate+'_'+slev+'_diff.jpg'
       spawn,'rm -f Figures/polar_nh_co_charm_'+sdate+'_'+slev+'_diff.ps'
    endif
