;
; GM perspective
; Collins 2011 proposal version has 4 lidar sites superimposed
;
...
; select theta levels to plot
      index=where(th ge 450. and th le 2000.,nth2)
...
;        !p.font=0
         device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
                /bold,/color,bits_per_pixel=8,/times,filename='Collins/'+sdate+'_3D_GM.ps'
         !p.charsize=1.25
         !p.thick=2
         !p.charthick=5
         !p.charthick=5
...
;
 	...
 	if lindex(0) ne -1 then begin
            ...
            contour,temp,xcn,ycn,levels=[180.,185.,190.,195.],color=mcolor,$
                    /T3D,zvalue=nz(kk),thick=3,max_value=1.e15
            contour,dum,xcn,ycn,levels=[0.1],color=0,$
                    c_labels=0,/T3D,zvalue=nz(kk),thick=10,max_value=1.e15
         endif
;
; anticyclones
;
...
yy=69.3 & xx=16.
               ...
               a=findgen(8)*(2*!pi/8.)
               usersym,2*cos(a),2*sin(a),/fill
               oplot,[xn,xn],[yn,yn],zvalue=nz(kk),/T3D,psym=8,color=mcolor*.8
               dist=nz(kp1)-nz(kk)
               if dist gt 0. then begin
               for m=0,10 do $
                   oplot,[xn,xn],[yn,yn],zvalue=nz(kk)+float(m)*dist/11.,/T3D,psym=8,color=mcolor*.8
               endif
;
...
loadct,39
        ...
        xyouts,.05,nz3,savgz+' km',color=0,/normal,charsize=2,charthick=5
;       xyouts,.08,nz4,thlevs(kk),color=0,/normal,charsize=2,charthick=5
        endif
    endfor	; loop over stacked polar plots
    !psym=0
    xyouts,0.3,0.875,date,/normal,charsize=3.0,color=0,charthick=5
;    xyouts,.08,.78,'Theta (K)',charsize=2,/normal,color=0,charthick=2
    xyouts,.03,.82,'Altitude (km)',charsize=2,/normal,color=0,charthick=5
;
; box behind names
;
loadct,0
    set_viewport,.625,.05,.925,.2
...
loadct,39
    xyouts,.65,.17,'Andoya, Norway',charsize=1.5,/normal,color=mcolor*.8,charthick=7
    xyouts,.65,.14,'Chatanika, Alaska',charsize=1.5,/normal,color=mcolor*.95,charthick=7
    xyouts,.65,.11,'Kangarlussuaq, Greenland',charsize=1.5,/normal,color=mcolor*.1,charthick=7
ue = string("374B)
loadct,38
    xyouts,.65,.08,'K'+ue+'hlungsborn, Germany',charsize=1.5,/normal,color=mcolor*.35,charthick=7
loadct,39
    plots,0.18,0.18,/normal
    plots,0.18,0.78,/continue,color=0,thick=5,/normal
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim Collins/'+sdate+'_3D_GM.ps -rotate -90 Collins/'+sdate+'_3D_GM.png'
       spawn,'/usr/bin/rm -f Collins/'+sdate+'_3D_GM.ps'
    endif
goto, jump
end
