...
!noeras=1
dir='/aura3/data/WACCM_data/Datfiles/wa3_tnv3_'
...
; --- Test for end condition and close windows.
      ...
      rd_waccm3_nc3,dir+ifile,nc,nr,nth,alon,alat,th,pv2,p2,$
         u2,v2,qdf2,mark2,sf2,o32,ch42,no2,h2o,iflag
...
;
      ...
      x(nc)=alon(0)+360.
mark2l=smooth(mark2,3,/edge_truncate)
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='Figures/'+ifile+'_sh_5000K.ps'
       device,/color
...
;   irot=0.
    ...
    for kk=0,nth2-1 do begin
        ...
        nz3=(kk+4.)*(1./(nth2+8.))
        nz4=(kk+8.)*(1./(nth2+16.))
        mark1h=transpose(mark2(*,*,lev))
        mark1l=transpose(mark2l(*,*,lev))
...
;
        ...
        col1(kk)=nz2*icolmax
        dum=fltarr(nc+1,nr2)
        dum(0:nc-1,0:nr2-1)=mark1l(0:nc-1,0:nr2-1)    ; SH
        dum(nc,*)=dum(0,*)
;
...
;
        ...
        lindex=where(dum gt 0.8,nl)
        imin=170.
        imax=310.
	if lindex(0) ne -1 then begin
print,th(lev),max(y2d(lindex))
;if max(y2d(lindex)) gt -30. then stop
...
;                  /T3D,zvalue=nz,thick=3,max_value=1.e15
           contour,dum,xcs,ycs,levels=[0.8],color=0,$
...
dx=x2d(1,0)-x2d(0,0)
           for ihigh=0,nhigh-1 do begin
               ...
               if max(y2d(index)) ge -40 then goto,jump1
...
;              index=where(tmp ne 9999. and y2d lt -13.7500 and dum eq -1.0*(ihigh+1))
               index=where(tmp ne 9999. and y2d lt 0. and dum eq -1.0*(ihigh+1))
               if index(0) eq -1 then goto,jump1
...
jumplev:
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim Figures/'+ifile+'_sh_5000K.ps -rotate -90 Figures/'+ifile+'_sh_5000K.jpg'
       spawn,'/usr/bin/rm Figures/'+ifile+'_sh_5000K.ps'
    endif
