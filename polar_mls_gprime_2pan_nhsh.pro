;
; polar plot of MLS GPH and anomaly in NH and SH
;
@stddat
@kgmt
@ckday
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='polar_mls_gprime_'+sdate+'_'+salt+'_nhsh.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
lon2(0:nc-1)=lon
lon2(nc)=lon2(0)+360.
;tlevel=82+.1*findgen(31)	; 0.004 hPa
;tlevel=63+.25*findgen(21)	; 0.1 hPa
tlevel=35.5+.1*findgen(21)	; 5 hPa
...
contour,gp2,lon2,lat,/noeras,/overplot,charsize=1.5,/cell_fill,c_color=col1,levels=tlevel
MAP_SET,-90,0,0,/ortho,/noeras,/grid,/contin,/noborder,color=mcolor
contour,smooth(gprime2,3,/edge_truncate),lon2,lat,levels=-2+0.2*findgen(10),color=mcolor,/follow,/overplot,c_linestyle=5,thick=3
contour,smooth(gprime2,3,/edge_truncate),lon2,lat,levels=0.2+0.2*findgen(10),color=0,/follow,/overplot,thick=3
...
a=findgen(8)*(2*!pi/8.)
usersym,cos(a),sin(a)
oplot,[360.-68.1333,360.-68.1333],[-67.5667,-67.5667],psym=8,symsize=3,color=0          ; Rothera
a=findgen(8)*(2*!pi/8.)
usersym,cos(a),sin(a),/fill
loadct,39
imin=min(tlevel)
...
!type=2^2+2^3+2^6
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='MLS GPH (km)'
ybox=[0,10,10,0,0]
x1=imin
...
;tlevel=80+.1*findgen(31)	; 0.004 hPa
;tlevel=59.+.25*findgen(21)	; 0.1 hPa
tlevel=32.+.25*findgen(21)	; 5 hPa
nlvls=n_elements(tlevel)
col1=1+(indgen(nlvls)/float(nlvls))*mcolor
xmn=xorig(0)
xmx=xorig(0)+xlen
...
a=findgen(8)*(2*!pi/8.)
usersym,cos(a),sin(a)
oplot,[26.5833,26.5833],[67.4167,67.4167],psym=8,symsize=3,color=0          ; Sodankylä
a=findgen(8)*(2*!pi/8.)
usersym,cos(a),sin(a),/fill
loadct,39
imin=min(tlevel)
...
x1=x1+dxx
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim polar_mls_gprime_'+sdate+'_'+salt+'_nhsh.ps -rotate -90 polar_mls_gprime_'+sdate+'_'+salt+'_nhsh.png'
    endif
