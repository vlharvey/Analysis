;
; MLS monthly mean temperature polar projections 
;
sver='v3.3'
a=findgen(8)*(2*!pi/8.)
...
for iprof=0L,mprof-1L do begin
    if mtemplev(iprof) ne -9.90000e+07 then begin
...
; GM
              ...
              h2omean(0,j)=h2omean(0,j)+mtemplev(iprof)
              ...
              for i=1L,nlon-1L do begin
                  if mlon(iprof) ge lonbin(i)-dx/2. and mlon(iprof) lt lonbin(i)+dx/2. then begin
                     h2omean(i,j)=h2omean(i,j)+mtemplev(iprof)
...
skipmls:
    ...
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='polar_nh_mls_temp_'+syyyymm+'.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
...
h2omean1(*,0)=0./0.
    ...
    !type=2^2+2^3+2^6
    plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],/noeras,color=0,charsize=1,xtitle='83 km Temperature (K)'
    ybox=[0,10,10,0,0]
    x2=imin
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim polar_nh_mls_temp_'+syyyymm+'.ps -rotate -90 '+$
                            'polar_nh_mls_temp_'+syyyymm+'.jpg'
;       spawn,'rm -f polar_nh_mls_temp_'+syyyymm+'.ps'
     endif
