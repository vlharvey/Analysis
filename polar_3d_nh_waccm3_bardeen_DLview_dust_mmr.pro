;
; Arctic vortex colored by dust mass mixing ratio (dust_mmr)
;
@stddat
@kgmt
@ckday
@kdate
@rd_waccm3_nc3_bardeen_dust
...
!noeras=1
dir='/aura3/data/WACCM_data/Datfiles/h0.lynn+dust.'
...
; --- Test for end condition and close windows.
      ...
      ifile=dir+sday+'_00Z.nc3'
      rd_waccm3_nc3_bardeen_dust,ifile,nc,nr,nth,alon,alat,th,pv2,p2,$
         u2,v2,qdf2,mark2,vp,sf2,q2,dust_ad,dust_md,dust_mmr,dust_nd,dust_re,iflag
      if iflag ne 0L then goto,jump
dust_mmr=dust_mmr*1.e9
tmp2=0.*p2
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='Figures/'+sday+'_nh_cb+dust_mmr.ps'
       device,/color
...
print,th(lev),min(temp1),max(temp1)
        ...
        temp(nc,*)=temp(0,*)
        dust=fltarr(nc+1,nr2)
        dust(0:nc-1,0:nr2-1)=dust1(0:nc-1,nr/2:nr-1)    ; NH
        dust(nc,*)=dust(0,*)
	...
	if lindex(0) ne -1 then begin
           for ii=0,nl-1 do begin
               oplot,[xcn(lindex(ii)),xcn(lindex(ii))],$
                     ...
                     color=1+((dust(lindex(ii))-imin)/(imax-imin))*icolmax
           endfor
...
jumplev:
    ...
    !psym=0
    xyouts,0.375,0.88,'Day '+sday,/normal,charsize=3.0,color=0,charthick=2
    ...
    !type=2^2+2^3+2^6
    iint=(imax-imin)/10.
    level=imin+iint*findgen(11)
    plot,[imin,imax],[0,0],yrange=[0,10],$
          xrange=[imin,imax],xtitle='Dust Mass Mixing Ratio',/noeras,$
          xtickname=strcompress(string(format='(f4.1)',level),/remove_all),$
          xstyle=1,xticks=10,charsize=1.5,color=0,charthick=2
    ybox=[0,10,10,0,0]
    x1=imin
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim Figures/'+sday+'_nh_cb+dust_mmr.ps -rotate -90 Figures/'+sday+'_nh_cb+dust_mmr.jpg'
       spawn,'/usr/bin/rm Figures/'+sday+'_nh_cb+dust_mmr.ps'
    endif
