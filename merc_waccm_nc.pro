...
;
;***Read WACCM data
      file=dir+syr+smn+sdy+'.nc'
      rd_waccm_nc,file,nc,nr,nl,alon,alat,th,pv2,p2,u2,v2,qdf2,o3,ch4,noy,iflg
...
; Mercator projection
      ...
      !type=2^2+2^3
      xyouts,.3,.9,'WACCM '+mb+' '+date,/normal,charsize=2
      ...
      MAP_SET,0,0,0,/noeras,/noborder,title='Methane',charsize=2
      contour,methane,alon,alat,levels=level,/overplot,/cell_fill,c_color=col1,/noeras
      contour,methane,alon,alat,levels=level,/overplot,c_color=[0],$
              c_labels=0*level,c_linestyle=level lt 0,/noeras
      MAP_SET,0,0,0,/GRID,/CONTIN,/noeras,/noborder
      imin=min(level)
      imax=max(level)
      ...
      omin=min(methane)
      omax=max(methane)
      plot,edat,methane,psym=3,xrange=[minval,maxval],yrange=[omin,omax],$
           xtitle='Potential Vorticity',ytitle=mb+' Methane',title='PV/CH4 Scatterplot'
