; 
; polar plots of WACCM QSUM in support of CHARM.
; difference plots between 30 minute time steps
;
@stddat
@kgmt
@ckday
...
;             if result.name eq 'CH4' then ch4grd=data
              ...
              if result.name eq 'QSUM' then qsumgrd=data
...
for ilev=0L,0L do begin
;for ilev=9L,9L do begin
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='Figures/polar_nh_qsum_charm_'+sdate+'_'+slev+'_diff.ps'
       device,/color
...
;   speed=sqrt(u^2+v^2)
    ...
    MAP_SET,90,0,-90,/ortho,/noeras,/grid,/contin,title='Total Ion Production',charsize=1.5,color=0,/noborder
...
;    contour,z,x,alat,/overplot,levels=10+findgen(150),/follow,c_labels=0*findgen(150),/noeras,color=mcolor
    MAP_SET,90,0,-90,/ortho,/noeras,/grid,/contin,charsize=1.5,color=mcolor,/noborder
...
;   oplot,xlon+0.*findgen(91),findgen(91),color=mcolor,psym=0,thick=3
    ...
    !type=2^2+2^3+2^6
    plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],xtitle='(s-1)',color=0
    ybox=[0,10,10,0,0]
    x1=imin
    ...
    set_viewport,xmn,xmx,ymn,ymx
    !type=2^2+2^3
    MAP_SET,90,0,-90,/ortho,/noeras,/grid,/contin,title='Half Hour Difference',charsize=1.5,color=0,/noborder
nlvls=21
imin=-1500.
imax=1500.
level=imin+150.*findgen(nlvls)	; 140 km
;imin=-100.
;imax=100.
;level=imin+10.*findgen(nlvls)	; 95 km
...
contour,noyd,x,alat,/overplot,levels=[100.,500.,1000.],/follow,c_labels=[0,0,0],/noeras,color=0
...
contour,noyd,x,alat,/overplot,levels=[-1000.,-500.,-100.],/follow,c_labels=[0,0,0],/noeras,color=mcolor,c_linestyle=5
;    contour,z,x,alat,/overplot,levels=10+findgen(150),/follow,c_labels=0*findgen(150),/noeras,color=mcolor
...
;   if setplot ne 'ps' then wait,0.5
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim Figures/polar_nh_qsum_charm_'+sdate+'_'+slev+'_diff.ps -rotate -90 Figures/polar_nh_qsum_charm_'+sdate+'_'+slev+'_diff.jpg'
       spawn,'rm -f Figures/polar_nh_qsum_charm_'+sdate+'_'+slev+'_diff.ps'
    endif
