...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
           /bold,/color,bits_per_pixel=8,/helvetica,filename='figure_1_tbar_mls.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
contour,jja_tyz,alat,altitude,/noera,/cell_fill,color=0,c_color=col1,levels=tlevel,xrange=[-90,90],yrange=[30,90],ytitle='Altitude (km)',charsize=1.5,charthick=2,title='JJA',xticks=6
...
ymx=yorig(1)+ylen
set_viewport,xmn,xmx,ymn,ymx
contour,djf_tyz,alat,altitude,/noera,/cell_fill,color=0,c_color=col1,levels=tlevel,xrange=[-90,90],yrange=[30,90],charsize=1.5,charthick=2,title='DJF',xticks=6
...
;
mls_djf_t=fltarr(n_elements(waccm_latitude),nz)
mls_jja_t=fltarr(n_elements(waccm_latitude),nz)
for j=0L,nr-1L do begin
    tprof=reform(tbar_djf(j,*))
    zprof=reform(zbar_djf(j,*))
    mls_djf_t(j,*)=interpol(tprof,zprof,altitude,/Nan)
index=where(altitude lt min(zprof) or altitude gt max(zprof))
if index(0) ne -1L then mls_djf_t(j,index)=0./0.		; do not allow extrapolation

    tprof=reform(tbar_jja(j,*))
    zprof=reform(zbar_jja(j,*))
    index=where(finite(tprof) eq 0)
    if index(0) ne -1L then tprof(index)=0.
    mls_jja_t(j,*)=interpol(tprof,zprof,altitude,/Nan)
index=where(altitude lt min(zprof) or altitude gt max(zprof))
if index(0) ne -1L then mls_jja_t(j,index)=0./0.
endfor
...
ymx=yorig(2)+ylen
set_viewport,xmn,xmx,ymn,ymx
contour,mls_jja_t,alat,altitude,levels=tlevel,/cell_fill,c_color=col1,/noeras,xticks=6,ytitle='Altitude (km)',color=0,yrange=[30,90],charsize=1.5,charthick=2
contour,mls_jja_t,alat,altitude,levels=tlevel,/foll,c_color=0,/noeras,/overplot,c_labels=0*tlevel
...
ymx=yorig(3)+ylen
set_viewport,xmn,xmx,ymn,ymx
contour,mls_djf_t,alat,altitude,levels=tlevel,/cell_fill,c_color=col1,/noeras,xticks=6,color=0,yrange=[30,90],charsize=1.5,charthick=2
contour,mls_djf_t,alat,altitude,levels=tlevel,/foll,c_color=0,/noeras,/overplot,c_labels=0*tlevel
...
jja_diff=jja_tyz-mls_jja_t
index=where(mls_jja_t eq 0.)
...
djf_diff=djf_tyz-mls_djf_t
index=where(mls_djf_t eq 0.)
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim figure_1_tbar_mls.ps -rotate -90 figure_1_tbar_mls.jpg'
;  spawn,'rm -f figure_1_tbar_mls.ps'
endif
