;
; save MERRA vortex area as a function of altitude and time about SSW days zeros
...
for issw = 0L, nevents - 1L do begin
    sevent=strtrim(strcompress(string(format='(I3.2)',issw+1)),2)
;
    sswdate0=sswdates(issw)
...
; save file
;
save,file='vortex_area_ssw_'+sswdate0+'_merra.sav',area_zt_nc4,th,sfile,y1,y2,koff
index=where(area_zt_nc4 le 0.)
if index(0) ne -1L then area_zt_nc4(index)=0./0.

plotit:
;restore,'/Users/harvey/Harvey_etal_2014/Post_process/vortex_area_ssw_'+sswdate0+'_merra.sav'
;restore,'vortex_area_ssw_'+sswdate0+'_merra.sav'
...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/times,filename='../Figures/profs_merra_vortex_area_ssw_event_'+sevent+'.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
...
plot,findgen(50),th,/nodata,thick=6,yrange=[500.,5000.],/noeras,ytitle='Theta (K)',color=0,xtitle='MERRA Vortex Area (% NH)',charsize=1.25,charthick=2
nlvls=61
col1=(1.+findgen(nlvls)/61.)*mcolor
for i=30,90 do begin
    if i mod 5 eq 0 then oplot,area_zt_nc4(i-30,*),th,color=col1(i-30),thick=10
endfor
imin=-30
imax=30
...
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,$
      xtitle='Days since SSW Onset ('+sswdate0+')',charthick=2,charsize=1.5
ybox=[0,10,10,0,0]
x1=imin
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ../Figures/profs_merra_vortex_area_ssw_event_'+sevent+'.ps -rotate -90 ../Figures/profs_merra_vortex_area_ssw_event_'+sevent+'.jpg'
endif
