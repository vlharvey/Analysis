;
; plot average IPV at any theta i.e. vertically interpolate if necessary
;
@calcelat2d
@stddat
@kgmt
...
jumpz:
    ...
    pvavg=pvavg+pv1
    markavg=markavg+mark1
    icount=icount+1L
goto,jump
plotit:
ifile1=ifile

    pv1=transpose(pvavg)/float(icount)
    mark1=transpose(markavg)/float(icount)
;
; calculate equivalent latitude
;
    elat1=calcelat2d(pv1,alon,alat)
    save,file=ifile0+'-'+ifile1+'_'+stheta+'K_elat.sav',alon,alat,elat1
    ...
    if setplot eq 'ps' then begin
       ...
       device,/landscape,bits=8,$
              filename='Figures/'+ifile0+'-'+ifile1+'_'+stheta+'K_pv+mark.ps'
       device,/color
    ...
    MAP_SET,90,0,0,/ortho,/noeras,/grid,/contin,/noborder,$
            title=ifile0+'-'+ifile1+'  '+stheta+' K',charsize=2.0,latdel=10,$
            limit=[30.,0.,90.,360.]
    ...
    contour,elat,x,alat,/overplot,levels=10*findgen(10),color=0,thick=2
    ...
    for i=0,nc-1 do begin
        markx=reform(mark(i,*))
        index=where(markx gt 0. and alat gt 0.)
        if index(0) ne -1 then begin
           xedge(i)=alon(i)
           yedge(i)=min(alat(index))
        endif
    endfor
    save,file=ifile0+'-'+ifile1+'_'+stheta+'K_vortex.sav',xedge,yedge
