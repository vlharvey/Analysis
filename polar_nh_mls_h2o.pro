...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='polar_nh_mls_h2o_'+sdate+'.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
...
;
    ...
    for i=0L,mprof-1 do begin
        oplot,[mlon(i),mlon(i)],[mlat(i),mlat(i)],psym=8,symsize=1.5,color=((mh2olev(i)-imin)/(imax-imin))*mcolor
    endfor
    ymnb=ymn -cbaryoff
    ymxb=ymnb+cbarydel
    set_viewport,xmn+0.01,xmx-0.01,ymnb,ymxb
    !type=2^2+2^3+2^6
    plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],/noeras,color=0,charsize=1,xtitle='82 km Water Vapor (ppmv)'
    ybox=[0,10,10,0,0]
    x2=imin
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert -trim polar_nh_mls_h2o_'+sdate+'.ps -rotate -90 '+$
                            'polar_nh_mls_h2o_'+sdate+'.jpg'
        spawn,'/usr/bin/rm polar_nh_mls_h2o_'+sdate+'.ps'
     endif
     jumpday:
goto,jump
end
