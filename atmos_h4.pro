...
;    PRO atmos_h4
;
; This Program is designed for Level-1B OMI, and All Levels of MODIS, and AIRS Atmospheric products. 
; However,it can also read other HDF files such as Level 1B radiance & subsets,  
; This is an IDL based program. It reads an HDF-EOS 
...
; applicable for that pixel. An example of unpacking is given for one conditions
; 'land/water mask' which consists of last two bits of first byte of Cloud Mask
; parameter. This program contains some code which are not activated, user can implement it. 
;
;
