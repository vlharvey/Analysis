...
;        !p.font=0
         ...
         device,/landscape,bits=8,$
                 filename='polar_temp+mark_geos5-mls_'+stheta+'km_'+sdate+'_dmp.ps'
         device,/color
...
;
; extract 500 K level from dmp file
;
index=where(thlev eq 450.)
velat_lev=reform(velat_prof(*,index(0),0))
elat_lev=reform(elat_prof(*,index(0)))
index=where(elat_lev ge velat_lev and velat_lev gt 0.,npts)
if index(0) ne -1L then begin
print,min(elat_lev(index)),max(elat_lev(index)),min(velat_lev(index)),max(velat_lev(index)),npts,n_elements(latitude(index))
vlat=latitude(index)
vlon=longitude(index)
endif
...
!type=2^2+2^3
!psym=0
xyouts,0.45,max(yorig)+ylen+0.05,sdate,charsize=2,charthick=2,/normal,color=mcolor
MAP_SET,90,0,-90,/ortho,/noeras,/grid,/contin,title='GEOS-5',charsize=1.5
nlvls=27
col1=1+indgen(nlvls)*icolmax/nlvls
level=200.+2.5*findgen(nlvls)
...
;loadct,0
;oplot,longitude,latitude,psym=1,color=mcolor*.5
;loadct,39
;if npts gt 1 then oplot,vlon,vlat,psym=8,color=mcolor
imin=min(level)
imax=max(level)
ymnb=ymn -cbaryoff
ymxb=ymnb  +cbarydel
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim polar_temp+mark_geos5-mls_'+stheta+'km_'+sdate+'_dmp.ps '+$
         ' -rotate -90  polar_temp+mark_geos5-mls_'+stheta+'km_'+sdate+'_dmp.jpg'
   stop
endif
goto,jump
end
