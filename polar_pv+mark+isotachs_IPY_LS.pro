...
;        !p.font=0
         ...
         device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
                /bold,/color,bits_per_pixel=8,/times,filename='Collins/'+sdate+'_PV_LS.ps'
         !p.charsize=1.25
         !p.thick=2
         !p.charthick=5
         !p.charthick=5
...
usersym,cos(a),sin(a)
    ...
    xyouts,.65,.12,'K'+ue+'hlungsborn, Germany',charsize=1.5,/normal,color=mcolor*.475,charthick=7
      ...
      if setplot eq 'ps' then begin
         device, /close
         spawn,'convert -trim Collins/'+sdate+'_PV_LS.ps -rotate -90 Collins/'+sdate+'_PV_LS.png'
         spawn,'/usr/bin/rm Collins/'+sdate+'_PV_LS.ps'
      endif
goto,jump
end
