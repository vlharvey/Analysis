...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='haloe_loc.ps'
   !p.thick=2
   !p.charthick=5
   !y.thick=2
   !x.thick=2
...
for iyear=0L,nyear-1L do begin
    if iyear le 7L then xyouts,xmn+iyear*xinc,ymn-0.1,syear(iyear),color=col1(iyear),/normal,charsize=1.5
    if iyear gt 7L then xyouts,xmn+(iyear-8L)*xinc,ymn-0.125,syear(iyear),color=col1(iyear),/normal,charsize=1.5
;
; SAGE II
;
...
; SOFIE
;
    ex=findfile(dirso+'cat_sofie_v1.3.'+syear(iyear)+'.sav')
    if ex(0) ne '' then begin
       restore,dirso+'cat_sofie_v1.3.'+syear(iyear)+'.sav'
;      oplot,fdoy,latitude,psym=8,color=col1(iyear)
    endif
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim haloe_loc.ps -rotate -90 haloe_loc.jpg'
endif
end
