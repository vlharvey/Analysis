...
;
      ...
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='PV_vs_SF/geos5_sh_sf_pv_mean+sigmalat_'+sdate+'.ps'
         device,/color
...
;
          ...
          for n=1,nbins-2L do begin
              t=where(lat lt -40. and pv ge pvbin(n) and pv lt pvbin(n+1))
              if n_elements(t) ge 2L then begin
                 result=moment(lat(t))
                 meanlat_pv(n,ilev)=result(0)
                 sigmalat_pv(n,ilev)=sqrt(result(1))
...
plot,sfbin(index),meanlat_sf(index,ilev),color=0,thick=2,/noeras,xtitle='SF Bins',$
     yrange=[-30.,-90.],/nodata,ytitle='Mean Latitude',xrange=[-5.e-6,2.e-5],xticks=4
sfdiff=fltarr(nth)
for ilev=nth-1L,0L,-1L do begin
    ...
    sfdiff(ilev)=meanlat_sf(0,ilev)+abs(min(meanlat_sf(index,ilev)))
;print,'SF ',th(ilev),meanlat_sf(0,ilev),min(meanlat_sf(index,ilev)),sfdiff(ilev)
endfor    ; loop over altitude
...
for ilev=nth-1L,0,-1L do begin
    ...
    pvdiff(ilev)=meanlat_pv(0,ilev)+abs(min(meanlat_pv(index,ilev)))
...
plot,sfdiff,th,color=0,thick=7,/noeras,xtitle='Mean Lat at Min SF - Max Lat',$
     ytitle='Theta (K)',xrange=[0.,30.]
set_viewport,.55,.95,.05,.45
plot,pvdiff,th,color=0,thick=7,/noeras,xtitle='Mean Lat at Max PV - Max Lat',$
     ytitle='Theta (K)',xrange=[0.,30.]

if setplot ne 'ps' then stop
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim PV_vs_SF/geos5_sh_sf_pv_mean+sigmalat_'+sdate+'.ps -rotate -90 '+$
         'PV_vs_SF/geos5_sh_sf_pv_mean+sigmalat_'+sdate+'.jpg'
   spawn,'/usr/bin/rm PV_vs_SF/geos5_sh_sf_pv_mean+sigmalat_'+sdate+'.ps'
endif
