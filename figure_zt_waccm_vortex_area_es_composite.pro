;
;
; read WACCM save files and plot composite vortex area
;
@stddat
@kgmt
@ckday
...
result=size(DAILYMEANTEMP)
nevents=result(1)
resultf=file_search(dir+'*_waccm.sav')
...
;
    dum=strsplit(resultf(ies),'/',/extract)
    dum2=strsplit(dum(4),'_',/extract)
    esdate0=dum2(3)
print,esdate0
    restore,dir+esdate0+'_waccm.sav'
    ...
    area_composite=area_composite+AREA_ZT_NC4
    ztavg_composite=ztavg_composite+ztavg
    ztmax_composite=ztmax_composite+ztmax
    ztavg_all(*,ies)=ztavg
    ztmax_all(*,ies)=ztmax
endfor
area_composite=area_composite/float(nevents)
ztavg_composite=ztavg_composite/float(nevents)
ztmax_composite=ztmax_composite/float(nevents)
;
; save file
;
save,file='/Users/harvey/Harvey_etal_2014/Post_process/vortex_area_ES_composite_waccm.sav',area_composite,th,koff,ztavg_composite,ztmax_composite,sfile

plotit:
restore,'/Users/harvey/Harvey_etal_2014/Post_process/vortex_area_ES_composite_waccm.sav'
kday=n_elements(SFILE)
;
...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/times,filename='../Figures/waccm_vortex_area_ES_event_composite.ps'
       !p.charsize=1.25
       !p.thick=2
       !p.charthick=5
       !p.charthick=5
...
xmax=30.
contour,area_composite,-1.*koff+findgen(kday),th,color=0,xtitle='Days From ES Onset',thick=6,yrange=[500.,5000.],/noeras,ytitle='Theta (K)',/cell_fill,$
...
plots,0,500
plots,0,5000.,/continue,color=mcolor,thick=5
...
plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,$
      xtitle='WACCM ES Composite Vortex Area (% NH)',charthick=2,charsize=1.5
ybox=[0,10,10,0,0]
x1=imin
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ../Figures/waccm_vortex_area_ES_event_composite.ps -rotate -90 ../Figures/waccm_vortex_area_ES_event_composite.jpg'
endif
