...
;
          ...
          for ii=0L,nbins-1L do begin
              index=where(y2d gt 20. and sf ge sfbins(ii)-dsf and sf lt sfbins(ii)+dsf)
...
;
; increment this level if PV does not vary monotonically and compute average PV sigma within
; those non-monotonic SF bins
;
n0=findgen(nbins)
n1=1.+findgen(nbins)
diff=meanpv(n1)-meanpv(n0)
index=where(diff gt 0.,npts)
if npts ge 5L then begin
;  pvfreq(ilev)=pvfreq(ilev)+npts
   pvfreq(ilev)=pvfreq(ilev)+1.
   pvsigma(ilev)=pvsigma(ilev)+total(sigmapv(index))/float(npts)
   pvlat(ilev)=pvlat(ilev)+total(meanlat(index))/float(npts)
   pvlatsigma(ilev)=pvlatsigma(ilev)+total(sigmalat(index))/float(npts)
;  plot,sfbins,meanpv,psym=0,thick=5,color=0
;  print,'# SF bins where PV decreases ',th(ilev),npts
;  stop
endif
...
goto,jump
plotit:
;pvfreq=100.*pvfreq/float(icount)/float(nbins)
...
save,file='geos5_pv_freq_monotonic_nh_'+syr+smn+'.sav',pvfreq,pvsigma,pvlat,pvlatsigma,th,sdates,sdate0,sdate1
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='geos5_nh_sf_vs_pv_'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_monotonic.ps'
   device,/color
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim geos5_nh_sf_vs_pv_'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_monotonic.ps -rotate -90 '+$
         'geos5_nh_sf_vs_pv_'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_monotonic.jpg'
   spawn,'/usr/bin/rm geos5_nh_sf_vs_pv_'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_monotonic.ps'
endif
end
