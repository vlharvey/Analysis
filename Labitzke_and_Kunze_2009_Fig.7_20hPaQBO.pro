...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='Labitzke_and_Kunze_2009_Fig.7_20hPaQBO.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
muchgeq20=-99.+0.*fltarr(nyear)                  ; Feb monthly standard deviation of zonal wind at the equator
...
for i=0,nyear-1L do begin
    ...
    if index(0) ne -1 then begin        ; there are years ini f10.7 record before and after ERA40
       ...
       if min(mulev20(index)) lt 0. and max(mulev20(index)) gt 0. then muchgeq20(i)=1
...
ulev20=reform(UBAR_EQ_ALL(*,ilev20))	; daily Ubar at Equator and 10 hPa
...
uavgeq20=-99.+0.*fltarr(nyear)			; Feb monthly average zonal wind at the equator
...
uchgeq20=-99.+0.*fltarr(nyear)			; Feb monthly standard deviation of zonal wind at the equator
...
for i=0,nyear-1L do begin
    ...
    if index(0) ne -1 then begin	; there are years ini f10.7 record before and after ERA40
       ...
       uavgeq20(i)=total(ulev20(index))/float(nn)
       ...
       if min(ulev20(index)) lt 0. and max(ulev20(index)) gt 0. then uchgeq20(i)=1
...
plot,f10_feb(index),tpavg30(index),xtitle='Solar Flux 10.7 cm',ytitle='NP GPH',psym=8,color=0,$
     xrange=[60.,260.],yrange=[tmin,tmax],title='QBO East at 20 hPa'
...
plot,f10_feb(index),tpavg30(index),xtitle='Solar Flux 10.7 cm',psym=8,color=0,$
     xrange=[60.,260.],yrange=[tmin,tmax],title='QBO West at 20 hPa'
for i=0L,nn-1L do xyouts,f10_feb(index(i)),tpavg30(index(i)),strmid(strcompress(years(index(i)),/remove_all),2,2),$
    charsize=1,/data,color=0
a=findgen(8)*(2*!pi/8.)
usersym,cos(a),sin(a),/fill
...
if setplot eq 'ps' then begin
device,/close
spawn,'convert -trim Labitzke_and_Kunze_2009_Fig.7_20hPaQBO.ps -rotate -90 Labitzke_and_Kunze_2009_Fig.7_20hPaQBO.jpg'
endif
end
