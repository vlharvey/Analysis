...
for thlev=0,nth-1 do begin
    ...
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       set_plot,'ps'
       device,/color,/landscape,bits=8,filename='geos5_10yr_ii_1pan_'+mon(m)+'_'+stheta+'K.ps'
       device,/bold
...
; of inertial instability
    ...
    MAP_SET,0,0,0,/contin,/grid,/noeras,charsize=2,$
            title=months(m)+' GEOS-5 at '+stheta+' K',limit=[-60,0,60,360]
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim geos5_10yr_ii_1pan_'+mon(m)+'_'+stheta+'K.ps'+$
         ' -rotate -90 geos5_10yr_ii_1pan_'+mon(m)+'_'+stheta+'K.jpg'
   spawn,'/usr/bin/rm geos5_10yr_ii_1pan_'+mon(m)+'_'+stheta+'K.ps'
endif
