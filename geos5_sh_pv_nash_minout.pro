...
; min PV is not in the highest half of SF bins
;
@stddat
@kgmt
@ckday
...
;         aindex=where(y2d lt -40. and mark1 eq 0.)
          aindex=where(y2d lt -40. and sf1 lt sfthresh)
          if aindex(0) eq -1L then goto,jumplev
          amin=min(pv1(aindex))
;         vindex=where(y2d lt -40. and mark1 eq 1.)
          vindex=where(y2d lt -40. and sf1 ge sfthresh)
...
save,file='geos5_pv_freq_minout_sh_'+syr+smn+'.sav',pvfreq,sdates,sdate0,sdate1,th
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='geos5_sh_pv_'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_minout.ps'
   device,/color
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim geos5_sh_pv_'+sdate+'_'+slabs(0)+'_nash_minout.ps -rotate -90 '+$
         'geos5_sh_pv_'+sdate+'_'+slabs(0)+'_nash_minout.jpg'
   spawn,'/usr/bin/rm geos5_sh_pv_'+sdate+'_'+slabs(0)+'_nash_minout.ps'
endif
end
