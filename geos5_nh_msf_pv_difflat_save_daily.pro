;
; compute the standard deviation of latitude within MSF and PV bins
;
@stddat
@kgmt
@ckday
...
;
      ...
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='PV_vs_SF/geos5_nh_msf_pv_difflat_'+sdate+'.ps'
         device,/color
...
;
; gridpoints within concentric MSF and PV contours poleward of 40 N
;
...
xmin=300000.
xmax=1000000.
plot,sfbin(index),meanlat_msf(index,ilev),color=0,thick=7,/noeras,xtitle='MSF Bins',$
...
for ilev=nth-1L,0L,-1L do begin
    ...
    sfbin=sfmin+((sfmax-sfmin)/float(nbins))*findgen(nbins)
    index=where(meanlat_msf(*,ilev) ne 0.)
    oplot,sfbin(index),meanlat_msf(index,ilev),color=col2(ilev),psym=0,thick=7
    if meanlat_msf(0,ilev) ne 0. then sfdiff(ilev)=abs(meanlat_msf(0,ilev)-max(meanlat_msf(index,ilev)))
endfor    ; loop over altitude
...
for ilev=nth-1L,0L,-1L do begin
    ...
    if sfbin(index(0)) gt xmin and sfbin(index(0)) lt xmax then $
       xyouts,sfbin(index(0)),meanlat_msf(index(0),ilev),'V',/data,color=0,charthick=5,charsize=2,alignment=0.5
endfor    ; loop over altitude
;
!type=2^2+2^3
...
;plot,sfdiff(index),th(index),color=0,thick=7,/noeras,xtitle='Mean Lat at Min MSF - Max Lat',$
plot,sfdiff(index),th(index),color=0,psym=2,/noeras,xtitle='Mean Lat at Min MSF - Max Lat',$
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim PV_vs_SF/geos5_nh_msf_pv_difflat_'+sdate+'.ps -rotate -90 '+$
         'PV_vs_SF/geos5_nh_msf_pv_difflat_'+sdate+'.jpg'
   spawn,'/usr/bin/rm PV_vs_SF/geos5_nh_msf_pv_difflat_'+sdate+'.ps'
endif
...
save,file='geos5_nh_msf_pv_diffwlat_save_daily.sav',sfdiff_all,pvdiff_all,sdates,th
end
