...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='polar_saber+geos5+speeddiff_4day_'+salt+'.ps'
   device,/color
...
if imn eq 1 and idy eq 24 then begin
    ...
    ymx=yorig(2)+ylen
    set_viewport,xmn,xmx,ymn,ymx
    map_set,90,0,-90,/ortho,/noerase,title='GEOS-5 - SABER',color=0,charsize=1.5
...
if imn eq 2 and idy eq 23 then begin
    ...
    !type=2^2+2^3+2^6
    plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],xtitle=salt+' km Isotachs (m/s)',color=0
    ybox=[0,10,10,0,0]
    x1=imin
    ...
    plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],/noeras,$
          xtitle=salt+' km Differences (m/s)',xticks=n_elements(level)/2,color=0
    ybox=[0,10,10,0,0]
    x2=imin
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim polar_saber+geos5+speeddiff_4day_'+salt+'.ps '+$
         ' -rotate -90 polar_saber+geos5+speeddiff_4day_'+salt+'.jpg'
endif
end
