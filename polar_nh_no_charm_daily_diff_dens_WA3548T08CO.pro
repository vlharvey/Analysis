...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='polar_nh_no_charm_'+sdate+'_'+slev+'_diff_dens_WA3548T08CO.ps'
       device,/color
...
;   noy1=reform(noygrd(*,*,ilev))*1.e6
    ...
    !type=2^2+2^3
    xyouts,.3,.8,'WA3548T08CO '+sdate+' '+slev+' ('+szavg+')',/normal,color=0,charsize=1.5
...
;   drawvectors,nc+1,nr,x,alat,u,v,20,1
    ...
    set_viewport,xmn,xmx,ymn,ymx
    !type=2^2+2^3
    MAP_SET,90,0,-90,/ortho,/noeras,/grid,/contin,title='Daily Difference',charsize=1.5,color=0,/noborder	;,limit=[60.,0.,90.,360.]
...
;velovect,u,v,x,alat,color=0,/overplot,length=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim polar_nh_no_charm_'+sdate+'_'+slev+'_diff_dens_WA3548T08CO.ps -rotate -90 polar_nh_no_charm_'+sdate+'_'+slev+'_diff_dens_WA3548T08CO.jpg'
       spawn,'rm -f polar_nh_no_charm_'+sdate+'_'+slev+'_diff_dens_WA3548T08CO.ps'
    endif
endfor	; loop over pressure

jumpstep:
if iday eq 2 then noold=no_conc
