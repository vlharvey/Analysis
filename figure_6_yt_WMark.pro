...
;
    ...
    if iday eq 0L then begin
       ...
       zprof=mean(Z3_DAILY_MEAN,dim=1)/1000.
       index=where(abs(zprof-ralt) eq min(abs(zprof-ralt)))
       ialt2=index(0)
;goto,quick
    endif
...
;
    ...
    vstaryt(iday,*)=reform(vstar(*,ialt2))
    wstaryt(iday,*)=reform(wstar(*,ialt2))
endfor
;
; save file
;
save,file='yt_WMark_smidemax_'+salt+'.sav',ndays,nr,alat,mmdd,tyt,uyt,myt,zyt,coyt,wstaryt,vstaryt
quick:
restore,'yt_WMark_smidemax_'+salt+'.sav'
;
; postscript file
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='figure_5_yt_WMark_smidemax.ps'
       !p.charsize=1
       !p.thick=2
       !p.charthick=2
       !y.thick=2
...
wyt=wstaryt*100.
    contour,wyt,findgen(ndays),alat,/noera,/fill,color=0,c_color=col1,levels=tlevel,xrange=[0,ndays-1],yrange=[-90,90],ytitle='Latitude',charsize=1,charthick=2,$
            xticks=nxticks-1,xtickname=xlabs,xtickv=xindex
index=where(tlevel gt 0.)
    contour,wyt,findgen(ndays),alat,/noera,/follow,color=0,levels=tlevel(index),/overplot
index=where(tlevel lt 0.)
    contour,wyt,findgen(ndays),alat,/noera,/follow,color=mcolor,levels=tlevel(index),/overplot,c_linestyle=5
...
xyouts,(x0+x1)/3.,y0-0.03,salt+' wstar (cm/s)',color=0,charsize=1.25,charthick=2,/normal
...
for j=0,nlvls-2 do begin
    ...
    set_viewport,xmn,xmx,ymn,ymx
    nlvls=23
    tlevel=-6+0.5*findgen(nlvls)        ;[130+5*findgen(nlvls),280,300,320,350,400,450,500]
    nlvls=n_elements(tlevel)
    col1=(findgen(nlvls)/float(nlvls))*mcolor
wyt=vstaryt
    contour,wyt,findgen(ndays),alat,/noera,/fill,color=0,c_color=col1,levels=tlevel,xrange=[0,ndays-1],yrange=[-90,90],ytitle='Latitude',charsize=1,charthick=2,$
...
plot,[0,0],[0,0],yrange=[0,10],xrange=[0,1],/noeras,xticks=nlvls-1L,$
        position = [x0,y0,x1,y1],xstyle=1,xtickname=slab,/nodata,color=0
xyouts,(x0+x1)/3.,y0-0.03,salt+' vstar (m/s)',color=0,charsize=1.25,charthick=2,/normal
slab=strcompress(string(format='(i4)',long(level)),/remove_all)
slabcolor = fltarr(nlvls)*0.
slabcolor[0:10] = 255        ; set first few labels to white so they are visible
ybox=[0,10,10,0,0]
...
;
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim figure_5_yt_WMark_smidemax.ps -rotate -90 figure_5_yt_WMark_smidemax.jpg
;      spawn,'rm -f figure_5_yt_WMark_smidemax.ps'
    endif
