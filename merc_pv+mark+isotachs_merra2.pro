...
lstmn=6L & lstdy=1L & lstyr=2011
ledmn=9L & leddy=30L & ledyr=2011
lstday=0L & ledday=0L
;read,' Enter starting date (month, day, year) ',lstmn,lstdy,lstyr
;read,' Enter ending date   (month, day, year) ',ledmn,leddy,ledyr
if lstyr lt 1980 then stop,'Year out of range '
if ledyr lt 1980 then stop,'Year out of range '
...
;        !p.font=0
         ...
         device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
                /bold,/color,bits_per_pixel=8,/times,filename='merc_pv_merra2_'+sdate+'_'+stheta+'K.ps'
         !p.charsize=1.25
         !p.thick=2
         !p.charthick=5
         !p.charthick=5
      ...
      MAP_SET,0,180,0,/merc,/noeras,/grid,/contin,$
              title=sdate,charsize=2.0,latdel=10,color=0,limit=[0,0,50,360]
...
loadct,23
      ...
      contour,pv,x,alat,/overplot,levels=level,/follow,c_labels=0*level,/noeras,color=0
;drawvectors,nc,nr,alon,alat,u1,v1,5,1
;
; vortex edge at all levels
;
      contour,mark,x,alat,/overplot,levels=[-0.1],thick=10,color=250
      index=where(mark lt 0.)
      if index(0) ne -1L then oplot,lon(index),lat(index),psym=1,color=240
...
;     contour,z,x,alat,/overplot,nlevels=20,thick=2,color=mcolor
      contour,sf,x,alat,/overplot,nlevels=30,/follow,c_labels=0*level,/noeras,color=mcolor,thick=2
      index=where(qdf lt 0.)
      if index(0) ne -1L then oplot,lon(index),lat(index),psym=1,color=mcolor
loadct,23
...
; limit (y0, x0, y1, x1)
loadct,39
      MAP_SET,0,180,00,/merc,/noeras,/grid,/contin,/noborder,charsize=2.0,latdel=10,color=mcolor,limit=[0,0,50,360]
loadct,23
      ymnb=ymn -cbaryoff
      ymxb=ymnb+cbarydel
      ...
      plot,[imin,imax],[0,0],yrange=[0,10],$
            xrange=[imin,imax],xtitle='MERRA-2 '+stheta+' K Potential Vorticity (PVU)',/noeras,$
      ...
      if setplot eq 'ps' then begin
         device, /close
         spawn,'convert -trim merc_pv_merra2_'+sdate+'_'+stheta+'K.ps -rotate -90 merc_pv_merra2_'+sdate+'_'+stheta+'K.png'
;        spawn,'rm -f merc_pv_merra2_'+sdate+'_'+stheta+'K.ps'
      endif
      icount=icount+1L
goto,jump
end
