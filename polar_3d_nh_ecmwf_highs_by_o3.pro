...
; Arctic vortex in black
; Anticyclones colored by ozone poleward of 2.5N
...
; --- Test for end condition and close windows.
      ...
      date=strcompress(string(FORMAT='(A3,A1,I2,A2,I4)',$
                              month(imn-1),' ',idy,', ',iyr))
      ifile=string(FORMAT='(i2.2,a1,i2.2,a1,i4.4,a8)',$
            imn,'_',idy,'_',iyr,'_12Z.nc3')
      rd_ecmwf_nc3,dir+ifile,nc,nr,nth,alon,alat,th,pv2,p2,$
         msf2,u2,v2,q2,qdf2,mark2,vp2,sf2,sh2,o32,iflag
...
; select theta levels to plot
    if icount eq 0L then begin
       zindex=where(th ge 400. and th le 1800.,nth2)
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='1989-1990/'+ifile+'_ho3.ps'
       device,/color
...
; substitute ozone mixing ratio
        temp(0:nc-1,0:nr2-1)=o31(0:nc-1,nr/2:nr-1)    ; NH
        temp(nc,*)=temp(0,*)
;       index=where(y2d lt 30. or temp eq 0.)
        index=where(temp eq 0.)
        if index(0) ne -1 then temp(index)=1.e15
index=where(temp ne 1.e15)
if index(0) ne -1 then print,th(lev),min(temp(index)),max(temp(index))
...
; pressure of theta surface
	...
	avgz=result(0)
        savgz=strcompress(string(FORMAT='(F5.1)',avgz))
	...
	if lindex(0) ne -1 then begin
           for ii=0,nl-1 do begin
               ...
               oplot,[xcn(lindex(ii)),xcn(lindex(ii))],$
                     ...
                     /T3D,zvalue=nz,psym=8,symsize=0.1,$
                     color=0	;((temp(lindex(ii))-imin)/(imax-imin))*icolmax
...
;              index=where(tmp ne -9999. and y2d gt 13.7500 and dum eq -1.0*(ihigh+1))
               index=where(tmp ne -9999. and y2d gt 0. and dum eq -1.0*(ihigh+1),nl)
;              oplot,xcn(index),ycn(index),psym=8,color=0,/T3D,zvalue=nz,symsize=2
           for ii=0,nl-1 do begin
               if temp(index(ii)) ne 1.e15 then $
               oplot,[xcn(index(ii)),xcn(index(ii))],$
                     [ycn(index(ii)),ycn(index(ii))],$
                     /T3D,zvalue=nz,psym=8,symsize=2,$
                     color=((temp(index(ii))-imin)/(imax-imin))*icolmax
           endfor
               contour,tmp,xcn,ycn,levels=[sedge],color=0,$
...
jumplev:
    ...
    !psym=0
    xyouts,0.33,0.88,date,/normal,charsize=3.0,color=0,charthick=2
    xyouts,.08,.81,'Theta (K)',charsize=2,/normal,color=0,charthick=2
    xyouts,.78,.81,'Pressure (hPa)',charsize=2,/normal,color=0,charthick=2
    set_viewport,.2,.78,.14-cbaryoff,.14-cbaryoff+cbarydel
    !type=2^2+2^3+2^6
    level=imin+findgen(imax+1)
    plot,[imin,imax],[0,0],yrange=[0,10],$
          xrange=[imin,imax],xtitle='Ozone (ppmv)',/noeras,$
          xtickname=strcompress(string(fix(level)),/remove_all),$
          xstyle=1,xticks=imax,charsize=1.5,color=0,charthick=2
    ybox=[0,10,10,0,0]
    x1=imin
...
;    if setplot ne 'ps' then stop
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim 1989-1990/'+ifile+'_ho3.ps -rotate -90 1989-1990/'+ifile+'_ho3.jpg'
    endif
