...
thdepthnhsm=smooth(thdepthnh,3,/edge_truncate)
thdepthshsm=smooth(thdepthsh,3,/edge_truncate)
NHIGHNHsm=smooth(NHIGHNH,3,/edge_truncate)
NHIGHSHsm=smooth(NHIGHSH,3,/edge_truncate)
;
; fill
;
...
contour,thdepthnh,fdoy_save-min(fdoy_save),latbin,levels=level,min_value=-9999.,yrange=[30.,85.],$
...
for ii=0L,n_elements(xindex)-1L do $
    xyouts,xindex(ii),25.,xlabs(ii),charsize=1.5,alignment=0.5,color=0,/data
loadct,0
;
...
contour,thdepthsh,fdoy_save-min(fdoy_save),latbin,levels=level,min_value=-9999.,yrange=[-85.,-30.],$
        title='WACCM 3 Antarctic',/noeras,/cell_fill,c_color=col1,color=0,xticks=nxtick-1L,/nodata,$
;       xtickv=xindex,xtickname=strmid(sdays(xindex),4,2)+'/'+strmid(sdays(xindex),2,2),ytitle='Latitude',charsize=1.5
        xtickv=xindex,xtickname=' '+strarr(n_elements(xindex)+1),ytitle='Latitude',charsize=1.5
for ii=0L,n_elements(xindex)-1L do $
    xyouts,xindex(ii),-90.,xlabs(ii),charsize=1.5,alignment=0.5,color=0,/data
loadct,0
;
; contour anticyclone frequency
;
