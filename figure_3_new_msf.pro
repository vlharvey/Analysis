...
; NH + SH version (MSF and PV are reversed in vortex)
;
; 4-year monthly means
; plot an altitude profile of the mean latitude difference
; between min (max) MSF (PV) contours and max latitude in
; any of the bins
;
@stddat
@kgmt
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='figure_3_new_msf.ps'
endif
erase
!type=2^2+2^3
xmn=xorig(0)
...
restore,'geos5_sh_msf_pv_diffwlat_save_daily.sav
...
ymx=yorig(1)+ylen
set_viewport,xmn,xmx,ymn,ymx
restore,'geos5_nh_msf_pv_diffwlat_save_daily.sav
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim figure_3_new_msf.ps -rotate -90 figure_3_new_msf.jpg'
;  spawn,'/usr/bin/rm figure_3_new_msf.ps'
endif
end
