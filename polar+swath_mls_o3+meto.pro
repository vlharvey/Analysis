...
; restore MLS data
;
restore,dirm+'MLS-Aura_L2GP_20060110.sav'
mpress=o3.pressure
...
for j=0,nr-1 do x2d(*,j)=x
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       set_plot,'ps'
       device,/color,/landscape,bits=8,filename='polar+swath_mls_o3+meto_'+sdate+'.ps'
...
;
    ...
    mark(nc,*)=mark(0,*)
    map_set,90,0,0,/stereo,/contin,/grid,/noeras,charsize=1.5,/noborder,title=sdate+' '+stheta+' K',color=lc
    contour,sf,x,alat,nlevels=30,c_color=lc,/overplot,/follow,c_labels=0,/noeras
    oplot,findgen(361),1.+0.*findgen(361),psym=8,symsize=0.5,color=0
...
loadct,0
    ...
    contour,mark,x,alat,levels=[-.1],c_color=mcolor*.9,/overplot,/follow,c_labels=0,/noeras,thick=8
loadct,38
...
;          oplot,mlon(kindex),mlat(kindex),psym=8,symsize=1.25,color=(float(i+1)/float(npt))*mcolor
           ...
           xyouts,xtmp(index(0)),5.,stime,/data,charsize=2,alignment=0.5,color=lc,charthick=2
        endif
    endfor
...
loadct,38    
    ...
    contour,o3swath,xswath,thswath,levels=level,/cell_fill,title='Swath at '+stime+' UT',$
            ...
            yrange=[500.,2000.],ytitle='Theta',charsize=1.25,color=lc
...
for i=0L,mprof-1L,20 do begin
    plots,i,500
    plots,i,400,/data,/continue,color=lc
    xyouts,i,250.,xlabels(i),/data,charsize=1.25,alignment=0.5,color=lc
    xyouts,i,350.,ylabels(i),/data,charsize=1.25,alignment=0.5,color=lc
endfor
i=mprof-1L
plots,i,500
plots,i,400,/data,/continue,color=lc
...
xyouts,i,350.,ylabels(i),/data,charsize=1.25,alignment=0.5,color=lc
    ...
    !type=2^2+2^3+2^5
    plot,[0,0],[imin,imax],xrange=[0,10],yrange=[imin,imax],title='O3 ppmv',color=lc
    xbox=[0,10,10,0,0]
    y1=imin
...
;
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim polar+swath_mls_o3+meto_'+sdate+'.ps -rotate -90 '+$
             'polar+swath_mls_o3+meto_'+sdate+'.jpg'
    endif
;endfor
END
