;
; plot longitude-time sections of MetO MSF, Anticyclones, Polar vortices, and a trajectory
    ...
    for k=0,nth-1 do begin
        ...
        for j=0,nr-1 do begin
            ...
            if setplot eq 'ps' then begin
               ...
               !p.font=0
               device,font_size=9
               device,/landscape,bits=8,filename='hov_MSFmark_'+sth+'K_'+slat+'_'+sfile(0)+'.ps'
               device,/color
            ...
            mtitle='UKMO Mont. Streamfunction '+sth+' K '+slat
            plt=reform(msfxt(*,*,j,k),nc,nfile)
            plt1=smooth(reform(markxt(*,*,j,k),nc,nfile),3)
            uplt=reform(uxt(*,*,j,k),nc,nfile)
            imin=min(plt)
            imax=max(plt)
;           print,sfile(0),' ',sth,' ',slat,' ',imin/1.e6,imax/1.e6
            if th2(k) eq 1000. then begin
               imin=700000.
               imax=770000.
               if abs(ymids(j)) lt 40. then begin
                  imin=730000.
                  imax=750000.
               endif
            endif
            if th2(k) eq 800. then begin
               imin=670000.
               imax=740000.
               if abs(ymids(j)) lt 40. then begin
                  imin=700000.
            ...
            contour,plt,alon,findgen(nfile),xrange=[0.,360.],/fill,$
                    ...
                    title=mtitle,/noeras,levels=level,charsize=1.5
...
;
            ...
            if setplot eq 'ps' then begin
               device, /close
               spawn,'convert -trim hov_MSFmark_'+sth+'K_'+slat+'_'+sfile(0)+'.ps '+$
                     '-rotate -90 hov_MSFmark_'+sth+'K_'+slat+'_'+sfile(0)+'.jpg'
               spawn,'/usr/bin/rm hov_MSFmark_'+sth+'K_'+slat+'_'+sfile(0)+'.ps'
            endif
