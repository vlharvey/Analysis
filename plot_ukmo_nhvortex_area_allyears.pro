...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='ukmo_nhvortex_area_allyears_'+sth+'K.ps'
   device,/color
...
!type=2^2+2^3+2^7
amax=150.
plot,[1,kday,kday,1,1],[0.,0.,amax,amax,0.],min_value=0.,color=0,$
      xrange=[1,kday],yrange=[0.,amax],/nodata,charsize=2,$
      ytitle='Millions of km!u2!n',xtickname=smon,xticks=n_elements(smon)-1L,$
      title='Arctic Vortex Area '+sth+' K'
...
for i=1L,kday do begin
    if area_min_lev(i-1) ne 0. then begin
       ...
       plots,i,area_max_lev(i-1),/continue,thick=3,color=200
    endif
endfor
loadct,38
...
for iyear=nyear-3L,nyear-1L do begin
    ...
    area_ave=smooth(area_ave,7)
    index=where(abs(area_ave) lt 0.01)
    if index(0) ne -1 then area_ave(index)=-9999.
    oplot,findgen(kday),area_ave,color=col1(iyear-(nyear-3L)),thick=4,min_value=-9999.
    xyouts,xmx+0.02,ymn+(iyear-(nyear-3L))*yinc,slab(iyear-(nyear-3L)),charsize=2,color=col1(iyear-(nyear-3L)),/normal,charthick=2
endfor

if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ukmo_nhvortex_area_allyears_'+sth+'K.ps -rotate -90 ukmo_nhvortex_area_allyears_'+sth+'K.jpg'
endif
end
