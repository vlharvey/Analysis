...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='waccm_mills+meto_ssw_freq_'+sth+'hPa.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !p.charthick=5
...
for imon=1L,12L do begin
    ...
    if imon lt 5L or imon gt 9L then gwa3_nh_minor(imon-1L)=100.*float(npts)/float(gtotdays)
    index=where(gwa3_smon eq imon and gshdtlev gt 0.,npts)
    if imon ge 4L and imon le 8L then gwa3_sh_minor(imon-1L)=100.*float(npts)/float(gtotdays)
endfor
...
for imon=1L,12L do begin
    ...
    if imon lt 5L or imon gt 9L then m0wa3_nh_minor(imon-1L)=100.*float(npts)/float(m0totdays)
    index=where(m0wa3_smon eq imon and m0shdtlev gt 0.,npts)
    if imon ge 4L and imon le 8L then m0wa3_sh_minor(imon-1L)=100.*float(npts)/float(m0totdays)
endfor
...
for imon=1L,12L do begin
    ...
    if imon lt 5L or imon gt 9L then m1wa3_nh_minor(imon-1L)=100.*float(npts)/float(m1totdays)
    index=where(m1wa3_smon eq imon and m1shdtlev gt 0.,npts)
    if imon ge 4L and imon le 8L then m1wa3_sh_minor(imon-1L)=100.*float(npts)/float(m1totdays)
endfor
...
ymx=yorig(1)+ylen
set_viewport,xmn,xmx,ymn,ymx
plot,1.+findgen(12),findgen(10),/nodata,xrange=[1,12],ytitle='Percent of Days',yrange=[0.,10],$
...
for imon=1L,12L do begin
    ...
    if imon lt 4L or imon gt 9L then gwa3_nh_major(imon-1L)=100.*float(npts)/float(totdays)
    index=where(gwa3_smon eq imon and gshdtlev gt 0. and gshu60lev lt 0.,npts)
    if imon ge 4L and imon le 8L then gwa3_sh_major(imon-1L)=100.*float(npts)/float(totdays)
endfor
...
for imon=1L,12L do begin
    ...
    if imon lt 4L or imon gt 9L then m0wa3_nh_major(imon-1L)=100.*float(npts)/float(totdays)
    index=where(m0wa3_smon eq imon and m0shdtlev gt 0. and m0shu60lev lt 0.,npts)
    if imon ge 4L and imon le 8L then m0wa3_sh_major(imon-1L)=100.*float(npts)/float(totdays)
endfor
...
for imon=1L,12L do begin
    ...
    if imon lt 4L or imon gt 9L then m1wa3_nh_major(imon-1L)=100.*float(npts)/float(totdays)
    index=where(m1wa3_smon eq imon and m1shdtlev gt 0. and m1shu60lev lt 0.,npts)
    if imon ge 4L and imon le 8L then m1wa3_sh_major(imon-1L)=100.*float(npts)/float(totdays)
endfor
...
xyouts,6,9.,'WACCM noaurfco '+wmiyr0+'-'+wmiyr1,/data,color=mcolor*.95
xyouts,6,8.,'WACCM noaurgco '+gwmiyr0+'-'+gwmiyr1,/data,color=mcolor*.8
xyouts,6,7.,'WACCM mee00fco '+m0wmiyr0+'-'+m0wmiyr1,/data,color=mcolor*.3
xyouts,6,6.,'WACCM mee01fco '+m1wmiyr0+'-'+m1wmiyr1,/data,color=mcolor*.5
xyouts,6,5.,'MetO '+miyr0+'-'+miyr1,/data,color=0

if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim waccm_mills+meto_ssw_freq_'+sth+'hPa.ps -rotate -90'+$
         ' waccm_mills+meto_ssw_freq_'+sth+'hPa.jpg'
endif
end
