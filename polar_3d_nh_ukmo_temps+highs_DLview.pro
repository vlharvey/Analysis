...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename=odirs(iyear)+ifile+'_DL.ps'
   device,/color
...
loadct,38
    ...
    xyouts,.76,.81,'Altitude (km)',charsize=2,/normal,color=0,charthick=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim '+odirs(iyear)+ifile+'_DL.ps -rotate -90 '+odirs(iyear)+ifile+'_DL.jpg'
;      spawn,'/usr/bin/rm -f '+odirs(iyear)+ifile+'_DL.ps' 
    endif
    jump:
