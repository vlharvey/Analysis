;
;Purpose: create a 5 panel plot of daily zonally averaged temperatures for HIRDLS, MLS, and MLS for all days.
;
;
;
;
...
restore, '/Volumes/MacD68-1/france/ES_paper/Post_process/Platentary_Wave_1_amplitudes_MLS.sav'
...
idayzero = where(strmatch(esdates, dates[x[ies]]) eq 1L)
print, idayzero
daysindex = findgen(61)-30L + idayzero[0]


MLSheight = MLSheight60[daysindex,*]
MLSstrat = MLSstratopausepole[daysindex,*]

x = where(MLSheight le 0.,nx)
if nx gt 0L then MLSheight[x] = !values.f_nan
MLSheight = smooth(MLSheight,3,/nan,/edge)
MLSz60= MLSheight
...
xyouts, .12,.95,'MLS 60 N GPH Wave 1 Amplitude - event ' + strtrim(strcompress(ies+1L),2) + ' -- ' + dates[idayzero]
xyouts, .5, .01,'Wave 1 Height Amplitude at 60!Uo!N N (m)', charsize =1., alignment = .5

;MLS
MLSz60 = MLSz60 ; km to meters
fake = MLSz60
x = where(MLSz60 gt max(level),nx)
if nx gt 0L then fake[x] = max(level)-.1
x = where(MLSz60 lt min(level),nx)
if nx gt 0L then fake[x] = min(level) +.1




  contour, /ylog, fake,findgen(61) - 30., pressure,  levels=level, /cell_fill, c_color = col1,$
  ...
  contour, /ylog, MLSz60,findgen(61) - 30., pressure,  levels=levela, color = 0, /overplot,/follow

  oplot, findgen(61) - 30.,MLSstrat,  psym = 8, color = 255,symsize = .5
...
loadct, 39
  ...
  if setplot eq 'ps' then begin
	...
	'pnmtopng idl001.ppm > /Volumes/MacD68-1/france/ES_paper/Figures/MLS_PW_1_Amp_event_'+strtrim(strcompress(ies+1L),2)+'.png'
