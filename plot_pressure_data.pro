...
;***Read UKMO data
      ...
      if icount eq 0L then begin 
         ...
         index=where(abs(p-rsfc) eq min(abs(p-rsfc)))
         lsfc=index(0)
         ssfc=strcompress(string(rsfc),/remove_all)
         ssfc=strmid(ssfc,0,7)
      endif

      psfile=(string(FORMAT='(a5,i2.2,a1,i2.2,a1,i4,a1,a7,a3)',$
              'ukmo_',imn,'_',idy,'_',iyr,'_',ssfc,'mb.'))
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/portrait,bits=8,filename=psfile+'ps'
         device,/color
...
; Autoscale if scale values for parameter/level are not defined
      nlvls=40
;minval=54.	; bounds for 0.14 hPa
;maxval=64.
;minval=49.	; bounds for 0.316 hPa
;maxval=60.
      level=minval+((maxval-minval)/nlvls)*findgen(nlvls)
      col1=32+indgen(nlvls)*icolmax/(nlvls+5)
...
; polar projection
      ...
      !psym=0
      erase
      MAP_SET,90,-90,0,/stereo,/contin,/grid,/noeras,title='MetO'+mb+$
             'Geopotential Height on '+date,charsize=1.5
      contour,z2,x,alat,levels=level,/cell_fill,/noeras,c_color=col1,/overplot
      contour,z2,x,alat,levels=level,/overplot,c_color=[0],$
              c_labels=0*level,/noeras,c_charsize=1.5
;     contour,z2,x,alat,levels=[54.,55.,56.,57.,58.],/overplot,c_color=0,$
;             c_labels=0*level,/noeras,thick=3

      MAP_SET,90,-90,0,/stereo,/contin,/grid,/noeras,color=0
      u=fltarr(nlg,nlat-1)
      ...
      v(nlg-1,*)=v(0,*)
;     drawvectors,nlg,nlat-1,wlon,wlat,u,v,10,1
...
; Draw color bar
      ...
      plot,[imin,imax],[0,0],yrange=[0,10],$
            xrange=[imin,imax],xtitle='(km)',charsize=1.5
      ybox=[0,10,10,0,0]
      x1=imin
      ...
      if setplot eq 'ps' then begin
         device, /close
         spawn,'convert -trim '+psfile+'ps '+psfile+'jpg'
      endif
      icount=icount+1L
goto, jump
