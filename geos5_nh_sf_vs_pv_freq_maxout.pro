...
; mean PV is largest in higher SF bins
;
@stddat
@kgmt
@ckday
...
;
index=where(meanpv eq max(meanpv))
if index(0) gt 4L then begin
...
;  plot,sfbins,meanpv,psym=0,thick=5,color=1+(float(ilev)/float(nth))*mcolor
   print,'PV max outside vortex ',th(ilev),index(0)
endif
...
;save,file='geos5_pv_freq_maxout_nh_'+syr+smn+'.sav',pvfreq,pvsigma,pvlat,pvlatsigma,th,sdates,sdate0,sdate1
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='geos5_nh_sf_vs_pv_'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_maxout.ps'
   device,/color
...
plot,pvfreq,th,xrange=[0.,100.],yrange=[500.,max(th)],color=0,title=sdate0+'-'+sdate1,$
     xtitle='Frequency of the time mean PV decreases with decreasing SF',ytitle='Theta (K)',thick=5
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim geos5_nh_sf_vs_pv_'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_maxout.ps -rotate -90 '+$
         'geos5_nh_sf_vs_pv_'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_maxout.jpg'
   spawn,'/usr/bin/rm geos5_nh_sf_vs_pv_'+sdate0+'-'+sdate1+'_'+slabs(0)+'_freq_maxout.ps'
endif
end
