...
lstmn=1L & lstdy=13L & lstyr=2007L
ledmn=1L & leddy=31L & ledyr=2007L
lstday=0L & ledday=0L
;
; get date range
;
...
;
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='geos5_nh_sf_vs_pv_'+sdate+'_'+slabs(0)+'_alllev_nash+meanlat.ps'
         device,/color
...
;     xindex=where(th eq 600. or th eq 1000. or th eq 2000. or th eq 3000. or th eq 4000. or th eq 4600.,nxticks)
      plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],color=0,xtitle='Theta (K)',charsize=1.5,$
           xticks=5	;nxticks-1,xtickv=xindex,xtickname=strcompress(long(th(xindex)),/remove_all)
      ybox=[0,10,10,0,0]
      x1=imin
      dx=(imax-imin)/float(nlvls)
      for j=0,nlvls-1 do begin
      ...
      if setplot eq 'ps' then begin
         device,/close
         spawn,'convert -trim geos5_nh_sf_vs_pv_'+sdate+'_'+slabs(0)+'_alllev_nash+meanlat.ps -rotate -90 '+$
               'geos5_nh_sf_vs_pv_'+sdate+'_'+slabs(0)+'_alllev_nash+meanlat.jpg'
;        spawn,'/usr/bin/rm geos5_nh_sf_vs_pv_'+sdate+'_'+slabs(0)+'_alllev_nash+meanlat.ps'
      endif
