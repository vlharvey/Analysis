;
; plot yearly scatterplots of MLS Tbar and CIPS frequency anomalies at a given latitude and altitude
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/times,filename='mls_tbar_ubar_yearly_timeseries_SHPMC_'+slat+'_'+slev+'_anomaly+cips.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
plot,[tmin,tmax],[-40,40],/nodata,yrange=[-40.,40.],xrange=[tmin,tmax],/noeras,charsize=2,charthick=2,color=0,xtitle='MLS T Anomaly (K)',title=slat+' '+slev+' hPa ('+salt+' km)',ytitle='PMC Anomaly'
dfsindex=where(dfs eq -40.,nseason)
...
tanom=smooth(tdatayear(index),7,/edge_truncate)-t30day
...
dum=freq-freq30
;axis,/yax,yrange=[-40,40],ytitle='PMC Frequency Anomaly (%)',color=0,/save

good=where(dfsyear ge min(ddd) and dfsyear le max(ddd))
dfsyear=dfsyear(good)
tanom=tanom(good)
good=where(ddd ge min(dfsyear) and ddd le max(dfsyear))
if good(0) ne -1L then dum=dum(good)
oplot,tanom,dum,color=col1(iseason),psym=1
;oplot,ddd,dum,color=col1(iseason),thick=3
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim mls_tbar_ubar_yearly_timeseries_SHPMC_'+slat+'_'+slev+'_anomaly+cips.ps -rotate -90 mls_tbar_ubar_yearly_timeseries_SHPMC_'+slat+'_'+slev+'_anomaly+cips.jpg'
endif
