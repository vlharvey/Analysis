...
;
      if icount eq 0L then begin
         rd_geos5_nc3_meto,dir+'20080101'+stimes(0)+'nc3',nc,nr,nth,alon,alat,th,$
...
;
      ...
      for k=0L,nth-2L do begin
          rlev=th(k)
          if k ne 0 then dlev=(th(k-1)-th(k+1))/2.
          if k eq 0 then dlev=(th(k)-th(k+1))/2.
          kindex=where(abs(mtheta-rlev) le dlev and mco ne -99.)
          if kindex(0) eq -1L then goto,jumplev
          codata2d=mco(kindex)*1.e7
          y2=histogram(codata2d,min=-1,max=8.,binsize=.25)/float(n_elements(codata2d))
          copdf2d(*,k)=y2
          kindex=where(abs(mtheta-rlev) le dlev and mh2o ne -99.)
          if kindex(0) eq -1L then goto,jumplev
...
syr1=syr
yrlab=syr0+'-'+syr1
save,file='mls_n2o+h2o_3dpdfs_'+yrlab+'.sav',copdf3d,h2opdf3d,th,kday,date_all,xbins
;
...
;
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='mls_n2o+h2o_pdfs_mix_vs_time_'+slev+'_'+yrlab+'_'+sver+'.ps'
         device,/color
...
;
      ...
      contour,copdf2d_time,xbins,findgen(kday),levels=level,title='N!l2!nO PDF',/noerase,/fill,c_color=col1,$
              xtitle='N!l2!nO (ppmv)',color=0,ytickv=xindex,yticks=nxtick,ytickname=' '+strarr(nxtick+1),xrange=[min(xbins),6.]
;     contour,copdf2d_time,xbins,findgen(kday),levels=level,/overplot,/follow,color=0,c_labels=0*level
      for ii=0L,n_elements(xindex)-1L do xyouts,6.25,xindex(ii),xlabs(ii),alignment=0.0,color=0,/data
;
; H2O PDF time vs mix
;
      !type=2^2+2^3
      ...
      contour,h2opdf2d_time,xbins,findgen(kday),levels=level,title='H2O PDF',/noerase,/fill,c_color=col1,$
              xtitle='H2O (ppmv)',color=0,ytickv=xindex,yticks=nxtick,ytickname=' '+strarr(nxtick+1),xrange=[2.,max(xbins)]
;     contour,h2opdf2d_time,xbins,findgen(kday),levels=level,/overplot,/follow,color=0,c_labels=0*level
      ...
      plot,[omin,omax],[0,0],yrange=[0,10],xrange=[omin,omax],xtitle='Frequency',$
            xticks=nlvls-1,xtickname=string(format='(f5.3)',level),$
            /noeras,xstyle=1,color=0
      ybox=[0,10,10,0,0]
      x1=omin
      dx=(omax-omin)/nlvls
      ...
      if setplot eq 'ps' then begin
         device,/close
         spawn,'convert -trim mls_n2o+h2o_pdfs_mix_vs_time_'+slev+'_'+yrlab+'_'+sver+'.ps -rotate -90 '+$
               'mls_n2o+h2o_pdfs_mix_vs_time_'+slev+'_'+yrlab+'_'+sver+'.jpg'
      endif
endfor
end
