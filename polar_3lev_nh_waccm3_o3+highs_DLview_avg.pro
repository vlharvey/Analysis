...
;      zindex=where(th ge 500.,nth2)
       zindex=where(th le 2000.,nth2)
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='Figures/3lev_nh_'+mon(imonth)+'DL.ps'
       device,/color
...
;
dum=dum/(xcn^2+ycn^2)
        index=where(dum lt -0.5,nl)

        if index(0) ne -1 then begin
           oplot,xcn(index),ycn(index),/T3D,zvalue=nz,psym=8,symsize=2,color=0
loadct,0
           contour,dum,xcn,ycn,levels=[-0.5],color=mcolor*.5,c_labels=0,/T3D,zvalue=nz,thick=3
;          contour,dum,xcn,ycn,levels=reverse(-0.3-0.1*findgen(6)),color=mcolor*.5,$
...
jumplev:
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim Figures/3lev_nh_'+mon(imonth)+'DL.ps -rotate -90 Figures/3lev_nh_'+mon(imonth)+'DL.jpg'
       spawn,'/usr/bin/rm Figures/3lev_nh_'+mon(imonth)+'DL.ps'
    endif
