...
;
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='yt_waccm_co+mark_'+slev+'_composite.ps'
         device,/color
...
imin=0.
if rlev eq 2000. then imin=-0.5
if rlev eq 1000. then imin=-0.1
;imax=max(plotarray(index))
if rlev eq 4000. then imax=8.
if rlev eq 3000. then imax=4.
if rlev eq 2000. then imax=2.
if rlev eq 1000. then imax=0.5
if rlev eq 500. then imax=0.02
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim yt_waccm_co+mark_'+slev+'_composite.ps -rotate -90 '+$
         'yt_waccm_co+mark_'+slev+'_composite.jpg'
endif
