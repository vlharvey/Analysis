...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='geos5_nh_sf_vs_pv_difflat_tz_yearly.ps'
endif
;
...
index=where( (syr eq strcompress(ii-1L,/remove_all) and strmid(smn,0,1) eq '1') or $
             ...
             (syr eq strcompress(ii,/remove_all) and smn eq '02') or $
...
!type=2^2+2^3
xmn=xorig(ii-2005)
xmx=xorig(ii-2005)+xlen
ymn=yorig(ii-2005)
ymx=yorig(ii-2005)+ylen
...
if ii eq 2005L then begin
   xyouts,xorig(ii-2005)-0.06,yorig(1)+ylen-0.015,'NH',/normal,color=0,charsize=2
   contour,sfdiff,findgen(kday),th,xrange=[0.,kday],yrange=[min(th),max(th)],c_color=col2,$
        ...
        xticks=nxticks-1,xtickv=xindex,xtickname=smn(xindex),title=syr0+'-'+syr1,charsize=0.8
;contour,sfdiff,findgen(kday),th,/overplot,levels=level
endif
if ii gt 2005 then begin
   contour,sfdiff,findgen(kday),th,xrange=[0.,kday],yrange=[min(th),max(th)],c_color=col2,$
...
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim geos5_nh_sf_vs_pv_difflat_tz_yearly.ps -rotate -90 '+$
         'geos5_nh_sf_vs_pv_difflat_tz_yearly.jpg'
;  spawn,'/usr/bin/rm geos5_nh_sf_vs_pv_difflat_tz_yearly.ps'
endif
end
