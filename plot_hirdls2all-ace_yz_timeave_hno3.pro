...
;
;restore,file='/aura3/data/HIRDLS_data/Analysis/HIRDLS2ALL_hno3_20060501-20060531.sav'
restore,file='/aura3/data/HIRDLS_data/Analysis/HIRDLS2ALL_hno3_v2.02.06.20060507-20060531.sav'
hdate0='20060501'
hdate1='20060531'
...
for k=0,n_elements(altitude)-1L do begin
    ...
    for kk=0L,n_elements(hirdls_pressure)-2L do begin
        ...
        if hirdls_HNO3Yp(j,kk) ne -99. and hirdls_HNO3Yp(j,kk+1L) ne -99. then begin
        if hz0 lt zp and hz1 ge zp then begin
           zscale=(hz1-zp)/(hz1-hz0)
           hirdls_hno3yz(j,k)=hirdls_HNO3Yp(j,kk+1)-zscale*(hirdls_HNO3Yp(j,kk+1L)-hirdls_HNO3Yp(j,kk))
;print,hz0,zp,hz1,zscale
;print,hirdls_HNO3Yp(j,kk),hirdls_hno3yz(j,k),hirdls_HNO3Yp(j,kk+1L)
;stop
        endif
        endif
    endfor
...
if setplot eq 'ps' then begin
   ...
   ysize=nydim/100.
   device,font_size=9
   device,/landscape,bits=8,filename='hirdls2allv2.02.06-ace_yz_'+adate0+'-'+adate1+'_hno3.ps'
   device,/color
   device,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize
endif

erase
hdata=HIRDLS_hno3YZ*1.e9
adata=ace_hno3YZ*1.e9
ddata=diff_hno3YZ
...
index=where(hdata le 1. or adata le 1. or ddata eq 0.)
if index(0) ne -1L then ddata(index)=-99999.
level=[0.5*findgen(15),8.,10.,12.5,15.]
...
;
    ...
    xlabs=' '+strarr(7)
    contour,hdata,latbin,altitude,xrange=[-90.,90.],yrange=[10.,40.],xticks=6,ytitle='Altitude (km)',$
            xtickname=xlabs,charsize=2.5,levels=level,/cell_fill,$
            title='HIRDLS v2.02.06 HNO!l3!n '+hdate0+'-'+hdate1,c_color=col1,color=0,min_value=-99999.
...
;   contour,ddata,latbin,altitude,/overplot,levels=[0],color=mcolor,/follow,min_value=-99999.,thick=3
    ...
    nlvls=n_elements(level)
    col1=1+indgen(nlvls)*mcolor/nlvls
    contour,adata,latbin,altitude,xrange=[-90.,90.],yrange=[10.,40.],xticks=6,ytitle='Altitude (km)',$
            xtickname=xlabs,charsize=2.5,levels=level,/cell_fill,$
            title='ACE HNO!l3!n '+adate0+'-'+adate1,c_color=col1,color=0,min_value=-99999.
...
if x(0) ne -1L then ddata(x)=-99999.
    ...
    contour,ddata,latbin,altitude,xrange=[-90.,90.],yrange=[10.,40.],xticks=6,$
            xtickname=xlabs,charsize=2.5,levels=dlevel,/cell_fill,ytitle='Altitude (km)',$
            title='HIRDLS-ACE HNO!l3!n Difference (%)',c_color=col1,color=0,min_value=-99999.,xtitle='Latitude'
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim hirdls2allv2.02.06-ace_yz_'+adate0+'-'+adate1+'_hno3.ps -rotate -90 hirdls2allv2.02.06-ace_yz_'+adate0+'-'+adate1+'_hno3.jpg'
endif
end
