;
; enter altitude and plot MLS CO and GEOS-4 Antarctic vortex
;
@stddat
@kgmt
@ckday
...
if max(pv1) eq 1.00000e+12 then print,'theta surface is discontinuous'
      sf1=transpose(sf2(*,*,ilev))
      u1=transpose(u2(*,*,ilev))
      p1=transpose(p2(*,*,ilev))
      t1=th(ilev)*((p1/1000.)^(.286))
...
;
      index=where(muttime gt 0. and mlat lt 0.)
...
;
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='polar_mls_co+geos4_edge_sh_'+sdate+'_'+slev+'.ps'
         device,/color
...
;
      ...
      map_set,-90,0,-90,/ortho,/contin,/grid,color=0,/noeras,charsize=1.5,$
              title='MLS CO + GEOS-4 Antarctic Vortex '+sdate+' '+slev
      contour,mark,x,alat,/overplot,levels=[0.1],color=0,thick=10,/noeras,/follow
      omin=-0.5 & omax=3.
;omin=min(codata) & omax=max(codata)
;print,omin,omax
print,min(z1(*,nr/2:nr-1)),max(z1(*,nr/2:nr-1))
      ...
      contour,mark,x,alat,/overplot,levels=[0.1],color=0,thick=12,/noeras,/follow,c_labels=0
;     contour,sf,x,alat,/overplot,nlevels=20,color=0,thick=3,/noeras,/follow,c_labels=0
      contour,speed,x,alat,/overplot,levels=50.+20.*findgen(20),color=0,thick=3,/noeras,/follow,c_labels=0
...
loadct,0
;     contour,z,x,alat,/overplot,levels=40.+findgen(100),color=150,thick=3,/noeras,/follow,c_charsize=2
;     contour,pv,x,alat,/overplot,nlevels=20,color=130,thick=3,/noeras,/follow,c_charsize=2
loadct,38
      ...
      if setplot eq 'ps' then begin
         device,/close
         spawn,'convert -trim polar_mls_co+geos4_edge_sh_'+sdate+'_'+slev+'.ps -rotate -90 '+$
               'polar_mls_co+geos4_edge_sh_'+sdate+'_'+slev+'.jpg'
      endif
goto,jump
end
