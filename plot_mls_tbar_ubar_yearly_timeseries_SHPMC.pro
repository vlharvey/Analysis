...
;
zindex=where(p le 100. and p ge 0.00100000,nlev)
;ralt=0.001
;print,p
...
if setplot eq 'ps' then begin
   ...
   device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/times,filename='mls_tbar_ubar_yearly_timeseries_SHPMC_'+slat+'_'+slev+'.ps'
   !p.charsize=1.25
   !p.thick=2
   !p.charthick=5
   !y.thick=2
...
plot,-40.+findgen(121),findgen(121),/nodata,xrange=[-40.,80.],yrange=[tmin,tmax],/noeras,charsize=2,charthick=2,color=0,ytitle='MLS Tbar (K)',title=slat+' '+slev+' hPa ('+salt+' km)',xtitle='DFS (21 Dec)'
dfsindex=where(dfs eq -40.,nseason)
...
dfsyear=dum
    index=where(finite(tdatayear) eq 1L)
    oplot,dfsyear(index),smooth(tdatayear(index),7,/edge_truncate),color=col1(iseason),thick=10
endfor
...
plot,-40.+findgen(121),findgen(121),/nodata,xrange=[-40.,80.],yrange=[umin,umax],/noeras,charsize=2,charthick=2,color=0,ytitle='MLS Ubar (m/s)'
for iseason=0L,nseason-1L do begin
    if dfsindex(iseason)+121L lt n_elements(sdate) then dfsyear=reform(dfs(dfsindex(iseason):dfsindex(iseason)+121L))
    if dfsindex(iseason)+121L lt n_elements(sdate) then udatayear=reform(udata(dfsindex(iseason):dfsindex(iseason)+121L))
    if dfsindex(iseason)+121L ge n_elements(sdate) then dfsyear=reform(dfs(dfsindex(iseason):n_elements(sdate)-1L))
...
dfsyear=dum
    index=where(finite(udatayear) eq 1L)
    oplot,dfsyear(index),smooth(udatayear(index),7,/edge_truncate),color=col1(iseason),thick=10
endfor
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim mls_tbar_ubar_yearly_timeseries_SHPMC_'+slat+'_'+slev+'.ps -rotate -90 mls_tbar_ubar_yearly_timeseries_SHPMC_'+slat+'_'+slev+'.jpg'
endif
