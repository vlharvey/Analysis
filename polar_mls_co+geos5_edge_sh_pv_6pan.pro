;
; enter altitude and plot MLS CO and GEOS-5 Antarctic vortex
...
ledmn=12L & leddy=1L & ledyr=2006L
lstday=0L & ledday=0L
;
; get date range
;
...
;
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='polar_mls_co+geos5_edge_sh_pv_'+sdate+'_'+slabs(itime)+'_'+slev+'.ps'
         device,/color
...
;     slevel=reverse(-1*3000.*findgen(20))/1.e6 
...
;
      ...
      ymx=yorig(4)+ylen
      set_viewport,xmn,xmx,ymn,ymx
      plot,codata,h2odata,psym=8,color=0,xrange=[-1,4],yrange=[4,8],symsize=0.25,$
...
;
      ...
      if setplot eq 'ps' then begin
         device,/close
         spawn,'convert -trim polar_mls_co+geos5_edge_sh_pv_'+sdate+'_'+slabs(itime)+'_'+slev+'.ps -rotate -90 '+$
               'polar_mls_co+geos5_edge_sh_pv_'+sdate+'_'+slabs(itime)+'_'+slev+'.jpg'
      endif
