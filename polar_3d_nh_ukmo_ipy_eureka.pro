...
; save postscript version
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='polar_3d_'+sdate+'_eureka.ps'
         device,/color
...
;           label=1,lonlab=1,charsize=3,latdel=180.,/t3d,zvalue=nz(kk),color=0
        MAP_SET,90,0,0,/stereo,/contin,/grid,/noborder,/noeras,londel=90.,$
            label=1,lonlab=1,charsize=2,latdel=180.,/t3d,zvalue=nz(kk),color=0
;
...
yy=69.3 & xx=16.
               ...
               a=findgen(8)*(2*!pi/8.)
               usersym,2*cos(a),2*sin(a),/fill
;              oplot,[xn,xn],[yn,yn],zvalue=nz(kk),/T3D,psym=8,color=75
               dist=nz(kp1)-nz(kk)
               if dist gt 0. then begin
;              for m=0,10 do $
;                  oplot,[xn,xn],[yn,yn],zvalue=nz(kk)+float(m)*dist/11.,/T3D,psym=8,color=75
               endif
;
...
yy=80. & xx=274.
               ...
               a=findgen(8)*(2*!pi/8.)
               usersym,2*cos(a),2*sin(a),/fill
               oplot,[xn,xn],[yn,yn],zvalue=nz(kk),/T3D,psym=8,color=150
               dist=nz(kp1)-nz(kk)
               if dist gt 0. then begin
               for m=0,10 do $
                   oplot,[xn,xn],[yn,yn],zvalue=nz(kk)+float(m)*dist/11.,/T3D,psym=8,color=150
               endif
;
...
;                  oplot,[xn,xn],[yn,yn],zvalue=nz(kk)+float(m)*dist/11.,/T3D,psym=8,color=mcolor*.95
        ...
        xyouts,.05,nz4,thlevs(kk),color=0,/normal,charsize=2,charthick=2
    endfor	; loop over stacked polar plots
    !psym=0
    xyouts,0.33,0.9,date,/normal,charsize=3.0,color=0,charthick=2
    xyouts,.08,.78,'Theta (K)',charsize=2,/normal,color=0,charthick=2
    xyouts,.76,.78,'Altitude (km)',charsize=2,/normal,color=0,charthick=2
;   xyouts,.8,.13,'Alomar (69N,16E)',charsize=1.5,/normal,color=25,charthick=2
    xyouts,.8,.11,'Eureka (80N,86W)',charsize=1.5,/normal,color=150,charthick=2
...
;   xyouts,.75,.07,'Poker Flat (65N,147W)',charsize=1.5,/normal,color=mcolor*.95,charthick=2
    ...
    plot,[imin,imax],[0,0],yrange=[0,10],$
          ...
          xstyle=1,xticks=9,charsize=1.5,color=0,charthick=2
    ybox=[0,10,10,0,0]
    x1=imin
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim polar_3d_'+sdate+'_eureka.ps -rotate -90 polar_3d_'+sdate+'_eureka.jpg'
       spawn,'/usr/bin/rm -f polar_3d_'+sdate+'_eureka.ps'
    endif
goto, jump
end
