;
; plot two NOx profiles for Erin Wood
; VLH 3/2/09
;
@rd_ukmo_nc3
@stddat
...
;
      if setplot eq 'ps' then begin
         ...
         device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
                /bold,/color,bits_per_pixel=8,/helvetica,filename='ace_nox_profs_'+sdate+'.ps'
         !p.charsize=1.25
         !p.thick=2
         !p.charthick=5
         !p.charthick=5
...
;     plot,noxace(0,*),altitude,title=sdate,/xlog,xrange=[1.e-4,1.],yrange=[20.,80.],$
;          xtitle='NOx (ppmv)',ytitle='Altitude (km)',color=0,/nodata
;
...
nhnox_prof=smooth(nhnox_prof,11)
shnox_prof=smooth(shnox_prof,11)
      oplot,nhnox_prof,altitude,color=mcolor*.3,thick=15
      oplot,shnox_prof,altitude,color=mcolor*.9,thick=15
      slatnh=strcompress(string(format='(f5.1)',nhymean),/remove_all)+' N'
      slatsh=strcompress(string(format='(f5.1)',shymean),/remove_all)+' S'
      xyouts,0.1,30.,slatnh,/data,color=mcolor*.3
      xyouts,0.1,25.,slatsh,/data,color=mcolor*.9

      if setplot ne 'ps' then stop
      if setplot eq 'ps' then begin
         device, /close
         spawn,'convert -trim ace_nox_profs_'+sdate+'.ps -rotate -90 ace_nox_profs_'+sdate+'.jpg'
         spawn,'/usr/bin/rm ace_nox_profs_'+sdate+'.ps'
      endif
      icount=icount+1L
goto,jump
end
