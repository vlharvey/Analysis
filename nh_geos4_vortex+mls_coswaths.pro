...
lstmn=12L & lstdy=1L & lstyr=2005L
...
;
      index=where(muttime gt 0. and mlat gt 25.,mprof)
...
;
;   dum=findfile(idir+'geos4_mark_at_mls_'+sdate+'.sav')
;   if dum(0) ne '' then begin
;      restore,idir+'geos4_mark_at_mls_'+sdate+'.sav'
;      goto,jumpint
;   endif
;
...
jumpz1:
    ...
    save,file=idir+'geos4_mark_at_mls_'+sdate+'.sav',gmark,altitude
jumpint:
;
...
;
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='nh_geos4_vortex+mls_coswaths_'+sdate+'.ps'
       device,/color
...
;
    erase
    xyouts,0.2,0.9,'MLS CO + MetO and GEOS-4 Arctic Vortex '+sdate,/normal,charsize=2.0,color=0,charthick=2
    !type=2^2+2^3
...
;       level=-10.+0.5*findgen(41)	; for copr
        ...
        contour,coswath,xswath,altitude,levels=level,/fill,c_color=col1,charsize=1.5,$
...
;       contour,gmarkswath,xswath,altitude,levels=[-0.2],color=0,/follow,/overplot,thick=3,c_labels=0
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim nh_geos4_vortex+mls_coswaths_'+sdate+'.ps -rotate -90 '+$
             'nh_geos4_vortex+mls_coswaths_'+sdate+'.jpg'
;      spawn,'/usr/bin/rm -f nh_geos4_vortex+mls_coswaths_'+sdate+'.ps'
    endif
    icount=icount+1L
goto,jump
end
