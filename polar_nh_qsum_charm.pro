; 
; polar plots of WACCM total ion production (QSUM) in support of CHARM.
;
@stddat
@kgmt
@ckday
...
;             if result.name eq 'O3' then  o3grd=data
              ...
              if result.name eq 'QSUM' then  qsumgrd=data	; /s
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='Figures/polar_nh_qsum_charm_'+sdate+'_'+slev+'.ps'
       device,/color
...
;   noy1=reform(noygrd(*,*,ilev))*1.e6
    noy1=reform(qsumgrd(*,*,ilev))
if max(noy1) eq 0. then goto,jumplev
...
if xlon gt 360. then xlon=xlon-360.
    ...
    !type=2^2+2^3+2^6
    plot,[imin,imax],[0,0],yrange=[0,10],xrange=[imin,imax],xtitle='Total Ion Production (s-1)',charsize=1.5,color=0
    ybox=[0,10,10,0,0]
    x1=imin
...
;   if setplot ne 'ps' then stop
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim Figures/polar_nh_qsum_charm_'+sdate+'_'+slev+'.ps -rotate -90 Figures/polar_nh_qsum_charm_'+sdate+'_'+slev+'.jpg'
       spawn,'rm -f Figures/polar_nh_qsum_charm_'+sdate+'_'+slev+'.ps'
    endif
