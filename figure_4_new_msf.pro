...
;
      ...
      for ilev=1L,nth-2L do begin
;         print,th(ilev),min(z2(60,*,ilev)),mean(z2(60,*,ilev)),max(z2(60,*,ilev)),(mean(z2(60,*,ilev-1))-mean(z2(60,*,ilev+1)))/2.
          print,th(ilev),z2(60,0,ilev),(z2(60,0,ilev-1)-z2(60,0,ilev+1))/2.,stdev(z2(60,*,ilev))
      endfor
stop
;
; read new vortex
...
;
      if setplot eq 'ps' then begin
         ...
         device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
          /bold,/color,bits_per_pixel=8,/helvetica,filename='figure_4_'+sdate+'_new_msf.ps'
      endif
;
...
;     xyouts,.41,.92,sdate,/normal,color=0,charsize=1.5
      ...
      plot,findgen(10),findgen(10),/nodata,xrange=[sfmin,sfmax],yrange=[pvmin,pvmax],$
           /ylog,color=0,xtitle='!4W!1  (m!u2!ns!u-2!n)',ytitle='Mean PV (Km!u-2!nkg!u-1!ns!u-1!n)',xticks=4,charsize=1,$
      ...
      for ilev=nth-1L,0L,-1 do begin
          sf=transpose(msf2(*,*,ilev))
...
;oplot,sf(index),pv(index),psym=8,color=col2(ilev),symsize=.5
;             if th(ilev) eq thlev or th(ilev) eq thlev1 then $
;                oplot,sf(index),pv(index),psym=8,color=0,symsize=.5
          ...
          if index(0) ne -1L then begin
             ...
             oplot,sfbins(index),meanpv(index),psym=0,color=col2(ilev),thick=10
;for ii=0L,n_elements(index)-1L do begin
...
ilev=5
      ...
      if index(0) ne -1L then begin
         ...
         if th(ilev) eq thlev or th(ilev) eq thlev1 then $
            oplot,sfbins(index),meanpv(index),color=0,psym=0,linestyle=2,thick=5
      endif
;
      imin=min(th)
      imax=max(th)
...
col1=1+(indgen(nlvls)/float(nlvls))*mcolor
      ...
      if setplot eq 'ps' then begin
         device,/close
         spawn,'convert -trim figure_4_'+sdate+'_new_msf.ps -rotate -90 figure_4_'+sdate+'_new_msf.jpg'
;        spawn,'/usr/bin/rm figure_4_'+sdate+'_new_msf.ps'
      endif
