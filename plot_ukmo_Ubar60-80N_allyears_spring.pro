...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='ukmo_Ubar60to80N_allyears_spring_'+sth+'K.ps'
   device,/color
...
plot,[1,kday,kday,1,1],[amin,amin,amax,amax,amin],min_value=0.,color=0,$
      ...
      ytitle='Zonal Mean U 60-80N',xtickname=smon,xticks=n_elements(smon)-1L,$
      title=sth+' K'
...
for iyear=nyear-3L,nyear-1L do begin
    ...
    if iyear lt nyear-1L then oplot,1.+findgen(kday),smax_ave,color=col1(iyear-(nyear-3L)),thick=10,min_value=-9999.
    if iyear eq nyear-1L then oplot,1.+findgen(118),smax_ave(0:117),color=col1(iyear-(nyear-3L)),thick=10,min_value=-9999.
    xyouts,kday-10.,amax-12.-(iyear-(nyear-3L))*yinc,slab(iyear-(nyear-3L)),charsize=2,color=col1(iyear-(nyear-3L)),/data,charthick=2
endfor
for i=0L,30 do xyouts,i+1,smax_ave(i),strcompress(i+1,/remove_all),/data,color=0,alignment=0.5

if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ukmo_Ubar60to80N_allyears_spring_'+sth+'K.ps -rotate -90 ukmo_Ubar60to80N_allyears_spring_'+sth+'K.jpg'
endif
end
