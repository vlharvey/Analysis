pro marker_lows_pv_v8,sf,mark,qdf,zeta,u,v,x,y,theta
...
; choose the "candidate" with largest wind speed
   ...
   if is gt 2 and min(lat(s))-latmin gt dy then begin
      mark(s)=1.0
      print,'Arctic vortex PV ',theta,max(wnode(iindex))
   endif
...
;return
;
; multiply CO by -1 to adhere to minimum in vortex like PV
;
sf=-1.*sf
