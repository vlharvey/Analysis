;
; plot ACE zonal mean temperature
;
loadct,38
...
if setplot eq 'ps' then begin
   ...
   ysize=nydim/100.
   device,font_size=9
   device,/landscape,bits=8,filename='ace_yz_temp_'+sdate0+'-'+sdate1+'.ps'
   device,/color
...
if index(0) ne -1L then $
   o3array(index)=o3array(index)/float(no3array(index))
...
ymx=yorig(0)+ylen
set_viewport,xmn,xmx,ymn,ymx
level=[120.+10.*findgen(19),350.,400.]
...
contour,o3array,latbin,altitude,xrange=[-90.,90.],yrange=[1.,120.],xticks=6,$
        ...
        title='Zonal Mean ACE Temperature',c_color=col1,color=0,min_value=-99.
...
xyouts,-30.,5.,sdate0+' - '+sdate1,charsize=2,/data,color=0
imin=min(level)
imax=max(level)
xmnb=xmx+.07
xmxb=xmnb+.01
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ace_yz_temp_'+sdate0+'-'+sdate1+'.ps -rotate -90 ace_yz_temp_'+sdate0+'-'+sdate1+'.jpg'
endif
end
