...
; (mol NO/mol air) * (1 molecule air/29 grams) * (1000 g air/1 kg air) * (AIR DENSITY/m^3 air) * (Avagadros #/1 mole air) = (molecules NO/m^3 air)
; Avagadros # = 6.022e23
;
...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='polar_sh_no+e_'+sdate+'_'+slev0+'_dens_WA3548T08CO.ps'
       device,/color
...
print,'Max Speed at ',lev(ilev),zavg,max(speed)
    ...
    !type=2^2+2^3
    xyouts,.15,.8,'WA3548T08CO '+sdate+' '+stime+' UT       '+slev+' ('+szavg+')',/normal,color=0,charsize=1.5
...
;velovect,u,v,x,alat,color=0,/overplot,length=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim polar_sh_no+e_'+sdate+'_'+slev0+'_dens_WA3548T08CO.ps -rotate -90 polar_sh_no+e_'+sdate+'_'+slev0+'_dens_WA3548T08CO.jpg'
       spawn,'rm -f polar_sh_no+e_'+sdate+'_'+slev0+'_dens_WA3548T08CO.ps'
    endif
