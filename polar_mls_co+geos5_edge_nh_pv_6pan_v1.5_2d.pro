...
lstmn=10L & lstdy=10L & lstyr=2004L
...
;
      ...
      for k=0L,nth-1L do begin
          ...
          copdf2d(*,k)=y2
          kindex=where(abs(mtheta-rrlev) le 50. and mh2o ne -99.)
          if kindex(0) eq -1L then goto,jumplev
...
;
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='polar_mls_co+geos5_edge_nh_pv_'+sdate+'_2d_v1.5.ps'
         device,/color
...
;
      ...
      ymx=yorig(2)+ylen
      set_viewport,xmn,xmx,ymn,ymx
      plot,codata,h2odata,psym=8,color=0,xrange=[-1,4],yrange=[3,8],symsize=0.25,$
           xtitle='CO',ytitle='H2O',/nodata,title='Scatter Plot'
      ...
      contour,nfreq,-1.+.5*findgen(19),-1.+.5*findgen(19),levels=[5.],color=0.1*mcolor,/follow,/overplot,thick=4
      contour,nfreq,-1.+.5*findgen(19),-1.+.5*findgen(19),levels=[10.],color=0.2*mcolor,/follow,/overplot,thick=4
      contour,nfreq,-1.+.5*findgen(19),-1.+.5*findgen(19),levels=[20.],color=0.3*mcolor,/follow,/overplot,thick=4
      contour,nfreq,-1.+.5*findgen(19),-1.+.5*findgen(19),levels=[30.],color=0.4*mcolor,/follow,/overplot,thick=4
      contour,nfreq,-1.+.5*findgen(19),-1.+.5*findgen(19),levels=[40.],color=0.5*mcolor,/follow,/overplot,thick=4
      contour,nfreq,-1.+.5*findgen(19),-1.+.5*findgen(19),levels=[50.],color=0.6*mcolor,/follow,/overplot,thick=4
      contour,nfreq,-1.+.5*findgen(19),-1.+.5*findgen(19),levels=[75.],color=0.7*mcolor,/follow,/overplot,thick=4
      contour,nfreq,-1.+.5*findgen(19),-1.+.5*findgen(19),levels=[100.],color=0.8*mcolor,/follow,/overplot,thick=4
      contour,nfreq,-1.+.5*findgen(19),-1.+.5*findgen(19),levels=[125.],color=0.9*mcolor,/follow,/overplot,thick=4
      contour,nfreq,-1.+.5*findgen(19),-1.+.5*findgen(19),levels=[150.],color=0.95*mcolor,/follow,/overplot,thick=4
print,max(nfreq)
;
; CO and H2O PDFs
;
      ...
      !linetype=0
      xbins=-1.+.25*findgen(37)
      plot,xbins,y1,xtitle='(ppmv)',ytitle='Frequency',charsize=1.2,$
...
;
      ...
      nlvls=n_elements(level)
      col1=1+indgen(nlvls)*mcolor/nlvls
      contour,copdf2d,xbins,th,levels=level,title='2D CO PDF',/noerase,/fill,c_color=col1,$
              ytitle='Theta (K)',xtitle='CO (ppmv)',color=0
      contour,copdf2d,xbins,th,levels=level,/overplot,/follow,color=0,c_labels=0*level
      set_viewport,xmx+cbaryoff,xmx+cbaryoff+cbarydel,ymn,ymx
      !type=2^2+2^3+2^5
      omin=min(level)
      omax=max(level)
      plot,[0,0],[omin,omax],xrange=[0,10],yrange=[omin,omax],title='Frequency',color=0
      xbox=[0,10,10,0,0]
      y1=omin
      dy=(omax-omin)/float(nlvls)
      for j=0,nlvls-1 do begin
...
;
      ...
      ymx=yorig(5)+ylen
      set_viewport,xmn,xmx,ymn,ymx
      contour,h2opdf2d,xbins,th,levels=level,title='2D H2O PDF',/noerase,/fill,c_color=col1,$
              ytitle='Theta (K)',xtitle='H2O (ppmv)',color=0
      ...
      if setplot eq 'ps' then begin
         device,/close
         spawn,'convert -trim polar_mls_co+geos5_edge_nh_pv_'+sdate+'_2d_v1.5.ps -rotate -90 '+$
               'polar_mls_co+geos5_edge_nh_pv_'+sdate+'_2d_v1.5.jpg'
      endif
