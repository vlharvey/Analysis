...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='polar_nh_noy_charm_'+sdate+'_'+slev+'_diff_dens.ps'
       device,/color
...
;   noy1=reform(noygrd(*,*,ilev))*1.e6
    ...
    MAP_SET,90,0,-90,/ortho,/noeras,/grid,/contin,title='NOy',charsize=1.5,color=0,/noborder	;,limit=[60.,0.,90.,360.]
;   MAP_SET,0,0,0,/noeras,/grid,/contin,title='NOy',charsize=1.5,color=0,/noborder
...
;    contour,z,x,alat,/overplot,levels=10+findgen(150),/follow,c_labels=0*findgen(150),/noeras,color=mcolor
    ...
    contour,q,x,alat,/overplot,levels=[1000.,3000.],/follow,c_labels=0,/noeras,color=mcolor,thick=3
...
if xlon gt 360. then xlon=xlon-360.
    ...
    set_viewport,xmn,xmx,ymn,ymx
    !type=2^2+2^3
    MAP_SET,90,0,-90,/ortho,/noeras,/grid,/contin,title='NOy Difference',charsize=1.5,color=0,/noborder	;,limit=[60.,0.,90.,360.]
;   MAP_SET,0,0,0,/noeras,/grid,/contin,title='NOy Difference',charsize=1.5,color=0,/noborder
...
;velovect,u,v,x,alat,color=0,/overplot,length=2
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim polar_nh_noy_charm_'+sdate+'_'+slev+'_diff_dens.ps -rotate -90 polar_nh_noy_charm_'+sdate+'_'+slev+'_diff_dens.jpg'
       spawn,'rm -f polar_nh_noy_charm_'+sdate+'_'+slev+'_diff_dens.ps'
    endif
endfor	; loop over pressure

jumpstep:
if iday eq 2 then noyold=noy_conc
