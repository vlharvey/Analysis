...
;
    if setplot eq 'ps' then begin
       ...
       device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
              /bold,/color,bits_per_pixel=8,/helvetica,filename='figure_4_zt_TUMark_NH_SH_smidemax_4pan.ps'
       !p.charsize=1
       !p.thick=2
       !p.charthick=2
       !y.thick=2
...
;
    ...
    contour,tnhshift,findgen(ndays),znhshift,/noera,/fill,color=0,c_color=col1,levels=tlevel,xrange=[0,ndays-1],yrange=[30,125],ytitle='Altitude (km)',charsize=1,charthick=2,title='NH',$
...
mnhshift=smooth(mnhshift,3,/edge_truncate)
    ...
    ymx=yorig(1)+ylen
    set_viewport,xmn,xmx,ymn,ymx
    contour,tsh,findgen(ndays),zsh,/noera,/fill,color=0,c_color=col1,levels=tlevel,xrange=[0,ndays-1],yrange=[30,125],ytitle='Altitude (km)',charsize=1,charthick=2,title='SH',$
...
xyouts,x0,y0-0.03,'    60-90 Temperature (K)',color=0,charsize=1.25,charthick=2,/normal
...
;
    ...
    nlvls=n_elements(tlevel)
    col1=(findgen(nlvls)/float(nlvls))*mcolor
    contour,unhshift,findgen(ndays),znhshift,/noera,/fill,color=0,c_color=col1,levels=tlevel,xrange=[0,ndays-1],yrange=[30,125],charsize=1,charthick=2,title='NH',$
            xticks=nxticks-1,xtickname=xlabshift,xtickv=xindex
    ...
    ymx=yorig(3)+ylen
    set_viewport,xmn,xmx,ymn,ymx
    contour,ush,findgen(ndays),zsh,/noera,/fill,color=0,c_color=col1,levels=tlevel,xrange=[0,ndays-1],yrange=[30,125],charsize=1,charthick=2,title='SH',$
            xticks=nxticks-1,xtickname=xlabs,xtickv=xindex
    index=where(tlevel gt 0.)
...
plot,[0,0],[0,0],yrange=[0,10],xrange=[0,1],/noeras,xticks=nlvls-1L,$
        position = [x0,y0,x1,y1],xstyle=1,xtickname=slab,/nodata,color=0
xyouts,x0,y0-0.03,' 60 degree Zonal Wind (m/s)',color=0,charsize=1.25,charthick=2,/normal
...
;
    ...
    if setplot eq 'ps' then begin
       device, /close
       spawn,'convert -trim figure_4_zt_TUMark_NH_SH_smidemax_4pan.ps -rotate -90 figure_4_zt_TUMark_NH_SH_smidemax_4pan.jpg
;      spawn,'rm -f figure_4_zt_TUMark_NH_SH_smidemax_4pan.ps'
    endif
