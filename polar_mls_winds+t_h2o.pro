...
; Read MERRA
;
        ncfile0='/Volumes/Data/MERRA_data/Datfiles/MERRA-on-WACCM_theta_'+sdate2+'.nc3'
        rd_merra_nc3,ncfile0,nc,nr,nth,alon,alat,th,pv2,p2,$
           u2,v2,qdf2,mark2,qv2,z2,sf2,q2,iflag
      restore,dir+sdate2+'.sav'
...
;
    if setplot eq 'ps' then begin
       ...
       ysize=nydim/100.
       set_plot,'ps'
       device,/color,/landscape,bits=8,filename='polar_mls_t+h2o_'+sdate+'_'+slev+'.ps'
...
smean=strcompress(round(zmean))
    erase
    xyouts,.4,.75,sdate,/normal,color=0,charsize=4
    !type=2^2+2^3
    xmn=xorig(0)
    xmx=xorig(0)+xlen
    ymn=yorig(0)
...
;   level=[0.01,0.1,0.25,0.5,1,1.5,2,2.5,3,4,5,6,7,10,15,20,30,50]		; good at 0.01
    ...
    map_set,90,0,-90,/ortho,/contin,/grid,/noerase,color=0,charsize=1.5,limit=[40.,0.,90.,360.]
    tp=smooth(tp,5,/edge_truncate)
    contour,tp,x,alat,levels=level,c_color=col1,/cell_fill,/overplot,/noeras
    contour,tp,x,alat,levels=level,color=0,/follow,/overplot,/noeras,c_labels=0*level
    map_set,90,0,-90,/ortho,/contin,/grid,/noerase,color=0,charsize=1.5,limit=[40.,0.,90.,360.]
oplot,[10.13,10.13],[54.33,54.33],psym=8,color=mcolor
    imin=min(level)
    imax=max(level)
    set_viewport,xmn,xmx,ymn-cbaryoff,ymn-cbaryoff+cbarydel
    !type=2^2+2^3+2^6
    plot,[imin,imax],[0,0],yrange=[0,10],$
          xrange=[imin,imax],xtitle='MLS Temperature (K) at ~'+smean+' km',/noeras,$
    ...
    ymx=yorig(1)+ylen
    set_viewport,xmn,xmx,ymn,ymx
    level=1+0.175*findgen(21)		; good at 0.1
...
h2o=smooth(h2o,5,/edge_truncate)
    ...
    contour,h2o,x,alat,levels=level,color=0,/follow,/overplot,/noeras,c_labels=0*level
    map_set,90,0,-90,/ortho,/contin,/grid,/noerase,color=0,charsize=1.5,limit=[40.,0.,90.,360.]
oplot,[10.13,10.13],[54.33,54.33],psym=8,color=mcolor
    imin=min(level)
    imax=max(level)
    ...
    !type=2^2+2^3+2^6
    plot,[imin,imax],[0,0],yrange=[0,10],$
          xrange=[imin,imax],xtitle='MLS H!l2!nO (ppmv) at ~'+smean+' km',/noeras,$
          xstyle=1,charsize=1.5,color=0,charthick=5
    ybox=[0,10,10,0,0]
    x1=imin
    dx=(imax-imin)/float(nlvls)
...
; Close PostScript file and return control to X-windows
     ...
     if setplot eq 'ps' then begin
        device, /close
        spawn,'convert polar_mls_t+h2o_'+sdate+'_'+slev+'.ps -rotate -90 '+$
                            'polar_mls_t+h2o_'+sdate+'_'+slev+'.jpg'
;       spawn,'rm -f polar_mls_t+h2o_'+sdate+'_'+slev+'.ps'
     endif
     jumpday:
goto,jump
end
