...
lstmn=11L & lstdy=10L & lstyr=2004L
...
;
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='polar_mls_co+geos5_edge_sh_pv_'+sdate+'_'+slabs(0)+'_'+slev+'_v1.5.ps'
         device,/color
...
;
      ...
      comin=-0.5
      if rlev le 2000. then comax=2.
      if rlev gt 2000. then comax=5.
...
;
      ...
      omax=8.
      if rlev gt 2000. then omin=3.
      if rlev le 2000. then omin=6.
      for i=0L,mprof-1L do begin
...
;
      ...
      !linetype=0
      xx=-1.+.25*findgen(37)
      plot,xx,y1,xtitle='(ppmv)',ytitle='Frequency',charsize=1.2,$
           title='PDFs',xrange=[-1.,8.],yrange=[0.,ymax],color=0
      oplot,xx,y1,color=.3*mcolor,thick=3
      y2=histogram(codata,min=-1,max=8.,binsize=.25)/float(n_elements(codata))
      oplot,xx,y2,color=.9*mcolor,thick=3
...
;        plots,vavg_co+vsig_co,ymax,color=mcolor*.9,/continue,linestyle=5,thick=2
      ...
      if setplot eq 'ps' then begin
         device,/close
         spawn,'convert -trim polar_mls_co+geos5_edge_sh_pv_'+sdate+'_'+slabs(0)+'_'+slev+'_v1.5.ps -rotate -90 '+$
               'polar_mls_co+geos5_edge_sh_pv_'+sdate+'_'+slabs(0)+'_'+slev+'_v1.5.jpg'
         spawn,'/usr/bin/rm -f polar_mls_co+geos5_edge_sh_pv_'+sdate+'_'+slabs(0)+'_'+slev+'_v1.5.ps'
      endif
      jumplev:
