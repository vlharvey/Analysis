PRO read_3s, out, label, sfdu, FILE = file, SWAP = swap
...
;
; PURPOSE:
;	This is an IDL program that will read UARS level 3AS or 3BS files which
;	are archived in UNIX format on an SGI (big-endian) platform at NASA's
;	Goddard DAAC.
;
; CALLING SEQUENCE:
; 	READ_3S, Data [, Label [, Sfdu] ] 
;
; INPUTS:
;	None.
;
...
;
; EXAMPLES:
;	Read level 3AS/3BS file, return just data records, and supply file name.
;
;		READ_3S, x, file='SUSIM_L3BS_D0123.V0016_C01_PROD'
;
;	Read level 3AS/3BS file, return all records, and prompt for file name.
;
;		READ_3S, x, h, s	; Program prompts for file name.
;
...
if n_params(0) eq 0 then begin		; check if required output was supplied.
  message, "usage: READ_3S, data [, label [, sfdu ] ]"
  goto, bye
endif
...
;  NOTE: This might be a 3AS or a 3TP file.  Can't be sure which yet.
;  Determine the record length manually from the PROD file.
  ...
  if (string(rec(0:7)) eq "! TYPE :") then begin	; You have a META file
   ...
   if (string(meta(i:i+2)) ne '3AS' and string(meta(i:i+2)) ne '3BS') then begin
     goto, message3
   endif
...
l3= {label3, satellite_id:'    ', record_type:'  ', $
     ...
     data_level:'   ', uars_day:'    ', num_data_pts:'    ', $
     base_wavelen:'      ', rec_length:'     ', ccb_version:'         ', $
     file_cycle_num:'     ', virtual_flag:' ', time_ver_file:'    ', $
     time_ver_rec:'    '}

; Stuff beyond file_cycle_num is never used.
;    file_cycle_num:'     ', virtual_flag:' ', num_entry_file:'    ', $
;    num_entry_rec:'    '}
...
; Make sure it is 3AS or 3BS.
if (l3.data_level ne '3AS' and l3.data_level ne '3BS') then goto, message3
sfdu = s1
label = l3
...
point_lun, unit, 2*long(reclen) + 64 + 4*2*long(pts)
num_params=0L
readu, unit, num_params

data = {satellite_id:'    ', record_type:'  ', instrument_id:'            ', $
       phys_rec_count:'        ', total_pts:0L, actual_pts:0L, $
       start_wavelen:0., time:lonarr(2), data:fltarr(pts), $
       quality:fltarr(pts), num_params:0L, param:strarr(2,num_params)}

cnt = long(label.num_phys_recs) - 1		; Number of Data Records.
out = replicate(data, cnt)

for i = 0, cnt - 1 do begin			; cnt should be 1 always.
  ...
  out(i).start_wavelen = float(rec(36:39),0)
...
; Check_math(0,1) prevents messages.
  ...
  out(i).num_params = num_params
  for j = 0, num_params - 1 do begin
    out(i).param(0,j) = string(rec(64 + 4*2*pts + 4 + j*40: $
                               64 + 4*2*pts + 4 + j*40 + 20 - 1))
    out(i).param(1,j) = string(rec(64 + 4*2*pts + 4 + j*40 + 20: $
                               64 + 4*2*pts + 4 + (j + 1)*40 - 1))
  endfor
...
goto, bye
message3: message, "must use a Level 3AS or 3BS *PROD or *META file."
