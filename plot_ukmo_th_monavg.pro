...
print,' Enter desired pressure sfc (0-21) '
print,'(1000, 681, 464, 316, 215, 147, 100, 68, 46, 32, 21)'
read,'(15, 10, 6.8, 4.6, 3.2, 2.1, 1.5, 1, .68, .46, .32) ',lsfc
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='psurf.ps'
...
if nday eq 0L then dat_avg=0.*dat
if nday eq 0L then u_avg=0.*u2
if nday eq 0L then v_avg=0.*v2
dat_avg=dat_avg+dat
u_avg=u_avg+u2
v_avg=v_avg+v2
nday=nday+1L
...
plotit:
dat_avg=dat_avg/float(nday)
u_avg=u_avg/float(nday)
v_avg=v_avg/float(nday)
...
; Mercator projection
      ...
      contour,dat_avg,x,alat,levels=level,/overplot,/cell_fill,$
              xtitle='Longitude',ytitle='Latitude',c_color=col1
      contour,dat_avg,x,alat,levels=level,/overplot,$
              xtitle='Longitude',ytitle='Latitude',$
              xrange=[0,360],yrange=[-90,90],c_color=[0],$
              c_labels=0*level,c_linestyle=level lt 0
contour,v_avg,xw,wlat,levels=[-20.,-18.,-16.,-14.,-12.,-10.,-8.,-6.,-4.,-2.],$
       /overplot
;!psym=8
;oplot,[90.,90.],[-5.,-5.],color=0
;stop
;!psym=0
      MAP_SET,0,0,0,/GRID,/CONTIN,/noeras,/noborder
      drawvectors,nlg+1,nlat-1,xw,wlat,u_avg,v_avg,3,0
...
; Orthographic projection
      ...
      contour,dat_avg,x,alat,levels=level,/overplot,$
              xtitle='Longitude',ytitle='Latitude',$
              xrange=[0,360],yrange=[-90,0],c_color=[0],$
              c_labels=0*level,c_linestyle=level lt 0
contour,v_avg,xw,wlat,levels=[-20.,-18.,-16.,-14.,-12.,-10.,-8.,-6.,-4.,-2.],$
       /overplot
;!psym=8
