...
lstmn=1L & lstdy=01L & lstyr=14L
ledmn=1L & leddy=30L & ledyr=14L
lstday=0L & ledday=0L
...
;        read,'Enter theta ',theta
      ...
      p1=transpose(p2(*,*,16))	; wind speed at 340 K
temp=th(16)*(p1/1000.)^.286
msf=transpose(msf2(*,*,16))
...
;        !p.font=0
         ...
         device,/landscape,/inch,xoff=4.25-ysize/2.,yoff=5.5+xsize/2.,xsize=xsize,ysize=ysize,$
                /bold,/color,bits_per_pixel=8,/times,filename='Collins/'+sdate+'_PV_LS+chicago.ps'
         !p.charsize=1.25
         !p.thick=2
         !p.charthick=5
         !p.charthick=5
      ...
      MAP_SET,90,0,-90,/stereo,/noeras,/grid,/contin,/noborder,$
              title=sdate,charsize=2.0,latdel=10,color=0,limit=[20.,0.,90.,360.]
...
;
; vortex edge between PV level and jet level
;
      loadct,0
;     contour,mark,x,alat,/overplot,levels=[-0.1],thick=10,color=200
      for k=16,thlev,-1 do begin	;thlev,-1 do begin
            theta=th(k)
            stheta=strcompress(string(fix(theta)),/remove_all)
            ...
            mark(0:nc-1,0:nr-1)=mark1(0:nc-1,0:nr-1)
            mark(nc,*)=mark(0,*)
            contour,mark,x,alat,/overplot,levels=[0.1],thick=15,color=((16.-float(k))/(float(16)-thlev))*mcolor,c_label=stheta,c_charsize=1.5,c_charthick=2
;           contour,mark,x,alat,/overplot,levels=[-0.1],thick=15,color=(float(k)/float(16))*mcolor,c_label=stheta,c_charsize=1.5,c_charthick=2
      endfor
      loadct,39
      contour,sp,x,alat,/overplot,levels=[35.],thick=10,color=.65*mcolor
      contour,sp,x,alat,/overplot,levels=[55.],thick=10,color=.75*mcolor
      contour,sp,x,alat,/overplot,levels=[75.],thick=10,color=.85*mcolor
      contour,sp,x,alat,/overplot,levels=[95.],thick=10,color=.95*mcolor
      MAP_SET,90,0,-90,/stereo,/noeras,/grid,/contin,/noborder,$
             charsize=2.0,latdel=10,color=0,limit=[20.,0.,90.,360.]
;
...
;   xyouts,.65,.12,'K'+ue+'hlungsborn, Germany',charsize=1.5,/normal,color=mcolor*.475,charthick=7
      ...
      if setplot eq 'ps' then begin
         device, /close
         spawn,'convert -trim Collins/'+sdate+'_PV_LS+chicago.ps -rotate -90 Collins/'+sdate+'_PV_LS+chicago.png'
;        spawn,'rm -f Collins/'+sdate+'_PV_LS+chicago.ps'
      endif
goto,jump
end
