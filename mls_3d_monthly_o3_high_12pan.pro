...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/portrait,bits=8,filename='mls_3d_o3_high_12pan.ps'
   device,/color
...
xyouts,.3,.95,'MLS Anticyclone Ozone',charsize=3,/normal
index=where(high_o3_avg_all eq 0.)
if index(0) ne -1L then high_o3_avg_all(index)=-9999.
contour,high_o3_avg_all,ybin,th,xrange=[-90.,90.],yrange=[400.,2000.],/noeras,/cell_fill,$
...
contour,high_o3_avg_all,ybin,th,/overplot,levels=level,/follow,color=0,min_value=-9999.,c_labels=0*level
index=where(high_o3_num_all le 0.)
oplot,x2d(index),y2d(index),psym=8,symsize=0.2
endfor		; loop over months
if setplot eq 'ps' then begin
   device,/close
   spawn,'convert -trim mls_3d_o3_high_12pan.ps mls_3d_o3_high_12pan.jpg'
endif
end
