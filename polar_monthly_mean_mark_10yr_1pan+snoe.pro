...
sth=strcompress(long(rth),/remove_all)
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='polar_wa3_mark_'+sth+'K+snoe.ps'
       device,/color
       device,/bold
...
erase
!type=2^2+2^3
xyouts,.3,.8,'WACCM3 + SNOE '+sth+' K',/normal,charsize=3
xmn=xorig(0)
xmx=xorig(0)+xlen
ymn=yorig(0)
ymx=yorig(0)+ylen
...
map_set,90,0,0,/stereo,/contin,/grid,/noeras,title='NH Summer (JJA)',charsize=2
...
for iyear=0L,nyear-1L do begin
    ...
    for i=0L,npmc-1L do begin
        oplot,[lon(index(i)),lon(index(i))],[lat(index(i)),lat(index(i))],psym=8,symsize=1.5,$
...
nlvls=n_elements(level)
col1=1+indgen(nlvls)*mcolor/nlvls
map_set,-90,0,0,/stereo,/contin,/grid,/noeras,title='SH Summer (NDJ)',charsize=2
...
imin=0.
set_viewport,xorig(0),xorig(1)+xlen,yorig(1)-cbaryoff,yorig(1)-cbaryoff+cbarydel
!type=2^2+2^3+2^6
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim polar_wa3_mark_'+sth+'K+snoe.ps -rotate -90 polar_wa3_mark_'+sth+'K+snoe.jpg'
;  spawn,'/usr/bin/rm polar_wa3_mark_'+sth+'K+snoe.ps'
endif
