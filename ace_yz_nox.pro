...
restore,dira+'no_ace_v2.2.'+syear0
...
if setplot eq 'ps' then begin
   ...
   ysize=nydim/100.
   device,font_size=9
   device,/landscape,bits=8,filename='ace_yz_nox_'+sdate0+'-'+sdate1+'.ps'
   device,/color
...
ymx=yorig(0)+ylen
set_viewport,xmn,xmx,ymn,ymx
level=[0.,0.1,0.2,0.5,1.,2.,5.,10.,20.,50.,100.,200.,500.,1000.,2000.,5000.,10000.,20000.,50000.,100000.]
...
contour,o3array,latbin,altitude,xrange=[-90.,90.],yrange=[1.,120.],xticks=6,$
        ...
        title='Zonal Mean ACE NO!lx!n',c_color=col1,color=0,min_value=-99.
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim ace_yz_nox_'+sdate0+'-'+sdate1+'.ps -rotate -90 ace_yz_nox_'+sdate0+'-'+sdate1+'.jpg'
endif
end
