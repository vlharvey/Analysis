...
; save postscript version
    if setplot eq 'ps' then begin
       ...
       !p.font=0
       device,font_size=9
       device,/landscape,bits=8,filename='3d_sh_'+mon(imonth)+'_geos5.ps'
       device,/color
...
;                  /T3D,zvalue=nz,thick=3,max_value=1.e15
           contour,dum,xcs,ycs,levels=[0.02],color=0,$
...
jumplev:
    ...
    if setplot eq 'ps' then begin
       device,/close
       spawn,'convert -trim 3d_sh_'+mon(imonth)+'_geos5.ps -rotate -90 3d_sh_'+mon(imonth)+'_geos5.jpg'
;      spawn,'/usr/bin/rm 3d_sh_'+mon(imonth)+'_geos5.ps'
    endif
