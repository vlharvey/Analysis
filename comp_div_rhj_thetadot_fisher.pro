...
rd_ukmo_th_new,imn,idy,iyr1,inc,inr,inth,xlon,xlat,th,$
           pv2,p2,msf2,u2,v2,q2,qdf2,mark2
nc=inc
nr=inr
nth=inth
x=fltarr(nc+1)
...
for n=0,4 do begin
dummy1=smooth(dummy1,3,/edge_truncate)
dummy1=smooth(dummy1,3,/edge_truncate)
dummy1=smooth(dummy1,3,/edge_truncate)
dummy1=smooth(dummy1,3,/edge_truncate)
dummy1=smooth(dummy1,3,/edge_truncate)
...
dummy=reform([dummy1,dummy1,dummy1],3*nc)
for n=0,4 do begin
dummy=smooth(dummy,3,/edge_truncate)
dummy=smooth(dummy,3,/edge_truncate)
dummy=smooth(dummy,3,/edge_truncate)
dummy=smooth(dummy,3,/edge_truncate)
dummy=smooth(dummy,3,/edge_truncate)
endfor
div2(j,*,l)=dummy(nc:2*nc-1)
endfor
endfor
...
snd=thd2(j,i,*)
pres=pn(j,i,*)
index=where(pres ge 50. and pres le 100.)
npnt=n_elements(index)
nbeg=index(0)
nend=index(npnt-1)
for l=nbeg,nend do begin
scale=(pres(l)-50.)/(100.-50.)
