;
; streamfunction and vortex edge
;
@rd_ukmo_nc3
...
thlev=index(0)
        ...
        if ipan eq 0 and setplot eq 'ps' then begin
           ...
           device,/landscape,bits=8,$
                   filename='Figures/'+ifile+'_'+stheta+'K_sf+mark.ps'
           device,/color
        ...
        MAP_SET,90,0,0,/ortho,/noeras,/grid,/contin,/noborder,$
                title='MetO Streamfuncion '+ifile+$
                '  '+stheta+' K',latdel=10
        !psym=3
        ...
        sfmax=max(sf(index))
        nlvls=20
        col1=reverse(1+indgen(nlvls)*icolmax/float(nlvls))
        sfint=(sfmax-sfmin)/(nlvls-1L)
        sflevel=sfmin+sfint*findgen(nlvls)
        contour,sf,x,alat,/overplot,levels=sflevel,c_color=col1,thick=1,$
                /cell_fill,/noeras
        contour,sf,x,alat,/overplot,levels=sflevel,/follow,color=0,$
                c_labels=0*sflevel
        contour,sf,x,alat,/overplot,levels=[180,185,190,195],/follow,thick=5,$
                c_labels=[1,1,1,1],color=0,charsize=3
        MAP_SET,90,0,0,/ortho,/noeras,/grid,/contin,/noborder,latdel=10,$
                color=0
...
;     endfor
        ...
        if setplot eq 'x' then begin
           ...
           write_gif,ifile+'_'+stheta+'K_sf+mark.gif',img
        endif
;stop
ipan=ipan+1
