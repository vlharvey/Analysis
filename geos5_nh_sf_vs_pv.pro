...
lstmn=12L & lstdy=1L & lstyr=2004L
ledmn=12L & leddy=1L & ledyr=2004L
lstday=0L & ledday=0L
;
; get date range
;
...
;
      if setplot eq 'ps' then begin
         ...
         !p.font=0
         device,font_size=9
         device,/landscape,bits=8,filename='geos5_nh_sf_vs_pv_'+sdate+'_'+slabs(0)+'_'+slev+'.ps'
         device,/color
...
;
      ...
      pvmax=max(pv(index))
      sfmin=min(sf(index))
      if sfmin lt 0. then sfmin=sfmin+0.5*sfmin
      if sfmin gt 0. then sfmin=sfmin-0.5*sfmin
      sfmax=max(sf(index))
      plot,findgen(10),findgen(10),/nodata,xrange=[sfmin,sfmax+0.5*sfmax],$
...
;
      ...
      if setplot eq 'ps' then begin
         device,/close
         spawn,'convert -trim geos5_nh_sf_vs_pv_'+sdate+'_'+slabs(0)+'_'+slev+'.ps -rotate -90 '+$
               'geos5_nh_sf_vs_pv_'+sdate+'_'+slabs(0)+'_'+slev+'.jpg'
         spawn,'/usr/bin/rm geos5_nh_sf_vs_pv_'+sdate+'_'+slabs(0)+'_'+slev+'.ps'
      endif
      jumplev:
      endfor
goto,jump
