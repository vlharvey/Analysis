;
; plot and save user entire season of daily MLS temperature at specified latitude range 
;
@stddat
@kgmt
@ckday
...
if setplot eq 'ps' then begin
   ...
   !p.font=0
   device,font_size=9
   device,/landscape,bits=8,filename='zt_mls_'+slat+'_tbar.ps'
   device,/color
...
;
erase
xyouts,.3,.95,'Tbar at Latitude= '+slat,charsize=2,charthick=2,/normal,color=0
xmn=xorig(0)
xmx=xorig(0)+xlen
ymn=yorig(0)
ymx=yorig(0)+ylen
...
if index(0) ne -1L then tbar(index)=0./0.
...
contour,tbar,dfs,altitude,levels=level,color=0,/follow,/overplot,c_labels=0*level
contour,tbar,dfs,altitude,levels=[150.],color=mcolor,/follow,/overplot,c_labels=0*level
contour,tbar,dfs,altitude,levels=[280.],color=0,thick=3,/follow,/overplot,c_labels=0*level
...
col2=1+indgen(11)
pdiff=0./0.*tbar
index=where(finite(tbar0) eq 1 and finite(tbar) eq 1)
...
if setplot eq 'ps' then begin
   device, /close
   spawn,'convert -trim zt_mls_'+slat+'_tbar.ps -rotate -90 zt_mls_'+slat+'_tbar.jpg'
;  spawn,'/usr/bin/rm -f zt_mls_'+slat+'_tbar.ps'
endif
end
